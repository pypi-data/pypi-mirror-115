...
class SessionHandler():
  ...
  def to_database(self, overwrite = False):
    ...
    obj = self
    if overwrite == True: obj = get_or_create(session, obj)
    session.add(obj)
    self.commit(session)
    return(self)
  ...
  def get_or_create(session, obj):
    params = {k: v for k, v in obj.__dict__.items() if v and k.startswith("_") == False}
    instance = session.query(obj.__class__).filter_by(**params).first()
    if instance:
        return instance
    else:
      obj
