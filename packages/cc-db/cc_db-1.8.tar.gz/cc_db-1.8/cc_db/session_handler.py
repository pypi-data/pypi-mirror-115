...
class SessionHandler():
  ...
  def to_database(self, overwrite = False):
    ...
    obj = self
    if overwrite == True: obj = self.update_or_create(session, obj)
    session.add(obj)
    self.commit(session)
    return(self)
  ...
  def all_to_database(self, obj_list, overwrite = False):
    ...
    for obj in obj_list:
      if overwrite == True:
        session = self.update_or_create(session, obj)
      else:
        session.add(obj)
    return self.commit(session)
  
  def update_or_create(self,session,obj):
    session_tmp = self.create_session()
    ...
    params = {k: v for k, v in obj.__dict__.items() if k.startswith("_") == False and k not in keys}
    q = session_tmp.query(obj.__class__).filter_by(**key_values)
    if q.count()==1:
      q.update(params)
      self.commit(session_tmp)
    else:
      session.add(obj)
    return session
