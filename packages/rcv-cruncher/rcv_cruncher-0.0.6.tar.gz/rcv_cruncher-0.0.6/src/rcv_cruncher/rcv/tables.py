...
class RCV_tables:
    ...
    def condorcet_tables(self):
        ...
        cleaned_dict = self.get_cvr_dict(self._contest_rule_set_name)
        ballot_set = [{'ranks': ranks.marks, 'weight': weight}
    ...
    def cumulative_ranking_tables(self):
        """
        ...
        # ballot rank limit
        ballot_length = self.stats()[0]['rank_limit'].item()

        # get cleaned ballots
        cleaned_dict = self.get_cvr_dict(self._contest_rule_set_name)
        ballot_set = [{'ranks': ranks.marks + (['NA'] * (ballot_length - len(ranks.marks))), 'weight': weight}
                      for ranks, weight in zip(cleaned_dict['ballot_marks'], cleaned_dict['weight'])]
    ...
    def first_choice_to_finalist_table(self, tabulation_num: int = 1) -> pd.DataFrame:
        ...
        ballot_set = [{'ranks': ranks, 'weight': weight, 'weight_distrib': distrib}
                      ...
                      in zip(self.get_initial_ranks(tabulation_num=tabulation_num),
                             self.get_initial_weights(tabulation_num=tabulation_num),
                             self.get_final_weight_distrib(tabulation_num=tabulation_num))]
    ...
    def round_by_round_table(self, tabulation_num: int = 1) -> pd.DataFrame:

        num_rounds = self.n_rounds(tabulation_num=tabulation_num)

        first_round_exhaust = self.stats()[tabulation_num-1]['total_pretally_exhausted'].item()
        ...
        ordered_candidates_names = [d['name']
                                    for d in sorted(reorder_dicts,
                                                    key=lambda x: (x['order'],
                                                                   -first_round_dict[x['name']],
                                                                   x['name']))
                                    ]
...
#
    ...
    def write_round_by_round_json(self, save_dir: Union[pathlib.Path, str]) -> None:

        n_tabulations = self.n_tabulations()
        for iTab in range(1, n_tabulations+1):

            # get rcv results
            n_rounds = self.n_rounds(tabulation_num=iTab)
            tally_tuples = [self.get_round_tally_tuple(round_num=i, tabulation_num=iTab)
                            for i in range(1, n_rounds+1)]
            transfer_dicts = [self.get_round_transfer_dict(round_num=i, tabulation_num=iTab)
                            for i in range(1, n_rounds+1)]

            outcomes = self.get_candidate_outcomes(tabulation_num=iTab)

            json_dict = {'config': {'notes': self.stats()[iTab-1]['notes'].item(),
                                    'date': self.stats()[iTab-1]['date'].item(),
                                    'jurisdiction': self.stats()[iTab-1]['jurisdiction'].item(),
                                    'office': self.stats()[iTab-1]['office'].item(),
                                    'threshold': '50%'},
                        'results': []}
            ...
            uid = self.stats()[iTab-1]['unique_id'].item()
