...
class MonitoringHistogram(MetricWrapperBase):
    ...
    def record(self, value_list):
        ...
        for i, value in enumerate(value_list):
            # the values from VAL start at the lower edge (i.e. edge) to the upper edge (edge + 1)
            # so we have to set the bucket count for the value index + 1
            # the first (and last) buckets would be set eventually by -Inf and +Inf buckets (see issue #123)
            self._buckets[i+1].set(value)
