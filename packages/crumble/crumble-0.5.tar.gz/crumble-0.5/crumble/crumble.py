...
class Crumble(object):
	...
	def parse(self,url):
		...
		if '(' in u and ')' in u:
			u = u[u.index('(')+1:u.index(')')].replace("'",'').replace(',','')
	...
	def __components__(self,url):
		...
		if res.group('a0') and url.linktype != 'relative':
			...
			if len(res.group('a0')[:s].split('.')) > 1:
				url.subdomain = res.group('a0')[:s].split('.')[:-1]
				for sub in url.subdomain:
					if '.'+sub in self.country:
						url.url_country_iso = self.country['.'+sub]['country'].strip('.')
				url.subdomain = '.'.join(url.subdomain)
				url.domain = res.group('a0')[:s].split('.')[-1]
			else:
