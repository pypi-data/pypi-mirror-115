    ...
    def read(self):
        ...
        for stmt in ofx.statements:
            ...
            if len(origin.account_id) == 22:
                ...
                bank_statement.errors.append("Não conseguimos identificar o número do seu cartão do Nubank, por favor, "
                                             "informe os 4 últimos dígitos de seu cartão.")
            if created_origin:
                bank_statement.read_status = BankStatement.INCOMPLETE
                bank_statement.errors.append("Encontramos transações sem origens bem definidas,por favor edite as "
            ...
            for tx in stmt.transactions:
                ...
                if cs.cash_date < cs.date:
                    cs.date, cs.cash_date = cs.cash_date, cs.date
                if cs.is_valid():
