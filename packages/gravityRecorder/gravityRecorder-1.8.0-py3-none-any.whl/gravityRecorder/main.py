...
class Recorder(Wsqluse):
    ...
    def update_opened_record(self, record_id, car_number=None, carrier=None,
        ...
        response = self.try_execute(command)
        if response:
            functions.add_change_notes(self, response, comment)
            if polygon:
                # Если поддерживается DUO - изменить присваивание
                self.reg_act_to_polygon(polygon, response)
            return {'status': 'success', 'info': 'Данные успешно изменены'}
        else:
            return response

    def get_table_dict(self, command):
        response = super().get_table_dict(command)
        if response['status'] == 'success':
            return response['info']

    def try_execute(self, command, returning='id'):
        response = super(Recorder, self).try_execute(command, returning)
        if response['status'] == 'success' and response['info']:
            print("RESPONSETARGET", response)
    ...
    def get_history(self, time_start, time_end, what_time='time_in',
        ...
        command = "SELECT rt.id as record_id, rt.trash_cat, rt.trash_type, rt.brutto, rt.cargo, rt.tara, rt.time_in, " \
                  "rt.time_out, rt.carrier, rt.notes, rt.operator, rt.wserver_sent, rt.wserver_get, " \
    ...
    def get_table_info(self, tablename, only_active=True, *args, **kwargs):
        ...
        print("GET TABLE INFO", response)
        response = {'table_info' : response}
