...
from .property import _get_nitrogen_content, _get_nitrogen_tan_content
...
def total_excreta_tan(inputs: list):
    """
    Get the total excreta ammoniacal nitrogen from all the inputs.
    ...
    kg_N_excreta = _filter_list_term_unit(excreta, Units.KG_N)
    kg_excreta = _filter_list_term_unit(excreta, Units.KG)
    excreta_as_tan = [list_sum(elem.get('value', [])) * _get_nitrogen_tan_content(elem) / 100 for elem in kg_N_excreta]
    excreta_conv_tan = [list_sum(elem.get('value', [])) * _get_nitrogen_tan_content(elem) / 100 for elem in kg_excreta]
    return list_sum(excreta_as_tan) + list_sum(excreta_conv_tan)


def total_excreta_n(inputs: list):
    """
    Get the total excreta nitrogen from all the inputs.
    ...
    kg_N_excreta = _filter_list_term_unit(excreta, Units.KG_N)
    kg_excreta = _filter_list_term_unit(excreta, Units.KG)
    excreta_as_n = get_total_value(kg_N_excreta)
    excreta_conv_n = [list_sum(elem.get('value', [])) * _get_nitrogen_content(elem) / 100 for elem in kg_excreta]
    return list_sum(excreta_as_n) + list_sum(excreta_conv_n)
