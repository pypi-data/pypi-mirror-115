...
def get_files(): return list(filter(lambda f: f.endswith(EXT), os.listdir(CURRENT_DIR)))


def _save_assessment(id: str):
    data = download_hestia(id, SchemaType.IMPACTASSESSMENT)
    file_path = f"{CURRENT_DIR}/{id}.jsonld"
...
def _file_exists(filename: str):
    ...
    if not exists:
        files = glob.iglob(os.path.join(CURRENT_DIR, '*' + EXT))
