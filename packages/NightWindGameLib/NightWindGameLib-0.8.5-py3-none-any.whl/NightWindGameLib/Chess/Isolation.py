...
class Main(QMainWindow, Ui_Isolation, Isolation):
    ...
    def get_rule(self):
        QMessageBox.about(parent=self, title="游戏规则", text=info)
        ...
        if self.started:
            ...
            if first == "先手":
                ...
                if level == "简单":
                        ...
                        dict(X=self.human_player, O=random_player),
                        verbose=True)
                elif level == "中等":
                    end = self.play_game(
                        ...
                        dict(X=self.human_player, O=self.mixed_player),
                        verbose=True)
                ...
                if level == "简单":
                        ...
                        dict(X=self.center_player, O=self.human_player),
                        verbose=True)
                elif level == "中等":
                    end = self.play_game(
                        dict(X=random.choice([self.open_player,
                                              self.center_player]),
                             O=self.human_player),
                        verbose=True)
                else:
