...
# all date syles, there are other styles like fractions etc. so have sort only dates
date_styles = [
    "14",
    "166",
    "167",
...
def readxlsx(file_name, temp_name="no_name"):
    ...
    with zipfile.ZipFile(file_name) as f_zip:
        ...
        temp_file = temp_file = tempfile.NamedTemporaryFile(
            prefix=worksheets[1]["name"], suffix=".csv", delete=False
        )
        data = scrape(f_zip, worksheets[1]["sheet_path"], shared_strings, styles)
        ...
        for r in range(1, max_row + 1):
            ...
            try:
                ...
                value = " \n"
            row += value
            temp_file.write(bytes(row, encoding="utf-8"))
    temp_file.seek(0)
    print(str(temp_file.read(), encoding="utf-8"), end="")
    return temp_file
...
def check_file(file_name):
    ...
    if type(file_name) is not str:
        raise UserWarning("Incorrect file entry ({}).".format(file_name))

    if not os.path.isfile(file_name):
        raise UserWarning("File ({}) does not exit.".format(file_name))
        ...
        if value == "":
            ...
            if test_value.isdigit():
                ...
                if styles[style] in date_styles:
                    ...
                    value = str(EXCEL_STARTDATE + timedelta(seconds=partialday * 86400))
                else:
                    value = str(value)
...
if __name__ == "__main__":
    in_str = "src/xlsxreader/excel_to_impala_demo.xlsx"
    readxlsx(in_str)
