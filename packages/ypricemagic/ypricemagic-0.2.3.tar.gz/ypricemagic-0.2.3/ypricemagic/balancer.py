...
from ypricemagic.utils.utils import get_decimals_with_override, PROXIES_reverse_lookup
...
from .constants import dai, usdc, wbtc, weth, PROXIES
...
def get_token_price_v2(token, block=None):
    ...
    print(list_pools_for_token_v2(token, block))
...
def get_token_price_v1(token, block=None):
    ...
    if not price:
        return
    print(price)
    print('price found via balancer v1')
    return price
