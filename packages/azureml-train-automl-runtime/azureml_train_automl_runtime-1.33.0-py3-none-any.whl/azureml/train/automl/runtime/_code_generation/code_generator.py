...
def generate_full_script(child_run: Run) -> str:
    with _codegen_utilities.use_custom_repr():
        ...
        if is_timeseries:
            ...
            output.append(get_modelwrapper_code(pipeline.steps[-1]))
            output.append("\n")
...
def get_model_code(sklearn_pipeline: Pipeline, featurization_enabled: bool) -> str:
    ...
    if isinstance(model, (PreFittedSoftVotingRegressor, PreFittedSoftVotingClassifier)):
        ...
        output.append("\n")
        output.append(get_build_model_pipeline_code(True, featurization_enabled))
        ...
        output.append("\n")
        output.append(get_build_model_pipeline_code(False, featurization_enabled))
...
def get_build_model_pipeline_code(is_ensemble: bool, featurization_enabled: bool) -> str:
...
def get_train_model_code(is_timeseries: bool) -> str:
    output = [
        ...
        "",
        "model = model_pipeline.forecast(X, y)" if is_timeseries else "model = model_pipeline.fit(X, y)",
        "return model",
    ]
