...
@LastEditTime: 2021-08-10 09:49:40
...
from seven_cloudapp_frame.models.db_models.user.user_base_model import *
...
class UserBaseModel():
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        try:
            ...
            if not user_base:
                user_base = UserBase()
                user_base.union_id = union_id
                user_base.open_id = open_id
                user_base.user_nick = user_nick
                user_base.avatar = avatar
                user_base.is_auth = 0
                user_base.user_state = 0
                user_base.create_date = now_datetime
                user_base.modify_date = now_datetime
                user_base.id = user_base_model.add_entity(user_base)
            else:
                if user_base.user_nick != user_nick or user_base.avatar != avatar:
                    user_base.user_nick = user_nick
                    user_base.avatar = avatar
                    user_base_model.update_entity(user_base, "user_nick,avatar")
                
            user_info = user_info_model.get_entity("act_id=%s and open_id=%s", params=[act_id, open_id])
            if not user_info:
                user_info = UserInfo()
                user_info.id = self._get_user_info_id(act_id, user_base.id)
            ...
            if open_id and acquire_lock_name:
    ...
    def update_user_info(self, app_id, act_id, user_id, open_id, user_nick, avatar):
        ...
        try:
            ...
            if not user_info_dict:
                ...
                user_base_model.update_table(update_sql, "id=%s", params=params)
    ...
    def get_prize_roster_list(self,app_id,act_id,user_id,open_id='',order_no=-1,goods_type=-1,prize_type=-1,logistics_status=-1,prize_status=-1,pay_status=-1,page_size=20,page_index=0,order_by="id desc",field="*",is_cache=True):
        """
    ...
    def check_pull_black(self,user_info_dict,is_black,refund_count,all_order_data):
        ...
        try:
            if user_info_dict["user_state"] == 0 and is_black == 1 and refund_count > 0:
                #退款的订单  子订单存在退款 记录一次
                refund_order_data = [i for i in all_order_data if i["refund_status"] not in SevenHelper.refund_status()]
