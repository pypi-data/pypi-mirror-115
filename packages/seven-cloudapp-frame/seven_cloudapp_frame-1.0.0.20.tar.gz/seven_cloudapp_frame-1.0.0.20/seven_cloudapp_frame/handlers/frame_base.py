...
@LastEditTime: 2021-08-03 14:15:15
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def reponse_common(self, success=True, data=None, error_code="", error_message=""):
        ...
        reponse_encrypt = config.get_value("reponse_encrypt", False)
        if reponse_encrypt == True:
    ...
    def check_continue_request(self, handler_name, app_id, object_id, expire=500):
        ...
        if object_id and handler_name and app_id:
            if SevenHelper.is_continue_request(f"continue_request:{handler_name}_{app_id}_{object_id}", expire):
                result = True, f"操作太频繁,请{expire}秒后再试"
        return result
        ...
        try:
            ...
            if dict_par["app_id"] and dict_par["open_id"]:
                ...
                if is_continue_request:
                    return self.reponse_json_error("error", error_message)
    ...
    def get_app_id(self):
        ...
        if app_id == "":
            ...
            if user_nick:
                ...
                if store_user_nick:
                    app_info_dict = AppInfoModel(context=self).get_dict("store_user_nick=%s", field="app_id", params=store_user_nick)
                    if app_info_dict:
                ...
                try:
                    ...
                    if clientheaderinfo_string:
                        ...
                        if now_time - device_dict["signature_stamp"] > int(1000 * 60 * 10):
                            return self.reponse_json_error("signature_stamp", "登录超时")
                        # 验证是否有头部信息签名
                        if not device_dict.__contains__("signature_md5"):
                            return self.reponse_json_error("signature_md5", "缺少参数signature_md5")
                        # 验证头部签名是否成功
                        ...
                        if signature_md5 != head_signature_md5:
                            return self.reponse_json_error("signature_fail", "头部签名验证失败")
                    else:
                        return self.reponse_json_error("no_device_info", "没有提交设备信息")
                except Exception as ex:
                    self.logging_link_error("【头部校验异常】" + str(ex))
                    return self.reponse_json_error("error", "头部验证失败")
