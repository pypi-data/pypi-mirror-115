...
@LastEditTime: 2021-07-27 15:41:23
...
class UpdateUserStatusHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        user_id = int(self.get_param("user_id"))
...
class UserBlackListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        user_base_model = UserBaseModel(context=self)
        self.reponse_json_success(user_base_model.get_black_list(app_id, act_id, page_size, page_index, audit_status, user_id, start_date, end_date, user_nick, open_id))

class UpdateUserAssetHandler(TaoBaseHandler):
    """
    :description: 变更资产
    """
    @filter_check_params("act_id,user_id,asset_type,asset_value")
    def post_async(self):
        """
        :description: 变更资产
        :param act_id：活动标识
...
class AssetLogListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        asset_base_model = AssetBaseModel(context=self)
        self.reponse_json_success(asset_base_model.get_asset_log_list(app_id, act_id, asset_type, page_size, page_index, user_id, asset_object_id, start_date, end_date, user_nick, user_open_id, source_type, source_object_id))
