...
@LastEditTime: 2021-08-03 10:22:58
...
class SubmitSkuHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        if not prize_roster or prize_roster.open_id != open_id or prize_roster.app_id != app_id:
            return self.reponse_json_error("no_user_prize", "对不起，找不到该奖品")
...
class SkuInfoHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = top_base_model.get_goods_list_by_goodsids(num_iids, app_info_dict["access_token"], is_log=is_log)
        ...
        if "items_seller_list_get_response" in invoke_result_data.data.keys():
            if "items" in invoke_result_data.data["items_seller_list_get_response"].keys():
                return self.reponse_json_success(invoke_result_data.data["items_seller_list_get_response"])
        else:
            act_prize = ActPrizeModel(context=self).get_dict("goods_id=%s and sku_json<>'' and is_sku=1 ", params=int(num_iids))
            if not act_prize:
                return self.reponse_json_error("error", "对不起，找不到该商品的sku")
...
class GoodsListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        if not app_info_dict:
            return self.reponse_json_error("error", "小程序不存在")
        invoke_result_data = top_base_model.get_goods_list(page_index, page_size, "", "", "", app_info_dict["access_token"], is_log=is_log)
        if invoke_result_data.success == False:
            return self.reponse_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
