...
@LastEditTime: 2021-08-02 12:00:13
...
class IpBaseModel():
    ...
    def save_ip_info(self, app_id,act_id,ip_id,ip_name,ip_pic,show_pic,sort_index,is_release):
        ...
        if is_add:
            ...
            ip_info_model.update_entity(ip_info)
        result = {}
    ...
    def get_ip_info_list(self,app_id,act_id,page_size,page_index,order_by="sort_index desc",is_cache=True):
        ...
        if is_cache == True:
            page_list, total = IpInfoModel(context=self).get_cache_dict_page_list(dependency_key=f"ip_info:{act_id}",cache_expire=600,field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        else:
