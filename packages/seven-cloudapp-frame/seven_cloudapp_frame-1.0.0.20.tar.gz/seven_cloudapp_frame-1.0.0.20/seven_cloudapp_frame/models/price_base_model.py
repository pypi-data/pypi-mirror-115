...
@LastEditTime: 2021-08-02 16:38:23
...
class PriceBaseModel():
    ...
    def get_price_gear_list(self,app_id,act_id,page_size,page_index,order_by="id desc",is_del=0,is_cache=True):
        ...
        price_gear_model = PriceGearModel(context=self.context)
        if is_cache == True:
            page_list, total = price_gear_model.get_cache_dict_page_list(dependency_key=f"price_gear:{act_id}",cache_expire=600,field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        else:
