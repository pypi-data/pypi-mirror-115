...
@LastEditTime: 2021-08-05 20:29:13
...
class AssetBaseModel():
    ...
    def _add_onlyid_warn_stat(self,app_id,handler_name=""):
        """
        :description: 添加唯一标识预警拦截计数
        :param app_id：活动标识
        :param handler_name：模块标识，没有填0
        :return: 
        :last_editors: HuangJianYi
        """
        if app_id and handler_name:
            handler_name = str(handler_name).lower()
            redis_init = SevenHelper.redis_init()
            
            hash_name_1 = f"app_handler_list_{str(SevenHelper.get_now_int(fmt='%Y%m%d'))}"
            hash_cache_key1 = f"appid_{app_id}_handlername_{handler_name}"
            if not redis_init.hexists(hash_name_1, hash_cache_key1):
                redis_init.hset(hash_name_1,hash_cache_key1, 1)
                redis_init.expire(hash_name_1, 24 * 3600)
            
            hash_name_2 = f"{hash_name_1}:appid_{app_id}_handlername_{handler_name}"
    ...
    def update_user_asset(self, app_id, act_id, module_id, user_id, open_id, user_nick, asset_type, asset_value, asset_object_id, source_type, source_object_id, source_object_name, log_title, only_id="",handler_name="",request_id="", info_json={}, sub_table=None):
        ...
        only_cache_key = ""
        if only_id:
            only_cache_key = f"asset_only_list:{app_id}_{SevenHelper.get_now_day_int()}"
            if redis_init.hexists(only_cache_key, only_id):
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
                invoke_result_data.error_message = "only_id已经存在"
                #添加唯一标识预警拦截计数,用于控制台跑数据进行并发预警
                self._add_onlyid_warn_stat(app_id,handler_name)
                return invoke_result_data
    ...
    def get_user_asset_list(self, app_id, act_id, user_id,open_id="", sub_table=None):
        """
        :description: 获取用户资产列表
        :param app_id：应用标识
        :param act_id：活动标识
        :param user_id：用户标识 多个逗号,分隔
        :param open_id：open_id 多个逗号,分隔
        :param sub_table：分表名称
        :return: 返回list
        :last_editors: HuangJianYi
        """
