...
@LastEditTime: 2021-08-05 09:41:39
...
class PrizeBaseModel():
    ...
    def get_act_prize_list(self,app_id,act_id,module_id,prize_name,is_del,page_size,page_index,is_cache=True):
        """
        ...
        act_prize_model = ActPrizeModel(context=self.context)
        if is_cache:
            page_list, total = act_prize_model.get_cache_dict_page_list(dependency_key=f"act_module_list:actid_{act_id}",field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by, params=params)
        else:
