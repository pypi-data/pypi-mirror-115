...
@LastEditTime: 2021-08-04 18:32:58
...
class IpBaseModel():
    ...
    def get_ip_info_list(self,app_id,act_id,page_size,page_index,order_by="sort_index desc",is_cache=True):
        ...
        if is_cache == True:
            page_list, total = IpInfoModel(context=self).get_cache_dict_page_list(dependency_key=f"ip_info_list:actid_{act_id}",cache_expire=600,field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        else:
