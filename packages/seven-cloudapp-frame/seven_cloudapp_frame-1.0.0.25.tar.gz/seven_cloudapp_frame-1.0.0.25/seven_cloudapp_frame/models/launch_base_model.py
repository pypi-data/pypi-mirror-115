...
class LaunchBaseModel():
    ...
    def init_launch_goods(self, app_id, act_id, source_types, online_url):
        ...
        for item in source_types:
            if int(item) == 1:
                prize_goods_id_list = ActPrizeModel(context=self.context).get_dict_list("act_id=%s and goods_id!=0 and is_del=0", field="goods_id", params=[act_id])
                if len(prize_goods_id_list) > 0:
                    goods_id_list += [int(goods_id["goods_id"]) for goods_id in prize_goods_id_list]
            elif int(item) == 2:
                gear_goods_id_list = PriceGearModel(context=self.context).get_dict_list("act_id=%s and goods_id!=0 and is_del=0", field="goods_id", params=[act_id])
        ...
        goods_exist_list = launch_goods_model.get_dict_list("act_id<>%s and goods_id in (" + goods_ids + ")", field="goods_id", params=act_id)
    ...
    def async_launch_goods(self, app_id, act_id, online_url, is_log=False):
        """
        ...
        if len(no_launch_goods_list) > 0:
            ...
            # 淘宝top接口
            invoke_result_data = top_base_model.miniapp_distribution_items_bind(no_launch_goods_ids, online_url, 'false', is_log)
        ...
        if len(launch_goods_list) > 0:
            ...
            # 淘宝top接口
            invoke_result_data = top_base_model.miniapp_distribution_items_bind(launch_goods_ids, online_url, 'true')
            if invoke_result_data.success == False:
                 invoke_result_data.success = False
                 invoke_result_data.error_code = "error"
                 invoke_result_data.error_message = "活动不存在"
    ...
    def launch_goods_list(self,app_id,act_id,page_size,page_index):
        """
        ...
        if len(launch_goods_list) > 0:
            ...
            top_base_model = TopBaseModel(context=self.context)
            invoke_result_data = top_base_model.get_goods_list_by_goodsids(goods_ids, self.get_taobao_param().access_token)
            if invoke_result_data.success == False:
                 invoke_result_data.success = False
                 invoke_result_data.error_code = "error"
                 invoke_result_data.error_message = "活动不存在"
