...
@LastEditTime: 2021-08-05 09:13:05
...
class DeleteActModuleHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        module_id = int(self.get_param("module_id", 0))
        act_base_model = ActBaseModel(context=self)
        invoke_result_data = act_base_model.update_act_module_status(app_id, module_id, 1)
        if invoke_result_data.success ==False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class ReviewActModuleHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        act_base_model = ActBaseModel(context=self)
        invoke_result_data = act_base_model.update_act_module_status(app_id, module_id, 0)
        if invoke_result_data.success ==False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
