...
@LastEditTime: 2021-08-09 16:34:21
...
class OrderBaseModel():
    ...
    def get_prize_order_list(self,app_id,act_id,user_id,user_open_id,nick_name,order_no,real_name,telephone,adress,order_status,create_date_start,create_date_end,page_size=20,page_index=0,order_by="id desc",field="*",is_search_roster=False,is_cache=True):
        """
        ...
        if is_cache:
            page_list, total = prize_order_model.get_cache_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition, group_by="",order_by=order_by, params=params,dependency_key=f"prize_order_list:actid_{act_id}")
        else:
    ...
    def create_prize_order(self,app_id,act_id,user_id,login_token,prize_ids,real_name,telephone,province,city,county,street,address):
        """
        :param app_id：应用标识
        :param act_id：活动标识
        :param user_id：用户标识
