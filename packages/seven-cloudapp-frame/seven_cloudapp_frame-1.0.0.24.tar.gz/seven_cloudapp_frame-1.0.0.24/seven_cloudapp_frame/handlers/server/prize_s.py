...
@LastEditTime: 2021-08-05 09:40:48
...
class ActPrizeListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        prize_base_model = PrizeBaseModel(context=self)
        page_list, total = prize_base_model.get_act_prize_list(app_id, act_id, module_id, prize_name, is_del, page_size, page_index, False)
        page_info = PageInfo(page_index, page_size, total, page_list)
        return self.response_json_success(page_info)


class ActPrizeHandler(TaoBaseHandler):
    """
    :description: 获取活动奖品信息
    """
    ...
    def get_async(self):
        ...
        prize_base_model = PrizeBaseModel(context=self)
        act_prize_dict = prize_base_model.get_act_prize_dict(prize_id, False)
        if act_prize_dict:
            if act_prize_dict["app_id"] != app_id:
                act_prize_dict = {}
        return self.response_json_success(act_prize_dict)
