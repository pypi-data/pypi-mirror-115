...
def graph_to_string(graph, libr = None):
  ...
  if libr is None:
    libr = lib
  diffs = [k[1]-k[0] for k in list(subgraph.edges())]
  if np.max(diffs)>1:
    branch_idx = np.where([j>1 for j in diffs])[0].tolist()
    branch_nodes = [list(subgraph.edges())[k][0] for k in branch_idx]
    branch = [list(subgraph.edges())[k][1] for k in branch_idx]
    branch_pool = [[k-1, k] for k in branch]
    branch = [[list(nx.get_node_attributes(subgraph, 'labels').values())[j] for j in k] for k in branch_pool]
    branch = [[libr[j] for j in k] for k in branch]
    branch = ["".join([k[j] if (j % 2) == 0 else '('+k[j]+')' for j in range(len(k))]) for k in branch]
  else:
    branch_pool = []
  glycan_motif = [libr[list(nx.get_node_attributes(subgraph, 'labels').values())[k]] for k in range(len(subgraph.nodes)) if k not in unwrap(branch_pool)]
