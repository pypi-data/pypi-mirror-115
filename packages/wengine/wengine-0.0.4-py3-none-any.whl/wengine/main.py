...
class WEngine:
    ...
    def start_round_thread(self, *args, **kwargs):
        ...
        if self.qwp.status_ready:
            ...
            response = {'status': 'failed', 'info': 'GCore занят в данный момент'}
        return response
    ...
    def operate_barrier(self, operation, barrier_name):
        """ Производит открытие/закрытие шлагбаумов"""
        if operation == 'close':
            return self.qsb.lock_point(barrier_name)
        else:
            return self.qsb.unlock_point(barrier_name)
    ...
    def make_record(self, weight, car_number, carrier=None, operator=None,
        ...
        response = self.qdw.init_round(weight, car_number, carrier, operator,
                                       trash_cat, trash_type, notes, polygon_int)
