...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_06_get_train_data(self):
        ...
        self.assertEqual(len(self.train_data), 569)
    ...
    def test_06b_get_train_data(self):
        data = self.remote_auto_pipelines.get_data_connections()[0].read(with_holdout_split=True)

        TestAutoAIRemote.train_data = data[0]
        holdout_data = data[1]

        self.assertEqual(len(self.train_data), 512)
        self.assertEqual(len(holdout_data), 57)
    ...
    def test_22b_serving_name_test(self):
        self.wml_client.set.default_space(self.space_id)
        ...
        self.wml_client.deployments.update(self.service.id, {'serving_name': self.deployment_serving_name_patch})
