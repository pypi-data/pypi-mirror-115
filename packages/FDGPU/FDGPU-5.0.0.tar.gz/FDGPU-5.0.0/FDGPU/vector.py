        ...
        if "spac" in kwargs or "h" in kwargs: # necessary for backward compatibility 0.5.2 => 0.6
            ...
            for i in range(self.ndims):
                for j in range(self.ndims):
                    if i==j:
                        self.components2.append(FinDiff(i, self.h[i], 2) )
                    else :
                        self.components2.append(FinDiff((i, self.h[i],1),(j,self.h[j],1)))

        if "coords" in kwargs:
            coords = kwargs.pop("coords")
            self.ndims = self.__get_dimension(coords)
            self.components = [FinDiff((k, coords[k], 1), **kwargs) for k in range(self.ndims)]
            self.components2 = [FinDiff(k, coords[k], 2) for k in range(self.ndims)]
            self.components2 = []
            for i in range(self.ndims):
                for j in range(self.ndims):
                    if i==j:
                        self.components2.append(FinDiff(i, coords[i], 2) )
                    else :
                        self.components2.append(FinDiff((i, coords[i],1),(j,coords[j],1)))
...
class Gradient(VectorOperator):
    ...
    def __call__(self, f):
        ...
        if not isinstance(f, np.ndarray):
            raise TypeError("Function to differentiate must be cupy.ndarray")
...
class Divergence(VectorOperator):
    ...
    def __call__(self, f):
        ...
        if not isinstance(f, np.ndarray) and not isinstance(f, list):
            raise TypeError("Function to differentiate must be cupy.ndarray or list of cupy.ndarrays")
...
class Hesian(VectorOperator):
    ...
    def __call__(self, f):
        ...
        result = []
        for k in range(self.ndims):
            dd_dxk2 = self.components2[k]
            result.append(dd_dxk2(f, acc=self.acc))

        return np.array(result)

def wrap_in_ndarray(value):
    """Wraps the argument in a cupy.ndarray.
