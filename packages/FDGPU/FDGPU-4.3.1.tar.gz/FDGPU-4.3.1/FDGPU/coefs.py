...
def coefficients(deriv, acc=None, offsets=None, symbolic=False):
    ...
    *Either* acc *or* offsets must be given.
    ...
    Assumes that the underlying grid is uniform. This function is available at the `FDGPU`
    package level.
...
def coefficients_non_uni(deriv, acc, coords, idx):
    ...
    :param coords:  1D cupy.ndarray: the coordinates of the axis for the partial derivative
