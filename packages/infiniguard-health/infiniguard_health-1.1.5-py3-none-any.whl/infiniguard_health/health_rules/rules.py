...
STATE_FOR_SNAPSHOTS_LIMIT_REACHED = ComponentState.ERROR
...
class SnapshotsNumRule(Rule):
    component_name = SYSTEM_COMPONENT_NAMES.snapshots
    component_change_message = Snapshots.component_change_message
    state_when_component_not_valid = STATE_FOR_SNAPSHOTS_LIMIT_REACHED
    component_positive_event = NumSnapshotsOk
    combine_positive_events = True

    @staticmethod
    def snapshots_nums_evaluator(snapshots: ComponentContainer, system_state: SystemState = None) -> List[Event]:
        from iba_mgmt.const import DDE_SNAPSHOTS_HIGH_WATERMARK

        num_snapshots = len(snapshots)
        snapshots_limit = DDE_SNAPSHOTS_HIGH_WATERMARK

        if len(snapshots) > snapshots_limit:
            return [SnapshotsNumExceeded(snapshots, num_of_snapshots_in_system=num_snapshots,
                                         snapshots_num_limit=snapshots_limit)]
    ...
    def __init__(self):
        ...
        super().__init__()
        # Annotation appears incorrect here, due to the wrapper
        self.negative_event_component_container_evaluators = [self.snapshots_nums_evaluator]
...
class ThresholdLevel:
    ...
    A level is defined according to  minimal value percent required to qualify for that level,
...
class ThresholdRule(Rule):
    ...
    def normal_level(self):
        return self.threshold_levels[0]

    @property
    def most_severe_level(self):
        return self.threshold_levels[-1]
    ...
    def get_current_threshold_level(self, component) -> ThresholdLevel:
        ...
        descending_levels = sorted(self.threshold_levels, key=lambda level: level.min_value, reverse=True)
