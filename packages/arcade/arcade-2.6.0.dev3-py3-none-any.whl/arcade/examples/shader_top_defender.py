...
class Player(arcade.SpriteSolidColor):
    ...
    def __init__(self):
        """ Set up player """
        super().__init__(40, 10, arcade.color.WHITE)
...
class MyGame(arcade.Window):
    ...
    float sun(vec2 uv, float battery)
    {
        ...
        float cut = 3.0 * sin((uv.y + 0.2 * (battery + 0.02)) * 100.0) 
    ...
    float grid(vec2 uv, float battery)
    {
        ...
        uv += vec2(iTime * 4.0 * (battery + 0.05), 0.0);
        uv = abs(fract(uv) - 0.5);
    ...
    def on_draw(self):
        ...
        self.shadertoy.draw(time=self.view_left * 0.001)
