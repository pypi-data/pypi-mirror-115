import { parseCodeCell, parseStepsFromCells } from '@boyuai/jupyter-helper';
        ...
        for await (const question of quizJSON.value) {
          ...
          if (
            ...
            if (!questionJSON.uuid) {
              throw new Error(`Quiz 解析失败: 题库题目 ${uuid} 解析异常`);
            }
            const questionIndex = quizJSON.value.findIndex(
              (option: any) => option.id === `uuid-${uuid}`
            );
            if (questionIndex === -1) {
              throw new Error(`Quiz 解析失败: 题库题目 ${uuid} 匹配失败`);
            }
