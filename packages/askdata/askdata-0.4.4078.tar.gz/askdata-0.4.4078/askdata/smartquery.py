...
class Condition:
    ...
    def __repr__(self):
        ...
        vuntils = []
        if self.vuntil is not None:
            for vunt in self.vuntil:
                vuntils.append(str(vunt))
            formatted_vuntil = "( " + ", ".join(vuntils) + " )"
        else:
            formatted_vuntil = None
        ...
        if formatted_value is not None and formatted_vuntil is None and self.direction is None:
            where_condition = (
                    field_with_agg + " " + operator + " " + str(formatted_value)
            )
        elif formatted_value is not None and formatted_vuntil is not None and self.direction is None:
            where_condition = (
                    field_with_agg + " " + operator + " " + str(formatted_value) + " " + str(formatted_vuntil)
            )
        elif formatted_value is None and formatted_vuntil is not None and self.direction is None:
            where_condition = (
                    field_with_agg + " " + operator + " " + str(formatted_vuntil)
            )
        elif formatted_value is None and formatted_vuntil is None and self.direction is not None:
