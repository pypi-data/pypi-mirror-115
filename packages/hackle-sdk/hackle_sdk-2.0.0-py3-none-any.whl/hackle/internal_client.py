...
class InternalClient(object):
    ...
    def decide_experiment(self, experiment_key, user, default_variation_key):
        ...
        experiment = config.get_experiment_or_none(experiment_key)
        if not experiment:
        ...
        evaluation = self.evaluator.evaluate(experiment, user, default_variation_key)
    ...
    def decide_feature_flag(self, feature_key, user):
        ...
        feature_flag = config.get_feature_flag_or_none(feature_key)
        if not feature_flag:
            return FeatureFlagDecision(False, DecisionReason.EXPERIMENT_NOT_FOUND)

        evaluation = self.evaluator.evaluate(feature_flag, user, 'A')
