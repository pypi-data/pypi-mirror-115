...
class UserAuthHelper:
    ...
    def get_user_auth_context_from_event(self, event):
        ...
        if requestContext is None:
            self.logger.warning("request context is none")
            return None

        authorizer = requestContext.get("authorizer")
        if authorizer is None:
            self.logger.warning("authorizer is none")
            return None

        claims = authorizer.get("claims")
        if claims is None:
            self.logger.warning("claims is none")
            return None

        email = claims.get("email")
        if (not isinstance(email, str)) or (email == ""):
            self.logger.warning("email none, invalid type, or empty")
            return None

        sub = claims.get("sub")
        if (not isinstance(sub, str)) or (sub == ""):
            self.logger.warning("sub none, invalid type, or empty")
            return None
