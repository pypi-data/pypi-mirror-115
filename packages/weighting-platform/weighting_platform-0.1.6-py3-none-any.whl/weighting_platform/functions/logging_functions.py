...
def init_gross_log(qdw):
    ...
    command = "INSERT INTO log_records (gross_log) VALUES ('{}')".format(message)
...
def init_tare_log(qdw):
    ...
    command = "UPDATE log_records set tare_log='{}' WHERE record={}"
    command = command.format(message, record_id)
    response = qdw.try_execute(command)['info'][0][0]
    return response
...
def fix_gross_event(qdw, message):
    ...
    command = "UPDATE log_records SET gross_log = gross_log || '{}' " \
              "WHERE record is null".format(message)
    print("EXECUTING_GROSS_COMMAND", command)
    response = qdw.try_execute(command)
    return response
...
def fix_tare_event(qdw, message):
    ...
    print("EXECUTING_TARE_COMMAND", command)
    response = qdw.try_execute(command)
    return response
