...
def process_sg_job_location_data(fact_data, geo_master, next_geo_key=1):
    ...
    if type(geo_master) == pd.core.frame.DataFrame:
        ...
        for m, processed_addr in enumerate(processed_addr_list):
            for i, row in processed_addr.loc[pd.isna(processed_addr["key.block_road_name"]) & update_filters[m], :].iterrows():
                block_road_name = row["block_road_name"]
                ...
                if sg_geo_cache.__contains__(block_road_name) and (not exist_searchkeys.__contains__(block_road_name)):
                    if (not added_searchkeys.__contains__(block_road_name)):
                        ...
                        processed_addr.loc[i, "key.block_road_name"] = str(next_geo_key)
                        ...
                        next_geo_key += 1
                    elif pd.isna(row["key.block_road_name"]) and (lat != 0):
                        processed_addr.loc[i, "key.block_road_name"] = added_searchkeys[block_road_name]

            for i, row in processed_addr.loc[pd.isna(processed_addr["key.postal_code"]) & update_filters[m], :].iterrows():
                block_road_name = row["block_road_name"]
                block_number = row["block_number"]
                road_name = row["road_name"]
                postal_code = row["postal_code"]
                ...
                if sg_geo_cache.__contains__(postal_code) and (not exist_searchkeys.__contains__(postal_code)):
                    if not added_searchkeys.__contains__(postal_code):
                        ...
                        processed_addr.loc[i, "key.postal_code"] = str(next_geo_key)
                        new_geo_master_key.append(str(next_geo_key))
                        new_geo_master_searchkey.append(block_road_name)
                        new_geo_master_block.append(block_number)
                        new_geo_master_road.append(road_name)
                        ...
                        new_geo_master_lng.append(lng)
                        next_geo_key += 1
                    elif pd.isna(row["key.postal_code"]) and (lat != 0):
                        processed_addr.loc[i, "key.postal_code"] = added_searchkeys[postal_code]

            for i, row in processed_addr.loc[pd.isna(processed_addr["key.road_name"]) & update_filters[m], :].iterrows():
                block_road_name = row["block_road_name"]
                block_number = row["block_number"]
                road_name = row["road_name"]
                postal_code = row["postal_code"]
                ...
                if sg_geo_cache.__contains__(road_name) and (not exist_searchkeys.__contains__(postal_code)):
                    if not added_searchkeys.__contains__(road_name):
                        ...
                        processed_addr.loc[i, "key.road_name"] = str(next_geo_key)
                        new_geo_master_key.append(str(next_geo_key))
                        new_geo_master_searchkey.append(block_road_name)
                        new_geo_master_block.append(block_number)
                        new_geo_master_road.append(road_name)
                        ...
                        new_geo_master_lng.append(lng)
                        next_geo_key += 1
                    elif pd.isna(row["key.road_name"]) and (lat != 0):
                        processed_addr.loc[i, "key.road_name"] = added_searchkeys[road_name]
