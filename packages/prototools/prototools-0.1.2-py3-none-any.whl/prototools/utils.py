...
from prototools.color import crear_estilo
...
def timer(_funcion=None, *, color: Optional[str] = None):
...
def continuar(indicacion: str = "Desea continuar? (s/n)") -> bool:
    ...
    """
    print(indicacion)
    return input().lower.starswith("s")
...
def test_funciones(
    ...
    print(maximo)
    for n in range(ns):
        bannerln(f"{mensaje} N°{n+1} | {args[n]:>{maximo}}")
...
def mostrar_matriz(m: list, ancho: int = 4) -> None:
