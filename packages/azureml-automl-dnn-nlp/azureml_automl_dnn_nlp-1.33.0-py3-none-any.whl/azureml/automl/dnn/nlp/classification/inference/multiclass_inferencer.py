...
class MulticlassInferencer:
    ...
    def load_training_artifacts(
        ...
        model_name_or_path = ModelNames.BERT_BASE_CASED
    ...
    def score(self, input_dataset_id: str, label_column_name: str) -> pd.DataFrame:
        ...
        inference_set = Dataset.from_pandas(_concat_text_and_preserve_label(df, label_column_name))
        if label_column_name in inference_set.column_names:
            inference_set = inference_set.remove_columns(label_column_name)
        inference_data = PyTorchMulticlassDatasetWrapper(inference_set, label_list, infer_data=True)
