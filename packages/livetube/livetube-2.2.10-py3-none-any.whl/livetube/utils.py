...
def parse_storyboard(return_type: str, spec_url: str, hq=False, length_seconds: int = 0):
    """Parse storyboard url

        Args:
            return_type: "url" or "parts" or "keys". url returns a list of urls, parts returns a dict with url and parts, keys returns a dict with sgp and sigh
            spec_url: The url provided in storyboard dict
            hq: There are two types of storyboards. The high quality storyboard contains a grid of maximum 25 stills, and a low quality (which is rendered first) which contains a grid of maximum 100 stills. Set true if you want high quality and false if you want a low quality grid.
            length_seconds: The lengthSeconds from videoDetails

        Returns:
            A list of urls or a dict, depending on return_type

        Raises:
            ValueError: If return_type is not "url", "parts" or "keys"

        Examples:
            >>> parse_storyboard("url", "https://i.ytimg.com/sb/dQw4w9WgXcQ/storyboard3_L$L/$N.jpg?sqp=-oaymwENSDfyq4qpAwVwAcABBqLzl_8DBgjG6eqGBg==|48#27#100#10#10#0#default#rs$AOn4CLA8K6eoNau8Nrfvw75AEozRajkKrw|80#45#108#10#10#2000#M$M#rs$AOn4CLDwls9iec-gEpWRBfceqOR896b3VQ|160#90#108#5#5#2000#M$M#rs$AOn4CLA7ioYvdz7gKhqPB3nVq40upUWrbw", True, 213)

        Notes:
            Original: https://www.npmjs.com/package/yt-storyboard
    """
    spec_parts = spec_url.split('|')
    ...
    if len(spec_parts) == 3:
        ...
        sigh_part_hq = spec_parts[1].split('t#')[1]
    else:
    ...
    if length_seconds < 250:
        amount_of_boards = math.ceil((length_seconds / 2) / division)
    elif 250 < length_seconds < 1000:
        amount_of_boards = math.ceil((length_seconds / 4) / division)
    elif length_seconds > 1000:
        amount_of_boards = math.ceil((length_seconds / 10) / division)

    storyboard_array = []

    for i in range(amount_of_boards):
        if hq:
            storyboard_array.append(f"{base_url_hq}M{i}{sgp_part}&sigh={sigh_part_hq}")
        else:
            storyboard_array.append(f"{base_url_lq}M{i}{sgp_part}&sigh={sigh_part_lq}")

    if return_type == 'url':
        return storyboard_array
    elif return_type == 'parts':
        return {
            'firstPart': base_url_hq + 'M',
            'secondPart': sgp_part + '&sigh=' + sigh_part_hq
        }
        ...
        raise ValueError("return_type must be 'url', 'parts' or 'keys'")
