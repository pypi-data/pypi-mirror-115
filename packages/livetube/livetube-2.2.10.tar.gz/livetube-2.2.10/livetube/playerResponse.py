"""
    ...
    Author: Sam
    Created: 2020/12/18 15:57
    File:    playerResponse.py
    Description: 
"""
import time
...
class streamingData:
    ...
    def __init__(self, data: dict, js_url: str):
        ...
        if adaptiveFormats:
            ...
            # Get best format
            real_best_bitrate: int = 0
            best_bitrate: int = 0
            best: Optional[dict] = None
            actual_best: Optional[dict] = None
            for _, formats in self.audios.items():
                if int(formats['bitrate']) > best_bitrate and formats['mimeType'].find("mp4") != -1:
                    best_bitrate = int(formats['bitrate'])
                    best = self.audios[formats['itag']]
                if int(formats['bitrate']) > real_best_bitrate:
                    real_best_bitrate = int(formats['bitrate'])
                    actual_best = self.audios[formats['itag']]

            self.audios['best'] = best if best else None
            self.audios['actual_best'] = actual_best if actual_best else None
...
class playerResponse:
    ...
    def __init__(self, player_response: dict, js_url: str):
        self.js_url = js_url
        if player_response.get("storyboards"):
            self.storyboards = player_response['storyboards']['playerStoryboardSpecRenderer']
    ...
    def update(self, update_items: dict):
        if update_items.get("storyboards"):
            self.storyboards = update_items['storyboards']['playerStoryboardSpecRenderer']
