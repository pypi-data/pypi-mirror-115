...
class ModuleApi(ApiBase):
    ...
    def get_module_cfg(self, ids: list = None, names: list = None) -> tuple:
        """
        根据板卡名称或板卡ID获取相应的板卡信息，两个参数只取其一如果两个参数同时存在优先取ID的值
        :param ids: 存储板卡ID的列表
        :param names: 存储板卡名称的列表
    ...
    def set_module_cfg(self, id: int = None, name: str = None, **kwargs):
        """
        修改板卡配置信息
        :param mid: 板卡ID
        :param mname: 板卡名称
        :return: 返回元祖类型数据，共3个元素：
                元素1: 状态码,1: 板卡配置修改成功 0: 板卡配置修改失败
                元素2: 数据信息，为空
                元素3: 状态描述
        """

        def logical(id, name=None, **kwargs):
            for key, value in kwargs.items():
                if key == 'clockppb':
                    self.module.clockppb(id, value)
                elif key == 'timesync':
                    self.module.timesync(id, value)
                else:
                    raise Exception(f'Unexpected config key: {key}')
                self.log(f'Setting {key} to {value} successful')
            return

        success_msg = 'All module config setting succeed'
        fail_msg = '[Module config setting error]'

        param = {'id': id, 'name': name, **kwargs}
    ...
    def release_module(self, id: int = None, name: str = None):
        """
    ...
    def reserve_module(self, id: int = None, name: str = None):
        """
