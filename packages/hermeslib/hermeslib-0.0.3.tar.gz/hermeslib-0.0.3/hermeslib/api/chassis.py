        ...
        def logical():
            ...
            for index, counts in enumerate(port_counts.split(' ')):
                ...
                if int(counts) > 0:
                    ...
                    # 占用板卡
                    # 先查询板卡状态
                    status = self.module.reservation(module)
                    # 闲置状态
                    if 'RELEASED' in status:
                        # 直接占用
                        self.module.reservation(module, 'RESERVE')
                    # 已被自己占用
                    elif 'RESERVED_BY_YOU' in status:
                        pass
                    # 已被其他人占用
                        ...
                        # 先释放他人连接
                        self.module.reservation(module, 'RELINQUISH')
                        # 再占用
                        self.module.reservation(module, 'RESERVE')
                    ...
                    # 释放板卡
                    self.module.reservation(module, 'RELEASE')
            # 占用仪表
