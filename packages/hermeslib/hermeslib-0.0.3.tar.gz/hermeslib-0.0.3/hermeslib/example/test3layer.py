...
def process1():
    from hermeslib.api.action import Action
    action = Action()
    # 连接仪表
...
def process2():
    ...
    import time
    from hermeslib.api.chassis import ChassisApi
    from hermeslib.api.module import ModuleApi
    from hermeslib.api.port import PortApi
    from hermeslib.api.stream import StreamApi
    ...
    # # 获取流配置信息
    for stream in streams:
        resp = sapi.get_stream_cfg(name=stream.get('name'))
        print(resp)
