...
def config(loader):
    ...
    loader.add_attr_list('wsd_cors_origins', [], help='Origins that are allowed as Access-Control-Allow-Origin header in WSD response')
...
EID_LOOP_RESPONSE_START = 1
EID_LOOP_RESPONSE_NEXT = 2
EID_LOOP_RESPONSE_END = 3
EID_DIVIDED_RESPONSES_APPEND = 4
EID_DIVIDED_RESPONSES_END = 5
...
class HttpdServer(object):
    ...
    async def get_web_service_discovery(self, request):
        headers = {}
        if 'Origin' in request.headers and request.headers['Origin'] in self.conf.wsd_cors_origins:
            headers['Access-Control-Allow-Origin'] = request.headers['Origin']
        return web.json_response(await self._create_wsd_dict(request), status=HTTPStatus.OK.value, headers=headers)
...
class WebSocketSession():
    ...
    async def send_bytes_if_available(self, request_id, event_id, data, loop_response_eid = 0, logger = None):
        log = logger if logger else self.logger
        if self.socket is not None and not self.socket.closed:
            try:
                if loop_response_eid == 0:
                    buf = msgpack.packb([request_id, event_id, data])
                else:
                    buf = msgpack.packb([request_id, loop_response_eid, [request_id, event_id, data]])
                size = len(buf)
                start = time.time()
                log.debug("SEND_BYTES|DATA=%s|MSGPACK_SIZE=%s", type(data), size)
                await asyncio.shield(self.socket.send_bytes(buf))
                log.debug("SEND_BYTES|DONE|TIME=%s, LOOP_RESPONSE=%s", time.time()-start, loop_response_eid)
                return True
            except Exception as e:
                log.exception("SEND_BYTES_ERROR=%s", e)

    async def send_bytes_or_wait_reconnect(self, request_id, event_id, data, loop_response_eid = 0, logger = None):
        log = logger if logger else self.logger
                ...
                if not self.socket.closed and self.request.transport is not None:
                    ...
                    try:
                        ...
                        if size < 1024**2 * 4:
                            ...
                            log.debug("SEND_BYTES|DRAIN|TIME=%s, LOOP_RESPONSE=%s", time.time()-start, loop_response_eid)
                            return True
                        else:
                            log.debug("SEND_BYTES_DIV|START|TIME=%s|DATA=%s|MSGPACK_SIZE=%s", time.time()-start, type(data), size)
                            bio = BytesIO(buf)
                            for buf in iter(lambda: bio.read(1024*1024 * 2), b''):
                                buf = msgpack.packb([request_id, EID_DIVIDED_RESPONSES_APPEND, buf])
                                async with self.send_lock:
                                    await asyncio.shield(self.socket.send_bytes(buf))
                                log.debug("SEND_BYTES_DIV|SENTB|TIME=%s", time.time()-start)
                                await asyncio.shield(self.socket.drain())
                            await asyncio.shield(self.socket.send_bytes(msgpack.packb([request_id, EID_DIVIDED_RESPONSES_END, None])))
                            await asyncio.shield(self.socket.drain())
                            log.debug("SEND_BYTES_DIV|DRAIN|TIME=%s, LOOP_RESPONSE=%s", time.time()-start, loop_response_eid)
                            return True
...
class WebSocketEventSession(EventSession):
    ...
    async def send_bytes_if_available(self, request_id, event_id, data, loop_response_eid = 0):
        return await self._ss.send_bytes_if_available(request_id, event_id, data, loop_response_eid, logger=self.logger)
    
    async def send_bytes_or_wait_reconnect(self, request_id, event_id, data, loop_response_eid = 0):
        return await self._ss.send_bytes_or_wait_reconnect(request_id, event_id, data, loop_response_eid, logger=self.logger)
    ...
    async def end_alive_monitoring(self, closed, auth):
        ...
        try:
            ...
            try:
                ...
                self.logger.warn('CLOSE_SESSION_FAILED|IGNORED=%s', e)
...
class WebSocketServer(object):
    ...
    async def _event_handler_alive_monitoring(self, session, msg, request_id, event_id, data, handle, event):
        ...
        try:
            ...
            async for ret in gen:
                if not await session.send_bytes_or_wait_reconnect(request_id, event_id, ret):
                    try:
    ...
    async def _event_handler_async_for(self, session, msg, request_id, event_id, data, handle, event):
        ...
        gen = handle(event)
        loop_response_eid = EID_LOOP_RESPONSE_START
        async for ret in gen:
            if not await session.send_bytes_or_wait_reconnect(request_id, event_id, ret, loop_response_eid):
                try:
                    gen.athrow(GeneratorExit)
                except GeneratorExit:
                    pass
                return False
            loop_response_eid = EID_LOOP_RESPONSE_NEXT
        await session.send_bytes_or_wait_reconnect(request_id, event_id, None, EID_LOOP_RESPONSE_END)
        return True
