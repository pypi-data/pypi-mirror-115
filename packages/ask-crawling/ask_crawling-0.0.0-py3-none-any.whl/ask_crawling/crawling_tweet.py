...
if __name__ == "__main__":
    ...
    http = 'http'
    RESULT_PATH_TWEET = path.RESULT_PATH_TWEET  # 결과 저장할 경로

    for i in range(len(query)):
        with open(fr'{RESULT_PATH_TWEET}/{output_file_name}', "w", encoding="utf-8") as output_file:
            statuses = twitter_api.GetSearch(term=query[i], count=10, lang='ko')
            ...
            for status in statuses:
                ...
                if mention_id != None and text.find(mention_id) != -1:
