...
class VoltageContext:
    ...
    def get_fine_chan_freqs_hz_array(self, voltage_coarse_chan_indices)->list:
        ...
        out_frequencies_len = len(voltage_coarse_chan_indices) * self.metafits_context.num_volt_fine_chans_per_coarse
        out_frequencies_type = ct.c_double * out_frequencies_len
        out_frequencies = out_frequencies_type()

        if mwalib_library.mwalib_voltage_get_fine_chan_freqs_hz_array(self._voltage_context_object,

                                                         error_message,
                                                         ERROR_MESSAGE_LEN) != 0:
            raise PymwalibVoltageContextGetFineChanFreqsArrayError(f"Error calling mwalib_voltage_context_get_fine_chan_freqs_hz_array(): "
                                                                   f"{error_message.decode('utf-8').rstrip()}")
