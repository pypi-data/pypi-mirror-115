...
def spec214_key_lookup(spec_122_schemas: schema_type_hint = None,
                       raw_214_schemas: raw_schema_type_hint = None) -> Dict[str, Tuple[str, str, str]]:
    """
    ...
    for section_214, schema_214 in raw_214_schemas.items():
        combined_schema = {name: {**schema_214[0]['spec214'], **schema} for name, schema in schema_214[1].items()}
...
def spec214_122_key_map(spec_122_schemas: schema_type_hint = None,
                        raw_214_schemas: raw_schema_type_hint = None) -> Dict[str, str]:
    """
    Generate a mapping of level0 214 key to 122 for translation of a 122 header.
    """
    return {key: value[0] for key, value in spec214_key_lookup(spec_122_schemas, raw_214_schemas).items()}
