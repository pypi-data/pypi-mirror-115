"""Implementation of LOESS smoothing from:
...
def main():
    ...
    with open("loess.csv", "w") as f:
        for x in np.linspace(xx.min(), xx.max(), num=100):
            e7 = loess.estimate(x, window=7, use_matrix=False, degree=1)
            e14 = loess.estimate(x, window=14, use_matrix=False, degree=1)
            f.write(f"{x}, {e7}, {e14}\n")
...
def plot():
    ...
    with open("data.csv", newline="") as f:
        reader = csv.reader(f)
        data = np.array([row for row in reader]).astype("f8")

    with open("loess.csv", newline="") as f:
        reader = csv.reader(f)
        smoothed = np.array([row for row in reader]).astype("f8")
    ...
    plt.scatter(xx, yy, s=4, label="data")
    plt.plot(xs, e7, "r", label="Window: 7")
    plt.plot(xs, e14, "m", label="Window: 14")
