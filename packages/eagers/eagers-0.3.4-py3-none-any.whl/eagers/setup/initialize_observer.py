...
from eagers.basic.get_data import get_data, get_weather
...
def initialize_building_observer(buildings,test_weather,date):
    ...
    for i in range(n_b):
        ...
        date = warm_up_date(building,date[0])
        weather = get_weather(test_weather,date)
        weather['timestamp'] = [d for d in date]
        T_zone,T_surf,humidity,frost = building_warmup(building,weather,date[0],None)
