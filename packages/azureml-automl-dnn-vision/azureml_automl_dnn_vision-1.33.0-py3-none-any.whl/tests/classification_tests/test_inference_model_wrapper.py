...
class TestInferenceModelWrapper:
    def test_inference_cpu(self, image_dir):
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            ...
            assert inference_model.resize_size == model_wrapper.resize_size
            assert inference_model.crop_size == model_wrapper.crop_size
    ...
    def _featurization_test(model_name, root_dir, image_dir, image_class_list_file_path,
        ...
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            ...
            model_wrapper.transforms = _get_common_valid_transforms(
                resize_to=model_wrapper.resize_size,
    ...
    def _score_test(model_name, root_dir, image_dir, image_class_list_file_path,
        ...
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            device = _get_default_device()
            model_settings = {ModelLiterals.RESIZE_SIZE: ModelParameters.DEFAULT_RESIZE_SIZE,
                              ModelLiterals.CROP_SIZE: ModelParameters.DEFAULT_CROP_SIZE}
            model_wrapper = ModelFactory().get_model_wrapper(model_name,
                                                             10,
                                                             multilabel=False,
                                                             device=device,
