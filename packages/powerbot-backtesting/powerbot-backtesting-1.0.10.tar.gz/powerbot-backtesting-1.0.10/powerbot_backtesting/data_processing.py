				...
				for k, v in orders.items():
					if v and k in ["bid", "ask"]:
						for x in v:
							...
							x["as_of"] = value.loc[nr, "as_of"].tz_localize(timezone.utc)
							bids_asks.append(x)
		else:
			...
			for nr, coll in enumerate(value):
				for k, v in coll.items():
					if v and k in ["bids", "asks"]:
						for x in v:
							x["type"] = k[:-1]
							x["best_bid"] = round(value[nr]["best_bid"], 2)
							x["best_bid_qty"] = round(value[nr]["best_bid_qty"], 2)
							x["best_ask"] = round(value[nr]["best_ask"], 2)
							x["best_ask_qty"] = round(value[nr]["best_ask_qty"], 2)
							x["vwap"] = round(value[nr]["vwap"], 2)
