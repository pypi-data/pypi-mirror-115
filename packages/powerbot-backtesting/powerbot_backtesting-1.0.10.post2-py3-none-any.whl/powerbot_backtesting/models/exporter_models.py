...
from powerbot_backtesting import init_historic_client, init_client, get_public_trades, get_contract_history, \
	get_contract_ids, get_ohlc_data, get_orderbooks, get_historic_data, plot_ohlc, plot_orderbook, plot_volume_history, \
	ohlc_table, get_signals, get_orders, get_own_trades, get_internal_trades, get_own_orders, get_public_trades_by_days, \
	calc_single_vwap, calc_trade_vwap, get_historic_contract_ids
...
class BaseExporter(ABC):
	...
	def get_ohlc_data(self,
		...
		"""
		return get_ohlc_data(trade_data=trade_data,
		                     timesteps=timesteps,
		                     time_unit=time_unit,
	...
	def get_orderbooks(self,
	                   contract_hist_data: dict[str, pd.DataFrame],
	                   delivery_area: str = None,
	                   timesteps: int = 15,
		...
		Returns:
			dict{key: DataFrame}: Dictionary of DataFrames
		"""
		return get_orderbooks(contract_hist_data=contract_hist_data,
		                      delivery_area=delivery_area,
		                      timesteps=timesteps,
		                      time_unit=time_unit,
		                      timestamp=timestamp,
	...
	def get_orders(contract_hist_data: dict[str, pd.DataFrame]) -> dict[str, list[dict]]:
		...
		"""
		return get_orders(contract_hist_data=contract_hist_data)
	...
	def calc_single_vwap(objects: dict[str, pd.DataFrame],
		...
		"""
		return calc_single_vwap(objects=objects,
	...
	def plot_ohlc(ohlc_data: dict[str, pd.DataFrame],
		...
		Returns:
			Plotly plot
		"""
		return plot_ohlc(ohlc_data=ohlc_data,
		                 ohlc_key=ohlc_key)

	@staticmethod
	def ohlc_table(ohlc_data: dict[str, pd.DataFrame],
		...
		Returns:
			DataFrame
		"""
		return ohlc_table(ohlc_data=ohlc_data,
		                  ohlc_key=ohlc_key)
	...
	def plot_orderbook(orderbooks: dict[str, dict[str, pd.DataFrame]],
		...
		Returns:
			Plotly plot
		"""
		return plot_orderbook(orderbooks=orderbooks,
	...
	def plot_volume_history(trade_data: dict[str, pd.DataFrame],
		...
		Returns:
			Plotly plot
		"""
		return plot_volume_history(trade_data=trade_data,
		                           trade_key=trade_key)
...
class ApiExporter(BaseExporter):
	...
	def __init__(self, api_key, host):
		self.client = init_client(api_key, host)
	...
	def get_contract_ids(self,
		...
		"""
		return get_contract_ids(api_client=self.client,
		                        time_from=time_from,
		                        time_till=time_till,
	...
	def get_public_trades(self,
		...
		Returns:
			dict{key: DataFrame}: Dictionary of DataFrames
		"""
		return get_public_trades(api_client=self.client,
		                         contract_ids=contract_ids,
		                         contract_time=contract_time,
		                         delivery_area=delivery_area,
	...
	def get_public_trades_by_days(self,
		...
		Returns:
			dict{key: DataFrame}: Dictionary of DataFrames
		"""
		return get_public_trades_by_days(api_client=self.client,
	...
	def get_contract_history(self,
		...
		Returns:
			dict{key: DataFrame}: Dictionary of DataFrames
		"""
		return get_contract_history(api_client=self.client,
		                            contract_ids=contract_ids,
		                            delivery_area=delivery_area,
		                            iteration_delay=iteration_delay,
		                            serialize_data=serialize_data,
	...
	def get_signals(self,
		...
		Returns:
			list[Signal]
		"""
		return get_signals(api_client=self.client,
		                   time_from=time_from,
		                   time_till=time_till,
		                   delivery_area=delivery_area,
	...
	def get_own_trades(self,
		...
		Returns:
			list[Trade]
		"""
		return get_own_trades(api_client=self.client,
		                      time_from=time_from,
		                      time_till=time_till,
		                      delivery_area=delivery_area,
		                      portfolio_id=portfolio_id)
	...
	def get_internal_trades(self,
		...
		Returns:
			list[InternalTrade]
		"""
		return get_internal_trades(api_client=self.client,
		                           time_from=time_from,
		                           time_till=time_till,
		                           delivery_area=delivery_area,
		                           portfolio_id=portfolio_id)
	...
	def get_own_orders(self,
		...
		Returns:
			list[OwnOrder]
		"""
		return get_own_orders(api_client=self.client,
		                      delivery_area=delivery_area,
		                      portfolio_id=portfolio_id,
	...
	def calc_trade_vwap(self,
		...
		"""
		return calc_trade_vwap(api_client=self.client,
		                       contract_time=contract_time,
		                       delivery_area=delivery_area,
...
class HistoryExporter(BaseExporter):
	...
	def __init__(self, exchange: str, delivery_area: str):
		self.client = init_historic_client(exchange, delivery_area)
	...
	def get_historic_data(self,
		...
		"""
		return get_historic_data(api_key=api_key,
	...
	def get_contract_ids(self,
	                     ...
	                     contract_time: str = "all",
	                     products: list[str] = None,
	                     allow_udc: bool = False) -> dict[str, list[str]]:
		"""
		...
		Returns:
			dict{key: (list[str])}: Dictionary of Contract IDs
		"""
		return get_historic_contract_ids(client=self.client,
		                                 time_from=time_from,
		                                 time_till=time_till,
		                                 contract_time=contract_time,
...
class SQLExporter(BaseExporter):
	...
	def __init__(self,
	             ...
	             port: int):
		"""
		Initializes SQLExporter and establishes connection.
