...
# Text Datasets:
# 'datasets': [{'path': 'path', 'keys': ['main_key', 'local_key'], 'text_metrics': ['separation_score', 'mean_length', 'unique_count']}, {'path': 'path1', 'keys': ['main_key1', 'local_key1']}]
...
class TextPreprocessor(BasePreprocessor):
    def __init__(self, params):
        super().__init__(
            params=params,
            params_keys=['datasets', 'compare_pairs', 'tokenize', 'vectorize', 'calc_distance','text_metrics', 'text_cols']
        )
        self.params = params
        self.text_metrics = []

        if self.params.get('text_cols'):            
    ...
    def transform(self, df):
        ...
        if not self.params.get('compare_pairs'):
            return df
        ...
        if self.params.get('vectorize'):
            for idx,dataset in enumerate(datasets):
                ...
                if self.params.get('datasets') and self.params['datasets'][idx].get('text_metrics'):
                    text_metrics.extend(self.params['datasets'][idx].get('text_metrics'))

                self._vectorize(dataset, 
                    self._get_dataset_features(idx, main_key=False),
                    text_metrics)

        df_full = self._prepare_full_dataset(df, datasets)
        field_names = self._calculate_distance(df_full)
        
        return self._make_result_dataset(df, df_full, field_names)
    ...
    def _vectorize(self, dataset, text_features, metrics):
        ...
        for col in text_features:
            self._calc_vector_metrics(dataset.df, col, metrics, res_metrics, text_metrics=True)
        ...
        for col in text_features:
            self._calc_vector_metrics(dataset.df, col, metrics, res_metrics, text_metrics=False)

        self.text_metrics = res_metrics
    ...
    def _calculate_distance(self, df_full):
        ...
        if self.params.get('calc_distance'):
