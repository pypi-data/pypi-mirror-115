...
class RenderScope(object):
    ...
    def __init__(self, _shield, name):
        ...
        self.templates = {
            "max": {
                "command": '3dsmaxcmdio.exe "<scenedir>/<timestamp>_<scenenamex>.max" -v 5 -frames:<start>-<end> -outputName:"<destination>/<scenenamex>..exr"',
    ...
    #     if partial_path.startswith("max"):
    #         return ["windows"]
    ...
    def get_script_template(self, partial_path):
        if partial_path.startswith("max"):
            return self.templates["max"]["script"]
        return self.templates["standalone"]["script"]

    def get_task_template(self, partial_path):
        if partial_path.startswith("max"):
...
class VraySWRenderScope(RenderScope):
    ...
    def get_render_package_paths():
        ...
        max_paths = [p for p in path_list if p.startswith(MAX_PREFIX) and not "/" in p]
        max_paths = sorted(list(set(max_paths)))

        vray_paths = [p for p in path_list if p.startswith(VRAY_PREFIX)]
        vray_paths = sorted(list(set(vray_paths)))

        return max_paths + vray_paths
...
class ArnoldRenderScope(RenderScope):
    ...
    def get_render_package_paths():
        ...
        arnold_paths = [
            p for p in path_list if p in VALID_MAYA_ARNOLD_VERSIONS
        ]
        arnold_paths = sorted(list(set(arnold_paths)))

        return max_paths + arnold_paths
