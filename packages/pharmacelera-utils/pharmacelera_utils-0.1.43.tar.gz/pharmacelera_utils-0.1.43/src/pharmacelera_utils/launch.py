...
class Configs:
    ...
    def s3_path_finder(self, load_file: str):
        if load_file.startswith("s3://"):
            ...
            print(f"{Colors.BOLD}{Colors.OKBLUE}{conf_name} file will be downloading from S3{Colors.ENDC}\n")
            self.download(bucket_name, filepath, conf_name)
            return conf_name
        else:
            print(f"{Colors.BOLD}{Colors.OKBLUE}{load_file} file will be loading from local.{Colors.ENDC}\n")
            return load_file
