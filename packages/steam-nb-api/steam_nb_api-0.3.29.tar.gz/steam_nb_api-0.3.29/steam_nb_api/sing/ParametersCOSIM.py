            ...
            for l in range(0,N_LEDET):
                ...
                for i in range(len(CoilSections)):  # Number of Ports
                    if split:
                        ...
                        if SplitCount == len(CSections): i = SplitCount-1
                    struct = {}
                    if split:
                        struct["name"] = "Port_" + str(SplitCount) + "_" + str(0)
                    else:
        ...
        for n in range(0, N_LEDET):
            ...
            for i in range(len(CoilSections)):  # Number of Ports
                if split and len(CoilSections[n]) != len(CoilSections):
                    if i+1 != n and len(CoilSections[n])>1: continue
                    if i>=len(CoilSections[n]): continue
                if split:
                    split_idx = split_idx + 1
                for j in range(2):  # 2Ports in PSpice
                    struct = {}
                    if split:
                        struct["name"] = "Port_" + str(split_idx) + "_" + str(j)
                        if CombinedModel: LSections[i] = Components[0] + "_" + str(split_idx)
                    else:
                    ...
                    Number = [int(s) for s in LSections[i].split('_') if s.isdigit()][0]
                    struct["inputs"] = []
