...
class Notebook_LEDET:
    def __init__(self, nameMagnet, typeMagnet='Cos-Theta', verbose=False, headerLines=1, **kwargs):
        ...
        if self.typeMagnet == 'solenoid':
            self.solenoid_data = read_yaml('circuit', self.nameMagnet)
            self.solenoid_data['iStartQuench'] = int(self.solenoid_data['iStartQuench'])
            self.solenoid_data['magnet_name'] = self.nameMagnet
            for block_name in self.solenoid_data['blocks'].keys():
                self.solenoid_data['blocks'][block_name]['wire_name'] = self.solenoid_data['blocks'][block_name]['wire']
                self.solenoid_data['blocks'][block_name]['wire'] = read_yaml('conductor', self.solenoid_data['blocks'][block_name]['wire'])

                winding_cell_rad_size = float(self.solenoid_data['blocks'][block_name]['wire']['strand']['wBare_inGroup']) + 2 * float(
                    self.solenoid_data['blocks'][block_name]['wire']['strand']['wIns_inGroup']) + float(self.solenoid_data['blocks'][block_name]['r_PrePreg'])
                strand_ins_ax_size = float(self.solenoid_data['blocks'][block_name]['wire']['strand']['hBare_inGroup']) + 2 * float(self.solenoid_data['blocks'][block_name]['wire']['strand']['hIns_inGroup'])
                self.solenoid_data['blocks'][block_name]['wire']['strand']['wIns_inGroup'] = float(self.solenoid_data['blocks'][block_name]['wire']['strand']['wIns_inGroup']) + float(self.solenoid_data['blocks'][block_name]['r_PrePreg']) / 2

                try:
                    self.solenoid_data['blocks'][block_name]['A1'] = float(self.solenoid_data['blocks'][block_name]['A1'])
                except:
                    ref_bloc_name = self.solenoid_data['blocks'][block_name]['A1']
                    self.solenoid_data['blocks'][block_name]['A1'] = float(
                        self.solenoid_data['blocks'][ref_bloc_name]['A2'])  # use reference block name for A1
                if force_tnt:
                    self.n_layers = int(self.solenoid_data['blocks'][block_name]['nl'])
                    self.n_turns_per_layer = int(self.solenoid_data['blocks'][block_name]['ntpl'])
                    ...
                    if force_l:
                        strand_ins_ax_size = (self.solenoid_data['blocks'][block_name]['B2'] - self.solenoid_data['blocks'][block_name]['B1']) / self.n_turns_per_layer  # correct insulation thickness in axial direction to allow for loos/tight wind
                        hIns_inGroup = (strand_ins_ax_size - self.solenoid_data['blocks'][block_name]['wire']['strand']['hBare_inGroup']) / 2
                        if hIns_inGroup > 0:
                            self.solenoid_data['blocks'][block_name]['wire']['strand']['hIns_inGroup'] = hIns_inGroup
                        else:
                            print(block_name)
                            raise Exception(
                                f"Calculated hIns_inGroup for {self.solenoid_data['blocks'][block_name]} is {hIns_inGroup} and <0. Please check number of layers in the solenoid or switch to force_l = False")
                    else:
                        self.solenoid_data['blocks'][block_name]['B2'] = self.solenoid_data['blocks'][block_name][
                                                  'B1'] + strand_ins_ax_size * self.n_turns_per_layer  # adjust B2 to allow for larger wire - unrealistic as winding mandrel is usually fixed
                else:
                    self.solenoid_data['blocks'][block_name].n_layers = int(np.rint((self.solenoid_data['blocks'][block_name]['A2'] - self.solenoid_data['blocks'][block_name]['A1']) / winding_cell_rad_size))
                    self.solenoid_data['blocks'][block_name].n_turns_per_layer = int(np.rint((self.solenoid_data['blocks'][block_name]['B2'] - self.solenoid_data['blocks'][block_name]['B1']) / strand_ins_ax_size))
            self.Magnet.Options.flag_typeWindings = 1
            self.sol_obj = Solenoid_magnet(self.solenoid_data)
            if kwargs['model_no']:
                self.out_dir = os.path.join(Path(os.getcwd()).parent, self.nameMagnet, str(kwargs['model_no']))
            else:
            ...
            if 'recalc_field' in kwargs:
                if kwargs['recalc_field']:
                    self.sol_obj.save_B_map(self.out_dir)
                    self.sol_obj.save_L_M(Ind_matrix_file)
            else:
                self.sol_obj.save_B_map(self.out_dir)
                self.sol_obj.save_L_M(Ind_matrix_file)
            self.Magnet.Inputs.T00 = self.solenoid_data['T']  # [K]
            self.Magnet.Inputs.I00 = self.sol_obj.current[0]
        ...
        if self.typeMagnet == 'solenoid':    # this can be only done after _load_ROXIE has been called.
            self.load_ConductorData(self.sol_obj.wires, self.sol_obj.wire_groups)
            max_distance = np.max([self.Magnet.Inputs.wIns_inGroup+self.Magnet.Inputs.wBare_inGroup, self.Magnet.Inputs.hIns_inGroup+self.Magnet.Inputs.hBare_inGroup])*1.01    # take largest 101% of largest wire size
    ...
    def _load_ROXIE(self):
        ...
        elif self.typeMagnet == 'Cos-Theta':
            self.halfTurnToTurn = np.tile(np.arange(1, self.nTurns + 1), 2) # multi-pole geometry assigns the first half of half-turns as go-lines, and the second half as return-lines
    ...
    def load_ConductorData(self, Conductor_L, Turns_L):
        ...
        for i in range(len(Conductor_L)):
            ...
            if not Conductor in self.AvailableConductor:
                ...
                return
            if self.typeMagnet == 'solenoid':
                #conductor_data = self.sol_obj.magnet_dict[i]['wire']
                conductor_data = self.solenoid_data['blocks'][list(self.solenoid_data['blocks'].keys())[i]]['wire']
            else:
                conductor_data = read_yaml('conductor', Conductor)
            #conductor_data['cable'] = conductor_data['cable']
            #conductor_data['strand'] = conductor_data['strand']
            type = conductor_data['type']
            SC = list(conductor_data['strand']['SC_type'].keys())[0]
            SC_data = conductor_data['strand']['SC_type'][SC]

            Values = np.array([0.0]*self.nGroups)
            for key in conductor_data['cable'].keys():
                if key in self.Magnet.Inputs.__annotations__:
                    try:
                        Values[Group_Mask] = float(conductor_data['cable'][key])
                    except:
                        Values[Group_Mask] = self.__translateDict(conductor_data['cable'][key])
                    self.Magnet.fillAttribute('Inputs', key, Values)
            for key in conductor_data['strand'].keys():
                if key in self.Magnet.Inputs.__annotations__:
                    try:
                        Values[Group_Mask] = float(conductor_data['strand'][key])
                    except:
                        Values[Group_Mask] = self.__translateDict(conductor_data['strand'][key])
                    self.Magnet.fillAttribute('Inputs', key, Values)
            ...
            if type == 'rectangular':
                wire_area = conductor_data['strand']['wBare_inGroup'] * conductor_data['strand']['hBare_inGroup'] - ((4 - np.pi) * conductor_data['strand']['scr_b'] ** 2)
            ...
            try:
                self.Magnet.Options.fScaling_Pex_AlongHeight = (2 * float(conductor_data['cable']['insulationAroundCables'])) / (
                            2 * float(conductor_data['cable']['insulationAroundCables']) + float(conductor_data['cable']['insulationBetweenLayers']))
            except:
                self.Magnet.Options.fScaling_Pex_AlongHeight = 1
                pass
            try:
                if float(conductor_data['strand']['T_Ref_RRR']) == 293:
                    self.Magnet.Inputs.RRR_Cu_inGroup[Group_Mask]  = self.Magnet.Inputs.RRR_Cu_inGroup[Group_Mask] / 1.086
            except:
                pass

            if len(self.Magnet.Inputs.Jc_Nb3Sn0_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'Jc_Nb3Sn0_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.Bc2_Nb3Sn_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'Bc2_Nb3Sn_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.Bc2_NbTi_ht_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'Bc2_NbTi_ht_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.Tc0_Nb3Sn_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'Tc0_Nb3Sn_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.c2_Ic_NbTi_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'c2_Ic_NbTi_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.c1_Ic_NbTi_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'c1_Ic_NbTi_inGroup',np.array([0]* self.nGroups))
            if len(self.Magnet.Inputs.Tc0_NbTi_ht_inGroup)!=self.nGroups:
                self.Magnet.fillAttribute('Inputs', 'Tc0_NbTi_ht_inGroup',np.array([0]* self.nGroups))

            if self.typeMagnet == 'solenoid':
                if list(conductor_data['strand']['SC_type'].keys())[0]=="Nb-Ti":
                    Jc_B_T = conductor_data['strand']['SC_type']['Nb-Ti']['c1_Ic_NbTi_inGroup']+conductor_data['strand']['SC_type']['Nb-Ti']['c2_Ic_NbTi_inGroup']*conductor_data['strand']['Ic_B']
                    A_SC = conductor_data['strand']['Ic'] / Jc_B_T
                    Values[Group_Mask] = A_SC/(wire_area*1e6)
                    self.Magnet.fillAttribute('Inputs', 'f_SC_strand_inGroup', Values)

        if self.typeMagnet == 'solenoid':
            self.Magnet.Inputs.nT = self.sol_obj.ntpl
            self.Magnet.Inputs.l_mag_inGroup = np.pi * (self.sol_obj.Rins + self.sol_obj.Routs)  # Length of turns of each layer
        ...
        if self.typeMagnet == 'Cos-Theta':
            ...
            self.Magnet.Inputs.GroupToCoilSection = self.sol_obj.section
    ...
    def set_SimulationType(self, Scenario):
        ...
        if Scenario == '2D+1D':
            if self.typeMagnet == 'solenoid':
                self.Magnet.Inputs.iStartQuench = np.array([int(self.solenoid_data['iStartQuench'])], dtype=np.int32)
                file_path = os.path.join(self.out_dir, f"{self.nameMagnet}_All_NoIron_NoSelfField.map2d")
                vQ, self.MPZ = self.Magnet.adjust_vQ(file_path)
                self.Magnet.Inputs.vQ_iStartQuench = vQ[self.Magnet.Inputs.iStartQuench] * 2  # *2 to account for both directions of propagation
                self.Magnet.Inputs.lengthHotSpot_iStartQuench = self.MPZ[self.Magnet.Inputs.iStartQuench]
            else:
    ...
    def _solenoid_locals(self, t_PC, simul_time='file'):
        if isclose(self.solenoid_data['U_EE'], 0.0):
            tEE = 9999
            R_EE_triggered = 0
        else:
            tEE = t_PC
            R_EE_triggered = (self.solenoid_data['U_EE'] - self.Magnet.Inputs.I00 * (
                    self.solenoid_data['R_circuit'] + self.solenoid_data['R_crowbar']) - self.solenoid_data['Ud_crowbar']) / self.Magnet.Inputs.I00  # Resistance of the energy-extraction system [Ohm]
            if R_EE_triggered < 0:
                R_EE_triggered = 0
        if simul_time=='file':
            time_v_array = [item for sublist in [[rng['start'], rng['step'], rng['stop']] for rng in
                            self.solenoid_data['simul_time']] for item in sublist]
        else:
            time_v_array = simul_time
        sim_start = time_v_array[0]
        self.Magnet.Inputs.t_PC_LUT = [sim_start, t_PC, t_PC + 0.01]            # this overwrites the
        solenoid_dict = {
        't_PC': t_PC,
        ...
        'fL_I': [0, 1000],
        'fL_L': [1, 1],
        'tStartQuench': sim_start,#self.solenoid_data['tStartQuench'],
        'tQuench': sim_start,#self.solenoid_data['tStartQuench'],
        'tStart': sim_start,      # this is needed for power supply LUT
        'time_vector_params': time_v_array
        }
        return {**solenoid_dict}

    def writeLEDETFile(self, nameFileLEDET, locals, **kwargs):
        if self.typeMagnet == 'solenoid':
            locals = {**locals, **self.solenoid_data, **self._solenoid_locals(**kwargs)}
