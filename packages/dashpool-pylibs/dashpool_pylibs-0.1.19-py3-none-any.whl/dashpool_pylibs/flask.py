...
def SecureFlask(appName, dashApps=None, plotApi=None, updateApi=None, updateCrontab=None):
    ...
    flaskApp.config.update(
        TESTING=cfg.getboolean('Flask', 'debug'),
        SECRET_KEY=cfg.get('Flask', 'secret_key', fallback='5vqUQ6MsEmvZpAsQBYpppxA8gsLQSv'),
        SESSION_COOKIE_PATH="/",
        SESSION_COOKIE_SAMESITE="None",
        SESSION_COOKIE_SECURE=True,
    )

    scheme = cfg.get('Flask', 'scheme')

    # use HOST from the environment or from the config
    host = environ.get('SERVER_HOST', cfg.get('Flask', 'host'))
    ...
    staticPort = int(cfg.get('Ports', 'static'))
    # load the static server port from the config file
    backendPort = int(cfg.get('Ports', 'backend'))
    ...
    consumer_key = cfg.get('OAuth', 'app_client')
    consumer_secret = cfg.get('OAuth', 'app_secret')
    tenant_name = cfg.get('OAuth', 'app_tenant')
    ...
    if updateApi is not None:
        ...
        if hasattr(updateApi, 'registerMongo'):
            ...
            print("# Init Mongo DB")
            flaskApp.config['MONGO_URI'] = cfg.get('Flask', 'mongodb')
        ...
        for d in dashApps:
            ...
            if useUwsgi:
                ...
                def calcLeftHeader(aux):
                    ...
                    return [html.P("Last Update", className="text-right mr-2"), html.P(startTime.strftime("%Y-%m-%d %H:%M:%S"))], initial_data, session_id
            else:
                @newDashApp.callback([
                ...
                def calcLeftHeaderB(aux):
                    ...
                    return [], initial_data, session_id
