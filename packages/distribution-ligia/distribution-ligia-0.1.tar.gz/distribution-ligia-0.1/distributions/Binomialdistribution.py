...
class Binomial(Distribution):
    ...
    def calculate_stdev(self):
        ...
        self.stdev =1.00* math.sqrt(self.n * self.p * (1-self.p))
        return self.stdev
    ...
    def replace_stats_with_data(self):
        ...
        self.p = 1.0 * np.count_nonzero(data)/ self.n
    ...
    def pdf(self, k):
        ...
        #pass
        a = math.pow(self.p, k) * math.pow(1 - self.p, self.n -k)
        b =  1.0 * math.factorial(self.n) / (math.factorial(k)* math.factorial(self.n - k))
        pdf_of_k = a * b
        return pdf_of_k
    ...
    def plot_bar_pdf(self):
        ...
        for i in range(self.n):
            y[i] = self.pdf(x[i])
        axis_ = pd.DataFrame({'x':x, 'pdf_of_x':y}, columns=['x', 'pdf_of_x'])
        #dataset = pd.DataFrame({'label': label, 'images': list(images)}, columns=['label', 'images'])

        axis_.plot("x", "pdf_of_x", kind = "bar")
        axis_.title('Probability Density Function')
        axis_.xlabel('outcome')
        axis_.ylabel('pdf')
        return x,y
    ...
    def __add__(self, other):
        ...
        new_binomial.p = other.p
        new_binomial.n = self.n + other.n
        new_binomial.mean = self.calculate_mean()
        new_binomial.stdev = self.calculate_stdev()
        return new_binomial
