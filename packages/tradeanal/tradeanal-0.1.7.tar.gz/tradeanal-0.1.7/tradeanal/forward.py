...
# class Forward:
#
#     def __init__(self, strategy: Strategy, df: pd.DataFrame, param_ranges: Dict[str, list]):
#         self.strategy = strategy
#         self.df = df
#         self.param_ranges = param_ranges
#
#         self.result = pd.DataFrame(
#             columns=['epoch', 'coef_eff', 'train_profit', 'train_rel_profit', 'test_profit'] + \
#                      list(param_ranges.keys()) + \
#                      ['ts_train_start', 'ts_train_end', 'ts_test_start', 'ts_test_end']
#         )
#         self.epoch_strategies = {}
#
#     def run(self, window_train_days: int, window_test_days: int) -> pd.DataFrame:
#         """ Запустить форвардный анализ """
#         train_window = window_train_days * 24 * 60
#         test_window = window_test_days * 24 * 60
#
#         epochs = int((len(self.df) - train_window) / test_window)
#
#         epoch_train_ts = {}
#         tasks = {}
#
#     def get_calc_strategy_func(self):
#         def calc_strategy(df, *params):
#             self.strategy(df, params)
#
#     @staticmethod
#     def find_best_params(
#             strategy: Strategy,
#             df: pd.DataFrame,
#             param_ranges: Dict[str, list]
#     ) -> Tuple[float, Dict[str, float], Strategy]:
#         params = {}
#         for param_name in param_ranges:
#             params[param_name] = ng.p.TransitionChoice(param_ranges[param_name])
#         parametrization = ng.p.Instrumentation(
