...
class Instrument(IOpenable, IClosable):
    ...
    def __init__(self, service_name='', service_description=''):
        ...
        self._elasticsearch_logger = ElasticSearchLogger() if os.environ.get('ELASTICSEARCH_ENABLED') in ["true",
                                                                                                          "True"] else None
        self._log_counters = LogCounters()
        self._prometheus_counters = PrometheusCounters() if os.environ.get('PROMETHEUS_ENABLED') in ["true",
                                                                                                     "True"] else None
        self._http_endpoint = HttpEndpoint()
        self._prometheus_service = PrometheusMetricsService() if os.environ.get('PROMETHEUS_ENABLED') in ["true",
                                                                                                          "True"] else None
    ...
    def set_context(self, service_name: str, service_description: str):
        """
    ...
    def open(self, correlation_id: Optional[str] = None):
        """
        Opens the component.
