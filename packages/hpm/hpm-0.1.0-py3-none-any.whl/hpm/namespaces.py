...
def insert_subnamspace(parent: dict, subnamespace: Group):
    ...
    if parent['id'] == subnamespace.parent_id:
        parent['children'].append(
            {'id': subnamespace.id, 'name': subnamespace.name, 'url': subnamespace.web_url, 'children': [], 'projects': []})
        return parent
...
def create_namespace_tree(groups: List[Group], projects: List[Project]) -> list:
    ...
    for group in groups:
        if group.parent_id is not None:
            ...
            parent.append({'id': group.id, 'name': group.name, 'url': group.web_url,
                          'children': [], 'projects': []})
    ...
    projects = tmp
    if len(projects) > 0:
        parent.append({'id': projects[0].namespace['id'], 'name': 'Personal', 'url': f'https://gitedu.hesge.ch/users/{projects[0].namespace["name"]}/projects',
                    'children': [], 'projects': []})
    for project in projects:
