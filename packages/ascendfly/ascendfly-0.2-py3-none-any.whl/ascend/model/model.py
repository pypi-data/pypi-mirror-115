...
class DataSet():
    ...
    data:
        _dataset : the created dataset

    function:
        dataset    : get the created dataset
        add_buffer : bind an AscendArray to the data buffer, and then add 
...
class ModelDesc():
    ...
    data:
        _model_desc : the created model desc

    function:
        dataset    : get the created dataset
        add_buffer : band an AscendArray to the data buffer, and then add 
                     the data buffer to the created dataset.
    """  
...
class AscendModel():
    ...
    param:
        context   : model will run on this context
        model_id  : model's id
        tensor    : a ordered-dict to save all input and output tensors
        net_in_n  : model's input numbers
        net_out_n : model's output numbers

    function:
        __load_model          : load model from string type input parameter model_path and create input/output tensor
        __create_input_tensor : create input tensor according to input number.
        __create_output_tensor: create output tensor according to output number.
        ...
        __release_model       : release the resource, which include unload model and free memory
    # """
    def __init__(self,
                context,
                model_path):
    ...
    def feed_data(self, args:dict):
        """Feed data to model.
        Args:
            args is an input dict.
            use likes: feed_data({'input_tensor_name1':input_data1, 'input_tensor_name2':input_data2})
            'input_tensor_name*' is the node of model input, and input_data* is the preprocessed data(image).
    ...
    def get_tensor_by_name(self, out_node_name:str):
        ...
        Args:
            ...
            'out_node_name' is the node of model output.

        Returns:
            output tensor of node out_node_name after model inference.
        """
    ...
    def tensor_names(self):
        ...
        Args:
            None.
            use likes: 
                output_names = model.tensor_names
                
            'output_names' is the node of model input and output.

        Returns:
            input/output node's name of model.
        """
    ...
    def set_shape(self, tensor_name, shape):
        """ set model input dynamic batch size.
        Args:
            shape : a tuple (w, h) of dynamic input shape
...
if __name__ == "__main__":
    resource = Context({0})
    model_path = "../tests/modelzoo/centerface_noaipp.om"
    model_path = "../tests/modelzoo/bert_delete.om"
    array = np.ones(shape=(384, 384), dtype='float32')
    data = AscendArray(shape=(384, 384), dtype=np.dtype('float32'))
    print(data.to_np)
    data = AscendArray.clone(array)

    for _, ctx in resource.context_dict.items():
        model = AscendModel(res, ctx, model_path)
        model.feed_data({'data':data})
        model.run()
...
if __name__ == "__main__":
    ...
    resource = Context({0})
    model_path = "../tests/modelzoo/BERT_text.om"
    array = np.ones(shape=(1, 512), dtype='int32')
    data = AscendArray(shape=(1, 512), dtype=np.dtype('int32'))
    print(data.to_np)
    data = AscendArray.clone(array)

    pdb.set_trace()
    for _, ctx in resource.context_dict.items():
        model = AscendModel(ctx, model_path)
        model.feed_data({'input_ids':data, 'input_mask':data})
        print(model.tensor)
        prof = Profiling(ctx, model.model_id)
        ...
        ascend_out = model.get_tensor_by_name('item_embedding:0')
        out = ascend_out.to_np
        print(ascend_out.to_np)
        print(out.data)
        print(out.dtype)
