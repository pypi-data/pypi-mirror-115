...
class Frame():
    ...
    attr:
        _packet    : object of push packet or image(AscendArray)
        _is_last   : define this Frame is last or not
        _frame_id  : id number of this frame
        _shape     : shape info of this Frame. format like (w, h)
        _mem(optinal) : if the input packet is a av.packet.Packet and not last frame, we 
                     malloc a device momory to save Frame.
 
    static method:
        data       : the Frame data pointer
        width      : width info of Frame
        height     : height info of Frame
        size       : buffer size of this frame
        is_last    : last frame flag
        frame_id   : id number of this Frame
