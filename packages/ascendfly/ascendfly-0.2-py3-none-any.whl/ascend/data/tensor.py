...
def imgs2tensor(imgs, tensor_fmt='NCHW', tensor_ptr=None):
    ...
    Args:
        imgs       : list[img(AscendArray)], a list that contains multiple images,
                     shape (h, w, c), support RGB/BGR, YUV444
        tensor_fmt : data format of output tensor.
        tensor_ptr : data pointer of output tensor. If it is None, we will create a
                     AscendArray and bind the array's data pointer to it.

    Returns:
        tensor/None: (AsendArray) Tensor that contains multiple images, shape (
                     N, C, H, W) or shape (N, H, W, C)
    """  
    ...
    if tensor_fmt == 'NCHW':
        ...
        raise ValueError(f"Tensor format only accept 'NCHW' or 'NHWC', but got {tensor_fmt}.")

    if not tensor_ptr:
        tensor = AscendArray(tensor_shape, dtype=imgs[0].dtype, format=tensor_fmt)
...
def tensor2imgs(tensor, mean=(0, 0, 0), std=(1, 1, 1), to_rgb=True):
    ...
    Args:
        tensor : Tensor that contains multiple images, shape (N, C, H, W) or shape (N, H, W, C).
    ...
    Returns:
        list[np.ndrray]: A list that contains multiple images.
    """
    if not isinstance(tensor, AscendArray):
        raise TypeError(f"Input tensor expects an AscendArray, but got {type(tensor)}.")

    if tensor.ndim != 4:
        raise ValueError(f"Input tensor expects a 4-dim, but got {tensor.ndim}.")

    if tensor.format not in ["NCHW", "NHWC"]:
        raise ValueError(f"Input tensor's format only support 'NCHW' or 'NHWC', but given {tensor.format}.")

    assert len(mean) == 3, f"Input mean of images expects a 3-elements tuple, but got {len(mean)}."
    assert len(std) == 3, f"Input std of images expects a 3-elements tuple, but got {len(std)}."
