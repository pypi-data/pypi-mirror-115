...
class AscendArray():
    ...
    Attributes:
        ascend_data (read, write) : get(read) the pointer of data malloced by itself(_data's value), 
                                    or band it to a new memory.
        shape (read, write)       : Tuple of array dimensions. (should work with write ascend_data).
        ndim (read, write)        : Number of array dimensions.
        dtype (read, write)       : get or write the data type.
        size (read)               : Number of elements in the array.
        nbytes (read, write)      : Total bytes consumed by the elements of the array.
        itemsize (read)           : Length of one AscendArray element in bytes, which equal to size * dtype(size).
        flag (read)               : Information about the memory layout of the array.
    ...
    def __init__(self, shape, dtype, size=0, buffer=None, format=None, context=None, flag='DEVICE'):
        ...
        try:
            ...
            raise TypeError(f'Input dtype expect a numpy.dtype, but got {type(dtype)}')
    ...
    def context(self, context):
        ...
        if not hasattr(self, '_mem'):
            raise ValueError(f"This AscendArray instance not support to set context.")
        else:
            del self._mem
    ...
    def shape(self, shape):
        ...
        if len(shape) <= 0 or shape[0] == 0:
            raise ValueError('Input shape is empty or format is invalid in calling function shape')
    ...
    def dtype(self, dtype):
        try: 
            ...
            raise TypeError(f'Input dtype expects a numpy.dtype, but got {type(dtype)}')
    ...
    def format(self, format):
        if not isinstance(format, (str, int)):
            raise TypeError(f"Input format expects int or string, but got {type(format)}.")
    ...
    def reshape(self, shape):
        """only reshape the view of AscendArray, and the data is original.
            @note
            only modify the view of AscendArray.
        Args:
            shape : the shape to be reshaped.

        Returns:
            AscendArray : the original object.
        """
        assert isinstance(shape, tuple), \
    ...
    def to_numpy(self, nbytes=None):
        ...
        if nbytes and nbytes > self._nbytes:
            raise ValueError(f"Input nbytes must lower than {self._nbytes}, but got {nbytes}.")
        ...
        try:
            ...
            raise TypeError(f"Convert AscendArray data to numpy not support this type {self._dtype}.")
    ...
    def to_ascend(self, array):
        ...
        Use like:
            ...
            @notes: copy data of array to ascend_array
        ...
        if self._flag == 'HOST':
            raise ValueError(f'Method to_ascend only be used with DEVICE or DVPP memory')
    ...
    def astype(self, dtype):
        """ Cast a tensor form src data type to dst data type. Firstly, we try to use Cast operator 
            to do. If it fails, we use numpy astype method. 
        Args:
            dtype : the shape to be reshaped.

        Returns:
            AscendArray : dst data.
        """
        try:
    ...
    def transpose(self, axes=None):
        ...
        Args:
            axes : permute the axes of array

        Returns:
            data of AscendArray
        """
        if not isinstance(axes, (tuple, list)):
            raise TypeError(f"Input axis expects a tuple or list, but got {type(axes)}.")
    ...
    def clone(cls, array, context=None, format=None, flag="DEVICE"):
        """New a AscendArray instance and Clone all the attributes of np.ndarray to it.
        Args:
            array : A np.ndarray data to be cloned

        Returns:
            An AscendArray object.
        """
        assert isinstance(array, np.ndarray), \
            f'Input args array expects class np.ndarray object, but got {type(array)}.'

        if context and not isinstance(context, int):
            raise TypeError(f"Input context expects int type, but got {type(context)}.")
        ...
        cloned_array = cls(shape=array.shape, dtype=array.dtype, format=format, context=context, flag=flag)
    ...
    def to(self, ascendarray):
        """ Copy a AscendArray data to another ascendarray(AscendArray).
        Args:
            ascendarray : A AscendArray to copy
...
if __name__ == "__main__":
    ...
    pdb.set_trace()
    array = AscendArray((1280,), np.dtype('uint8'), size=size, buffer=ptr, flag='DEVICE')
