...
def aligned(value, func='vdec', aligned='w'):
    """Align size according to func.

    Args:
        val (int): input value to be aligned.
        func (str, optional): alinged method for specified function
            Default: 'vdec'.
        aligned (str, optional): aligne w or h, Default: 'w'.

    Returns:
        formal (int): A aligne value.
    """    
    if func not in ['jpegd', 'pngd', 'vdec', 'resize', 'crop', 'crop_and_paste', 'encode']:
        raise ValueError(f"Aligned func only support:'jpegd','vdec','resize','crop','crop_and_paste'.")
...
def calc_size(width, height, iformat):
    ...
    Returns:
        size (int): the caculated image size.
    """
    if not (isinstance(width, int) and width > 0):
        raise TypeError(f"input width expects an positive int, but got {type(width)}.")

    if not (isinstance(height, int) and height > 0):
        raise TypeError(f"input height expects an positive int, but got {type(height)}.")

    if not isinstance(iformat, int):
        raise TypeError(f"input iformat expects an int, but got {type(iformat)}.")
