...
class Aipp():
    ...
    param:
        ...
        shape   : the original image shape(tuple(w, h))
    ...
    def __init__(self, context, model_inst, tensor_name, **kwargs):
        if not isinstance(context, int):
            raise TypeError(f"Input context expects an int value, but got {type(context)}.")

        if not isinstance(model_inst, AscendModel):
            raise TypeError(f"Input model_inst expects an AscendModel, but got {type(context)}.")

        if not isinstance(tensor_name, str):
            raise TypeError(f"Input tensor_name expects a string, but got {type(tensor_name)}.")
        ...
        for index in range(model_inst.net_in_n):
            aipp_type, _, ret = acl.mdl.get_aipp_type(model_inst.model_id, index)
        ...
        for key, arg_val in kwargs.items():
            if key == 'shape':
                ...
                self.set_aipp(model_inst.model_id, model_inst.dataset, input_id=need_aipp[0])
    ...
    def shape(self, shape):
        ...
        if not isinstance(shape, tuple):
            raise TypeError(f"Input shape expects a tuple, but got {type(shape)}.")
        ...
        if ret != ACL_SUCCESS:
            raise ValueError(f"set aipp input image format failed, return {ret}.")
    ...
    def set_csc(self, csc_mat, i_bias_mat, o_bias_mat):
        ...
        if not isinstance(csc_mat, np.ndarray):
            raise TypeError(f"Input csc_mat expects a np.ndrray, but got {type(csc_mat)}.")

        if not isinstance(i_bias_mat, np.ndarray):
            raise TypeError(f"Input bias_mat expects a np.ndrray, but got {type(i_bias_mat)}.")

        if not isinstance(o_bias_mat, np.ndarray):
            raise TypeError(f"Input bias_mat expects a np.ndrray, but got {type(o_bias_mat)}.")
        ...
        if ret != ACL_SUCCESS:
            raise ValueError(f"set aipp color space convert parameters failed, return {ret}.")
    ...
    def swap_switch(self, swap_switch):
        ...
        if ret != ACL_SUCCESS:
            raise ValueError(f"set aipp swap R/B or U/V swith failed, return {ret}.")
    ...
    def set_means(self, means, input_id=0):
        ...
        if not isinstance(means, np.ndarray):
            raise TypeError(f"Input means expects a np.ndrray, but got {type(means)}.")
        ...
        ret = acl.mdl.set_aipp_dtc_pixel_mean(self.aipp_set, means[0], means[1], means[2], means[3], input_id)
        if ret != ACL_SUCCESS:
            raise ValueError(f"set input mean value of channel failed, return {ret}.")
    ...
    def set_min(self, minm, input_id=0):
        ...
        if not isinstance(minm, np.ndarray):
            raise TypeError(f"Input minm expects a np.ndrray, but got {type(minm)}.")
        ...
        ret = acl.mdl.set_aipp_dtc_pixel_min(self.aipp_set, minm[0], minm[1], minm[2], minm[3], input_id)
        if ret != ACL_SUCCESS:
            raise ValueError(f"set input minimum value of channel failed, return {ret}.")
    ...
    def set_var(self, var, input_id=0):
        ...
        if not isinstance(var, np.ndarray):
            raise TypeError(f"Input var expects a np.ndrray, but got {type(var)}.")
        ...
        ret = acl.mdl.set_aipp_pixel_var_reci(self.aipp_set, var[0], var[1], var[2], var[3], input_id)
        if ret != ACL_SUCCESS:
            raise ValueError(f"set aipp swap aipp variance pixel value failed, return {ret}.")
    ...
    def set_crop(self, rect, input_id=0):
        ...
        if not isinstance(rect, np.ndarray):
            raise TypeError(f"Input var expects a np.ndrray, but got {type(rect)}.")
        ...
        ret = acl.mdl.set_aipp_crop_params(self.aipp_set, 1, startx, starty, w, h, input_id)
        if ret != ACL_SUCCESS:
            raise ValueError(f"set aipp swap aipp variance pixel value failed, return {ret}.")
    ...
    def set_aipp(self, model_id, dataset, input_id=0):
        ...
        Returns:
            None.
        """
        ret = acl.mdl.set_aipp_by_input_index(model_id, dataset, input_id, self.aipp_set)
        if ret != ACL_SUCCESS:
            raise ValueError(f"Add aipp to model according to input model index failed, return {ret}.")
