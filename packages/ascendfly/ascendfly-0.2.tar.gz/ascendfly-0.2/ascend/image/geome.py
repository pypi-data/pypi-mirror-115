...
def align_img(img, func='vdec'):
    ...
    Args:
        img   : (AsendArray) image or croped area of image(bbox).
...
def create_align_img(w, h, format=PIXEL_FORMAT_YUV_SEMIPLANAR_420, func='vdec'):
    ...
    Args:
        w, h : the original size of image or (w, h) not to be aligned.

    Returns:
        created image : AscendArray
    """
...
class PicDesc():
    """ define a PicDesc object, to construct an pic info.

    param:
        image   : the output image bind with an AscendArray object, image.shape(tupe(h, w, c))
    ...
    @property
    def desc(self):
        """get the create pic desc or batch pic desc.

        Args:
            None

        Returns:
            pic desc or batch pic desc
        """
...
class Image():
    ...
    param:
        context   : the device resource of context
        stream    : 

    function:
        __pic_channel : create a pic process channel, and initial with a channel desc.
    ...
    def imdecode(self, array, format=PIXEL_FORMAT_YUV_SEMIPLANAR_420, return_shape=False):
        ...
        Args:
            ...
            format       : the supported decode image format.
            return_shape : return original shape(h, w, c) of image.

        Returns:
            image : the decoded image(AscendArray type)
        """
    ...
    def imencode(self, ext, img, params=None):
        ...
        Args:
            ...
            params : Format-specific parameters, not use.

        Returns:
            image : Output buffer resized to fit the compressed image.
        """
    ...
    def imresize(self, img, dsize, interpolation='huawei'):
        """ imresize(array, img, dsize, interpolation='huawei') -> retval
            ...
            @code
                ...
                resize(img, (w//2, h//2), interpolation);
            @endcode
            To shrink an image, it will generally look best with #INTER_AREA interpolation

        Args: 
            img           : (AscendArray) input image.
            dsize (tuple) : output image size (tuple[int](w, h));
            interpolation : interpolation method, the default method is researched by ourself.
        
        Return:
            dst_img(AscendArray): the resized image
        """
        if not isinstance(img, AscendArray):
            raise TypeError(f"Input img expects an AscendArray object, but got {type(img)}.")
    ...
    def imrescale(self, img, scale, interpolation='huawei', return_shape=False):
        ...
        Args:
            ...
            interpolation: Same as :func:`imresize`.
            return_shape : return scale shape not aligned.

        Returns:
            rescaled_img : (AsecendArray) The rescaled image.
        """
        if not isinstance(img, AscendArray):
            raise TypeError(f"Input img expects an AscendArray object, but got {type(img)}.")
    ...
    def imrotate(self, 
        ...
        Args:
            ...
            center (tuple[float], optional): Center point (w, h) of the rotation 
                                in the source image. If not specified, the center 
    ...
    def imcrop(self, img, bboxes, scale=1.0):
        ...
        3 steps: caculate bbox shape -> clip bboxes -> do crop.

        Args:
            img (AscendArray) : Image to be cropped.
            bboxes (ndarray)  : Shape (k, 4) or (4, ), location of cropped bboxes.
            scale (float)     : Scale ratio of bboxes, the default value
                                1.0 means no padding.
            pad_fill (Number | list[Number]): Value to be filled for padding.
                                Default: None, which means no padding.

        Returns:
            list[AscendArray] | AscendArray: The cropped image patches.
        """
        if not isinstance(img, AscendArray):
    ...
    def bbox_resize(self, img, bboxes, sizes, inner_resize=True):
        ...
        Args:
            ...
            inner_resize      : use dvpp auto resize function to resize croped boxes

        Returns:
            list[AscendArray] | AscendArray: The cropped and resized image patches.
        """
        if not isinstance(img, AscendArray):
    ...
    def imcrop_paste(self, src_img, dst_img, crop_bbox, paste_bbox):
        ...
        Args:
            src_img    : Input src image(AscendArray) to be croped.
            dst_img    : Input dst image(AscendArray) to be pasted.
            crop_bbox  : Shape (4, ) or (n, 4), location of cropped bboxe, data arrangement 
                         np.array([startx, starty, endx, endy], dtype=int).
            paste_bbox : Shape (4, ) or (n, 4), location of pasted bboxe, data arrangement 
                         same to crop_bbox
