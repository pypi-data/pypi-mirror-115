...
class EncFrame():
    ...
    attr:
        frame_conf: initial frame config

    function:
        eos : create an eos frame
    """
    def __init__(self):
...
class Venc():
    ...
    attr:
        ...
        frame_interval   : key frame interval, and the defaut value is 25
        pix_fmt (int)    : the frame's pixel format, only support yuv420 NV12/NV21
        timeout (int)    : the encoding thread time out of dvpp 
 
    function:
        finish           : create a eos stream desc and set it to vdec.
        process          : assemble one frame and encode to video stream
    
    static method:
        width            : set/get frame's width
        height           : set/get frame's height
        encoder_type     : set/get encode type
        pix_fmt          : set/get the decoded frame's pixel format
        frame_itval      : set/get key frame interval
        rc_mode          : set/get frame number of video stream
        fps              : set/get frame per second of encoding stream 
        bit_rate         : set/get bit rate
    """
    def __init__(self, 
                context, 
                stream_dir,
