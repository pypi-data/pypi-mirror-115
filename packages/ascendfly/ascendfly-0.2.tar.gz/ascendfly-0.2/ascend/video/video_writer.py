...
class VideoWriter():
    ...
    attr:
        context (int)        : context resource video writer working on
        filename (str, Path) : the stream name to be writed
        fps (int)            : frame per second
        frameSize (tupe(w,h)): frame shape to be encoded.
    ...
    static method:
        ...
        frames              : Number of frames in the video file.
        format              : video stream format
        fps                 : Frame rate.
        bitrate             : (read-only) Video bitrate in kbits/s
        pix_fmt             : the encode frame's pixel format, only support yuv420 NV21/12
    """
    ...
    def set(self, attr, value):
        ...
        Args:
            attr : Property from VideoWriter Properties (eg. 'width', 'fps', ...)
            value: Value of the property

        Returns:
            bool value: `true` if the property is supported by the backend used by the VideoWriter instance.
        """
        if self._status == Status.READY:
            if attr == 'width': 
