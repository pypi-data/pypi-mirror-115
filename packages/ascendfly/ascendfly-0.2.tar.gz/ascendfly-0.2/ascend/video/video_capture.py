...
class VideoCapture():
    ...
    attr:
        ...
        stream_path (str): an IP video stream for video capturing. stream_path expect an absolute path for video dir. 
        channel (int)    : Optional, the created video capture channel
    ...
    static method:
        ...
        coded_height        : video coded height
        codel_width         : video codel width
        display_aspect_ratio: display aspect ratio of video
        format              : video stream format
        framerate           : video stream framerate
        fps                 : video stream framerate, same to framerate
        has_b_frames        : bool value, indicate stream has b frames
        pix_fmt             : the decoded frame's pixel format
        frames              : frame number of video stream
    """
    ...
    def display_aspect_ratio(self):
        """get the video stream display aspect ratio.
        Args:
            None

        Returns:
            an fraction object like Fraction(16, 9).
        """
        return self.stream.display_aspect_ratio

    @property
    def format(self):
        """get the video stream format info like <av.VideoFormat yuv420p, 3840x2160>.
        Args:
            None

        Returns:
            an av.VideoFormat object.
        """
    ...
    def skip_frame(self, skip_type):
        ...
        Args:
            skip_type : a class of av.codec.context.SkipType
    ...
    def is_open(self):
        """open the video capture and ready to decode.
            ...
            1. Status IDLE : the idle status of decode, and the resource is uninitial, and
                             nothing can be used in instance.
            2. Status READY: class VideoCapture and Vdec is initialized, and waiting for start.
            3. Status START: this status start to open vdec and the dequeue is null, so it 
                             will push packet until dequeue has decoded-image data.
            4. Status RUN  : run video decoder and pull stream packet until packet is null.
            5. Status END  : end of push packet to the vdec, and pop image data until the 
                             dequeue is null.
        Args:
            None
    ...
    def set(self, attr, value):
        ...
        Args:
            attr : Property from VideoCapture Properties (eg. 'width', 'fps', ...)
            value: Value of the property

        Returns:
            bool value: `true` if the property is supported by the backend used by the VideoCapture instance.
        """
    ...
    def get(self, attr):
        ...
        Returns:
            Value : Value for the specified property. Value 0 is returned when querying a property that is
