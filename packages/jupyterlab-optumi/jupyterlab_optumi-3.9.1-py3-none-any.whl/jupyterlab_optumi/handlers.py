...
class LaunchNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            hashes = [hash_file(f) for f in paths]
            stats = [os.stat(f) for f in paths]
            lastModifieds = [datetime.datetime.fromtimestamp(stat.st_mtime).isoformat() + 'Z' for stat in stats]
            sizes = [str(stat.st_size) for stat in stats]
            uuid = data['uuid']
            timestamp = data['timestamp']
    ...
    try:
        ...
        if len(filesToUpload) > 0:
            ...
            try:
                ...
                lock.release()
    except (CancelledError, HTTPError) as e:
