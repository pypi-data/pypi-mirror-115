...
class ICsvDataService(IDataService):
    ...
    def add_or_update(self, type: T, objs: List[T], objs_as_df: pd.DataFrame = None) -> List[T]:
        ...
        if self.write_data(all_data, self.data_file_path):
            return self.retrieve_objs(type=type, ids=[str(vars(obj)[self.obj_identifier]) for obj in objs])
        else:
    ...
    def delete(self, ids: List[str]) -> Dict[str, bool]:
        try:
            existing_data = self.read_in_data(self.data_file_path)
        except:
            existing_data = None
