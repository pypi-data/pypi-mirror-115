...
import internal.rules_pb2
import internal.rules_pb2_grpc
...
class WEMOClient(object):
    ...
    def __update_rule__(self, rule:internal.rules_pb2.Rule) -> bool:
        req = internal.rules_pb2.WanemdRequest(request_id=self.__request_id__(), rule=rule)
    ...
    def connect(self, host:str=None, port:int=None) -> bool:
        ...
        self.client = internal.rules_pb2_grpc.ServiceManagerStub(self.channel)
    ...
    def clear(self, rule_id:int=1) -> bool:
        ...
        # Clear the rule
        clear_rule = internal.rules_pb2.Rule(id=rule.id, name=rule.name, enabled=rule.enabled)
        return self.__update_rule__(clear_rule)

    def get_rules(self) -> List[internal.rules_pb2.Rule]:

        req = internal.rules_pb2.WanemdRequest(request_id=self.__request_id__())
    ...
    def get_rule(self, rule_id:int=1) -> internal.rules_pb2.Rule:

        req = internal.rules_pb2.WanemdRequest(request_id=self.__request_id__(), rule_id=rule_id)
