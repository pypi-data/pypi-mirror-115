...
import wemopy.internal.rules_pb2_grpc
...
class WEMOClient(object):
    ...
    def __update_rule__(self, rule: wemopy.internal.rules_pb2.Rule) -> bool:
        req = wemopy.internal.rules_pb2.WanemdRequest(request_id=self.__request_id__(), rule=rule)
    ...
    def connect(self, host:str=None, port:int=None) -> bool:
        ...
        self.client = wemopy.internal.rules_pb2_grpc.ServiceManagerStub(self.channel)
    ...
    def clear(self, rule_id:int=1) -> bool:
        ...
        # Clear the rule
        clear_rule = wemopy.internal.rules_pb2.Rule(id=rule.id, name=rule.name, enabled=rule.enabled)
        return self.__update_rule__(clear_rule)

    def get_rules(self) -> List[wemopy.internal.rules_pb2.Rule]:

        req = wemopy.internal.rules_pb2.WanemdRequest(request_id=self.__request_id__())
    ...
    def get_rule(self, rule_id:int=1) -> wemopy.internal.rules_pb2.Rule:

        req = wemopy.internal.rules_pb2.WanemdRequest(request_id=self.__request_id__(), rule_id=rule_id)
    ...
    def set(self, rule_id:int=1, name:str=None, enabled:bool=None, rate:int=None, ul_rate:int=None, dl_rate:int=None,
