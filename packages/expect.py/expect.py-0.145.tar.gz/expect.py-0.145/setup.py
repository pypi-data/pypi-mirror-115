...
if len(sys.argv)==2:
  ...
  os.system( "chmod +x "+sys.argv[1]  )
                    ...
                    text='''#!/usr/bin/python
# -*- coding: UTF-8 -*-
if   len(os.popen("git config --global git.key").read()): 
    import os,base64
    # key = os.popen("git config --global git.key").read().rstrip("\n")
    key = os.popen("git config --global git.key").read().rstrip("\\n")
    sss = base64.b64decode( key  ).decode("utf-8")  ################## [babys->str]
    tmp = open( "/root/.ssh/id_rsa" , 'w+').write( sss )
    os.system("chmod 600 /root/.ssh/id_rsa")
    os.system("ssh-keyscan gitlab.com > /root/.ssh/known_hosts")
    print(os.popen("ssh -T git@gitlab.com").read())
else:
    print("沒有設定[ git config --global git.key ]")

                    '''
                    open("/usr/bin/gitlab","w").write(text)
                    os.system(f"chmod +x /usr/bin/gitlab") 
...
def baseOS(name,pic):
    ...
    if   len(os.popen("git config --global git.key").read()): 
        # value  = os.popen("git config --global git.key").read() 
        # os.system('git config --global '+str(name)+'  "'+ str(value) +'"')
        os.system('/usr/bin/gitlab')
        ...
        if  name=="git.key":
            os.system('echo "git config --global '+str(name)+' '+ str(value) +'">/content/id_rsa')
        elif name=="ssh.pub":
            os.system('cat /root/.ssh/id_rsa.pub >/content/id_rsa.pub')
        else:        
            ##os.system('git config --global '+str(name)+'  "'+ str(value) +'"')
            pass
    else:
...
## 宣告
baseOS("git.key","/root/.ssh/id_rsa")
baseOS("ssh.pub","/root/.ssh/id_rsa.pub")
...
# # -*- coding: UTF-8 -*-
# import os
# print(os.popen("git config --global ssh.key").read())
#                     '''
#                     open("/usr/bin/ssh.key","w").write(text)
#                     os.system(f"chmod +x /usr/bin/ssh.key")
#                     #!/bin/sh
#                     text='''#!/usr/bin/python
# # -*- coding: UTF-8 -*-
# import os
# if len(os.popen("git config --global git.key").read()):
#     print("使用了自定的[ git.key ]!!. 所以沒有產生pub!!.")
# else:
#     print(os.popen("git config --global ssh.pub").read())
#                     '''
#                     open("/usr/bin/ssh.pub","w").write(text)
#                     os.system(f"chmod +x /usr/bin/ssh.pub")
#                     #!/bin/sh
#                     text='''#!/usr/bin/python
# # -*- coding: UTF-8 -*-
...
#     print("使用了自定的[ git.key ]!!. 所以沒有產生pub!!.")
# else:
#     print(os.popen("cat /root/.ssh/id_rsa.pub").read())

#                     '''
#                     open("/usr/bin/cat.pub","w").write(text)
#                     os.system(f"chmod +x /usr/bin/cat.pub")
...
'''
                    ...
                    if not  len(os.popen("git config --global git.key").read()):
                          os.system("/usr/bin/git-ssh")
                          print(os.popen("/usr/bin/cat.pub").read())
                    else:    
                          print(os.popen("/usr/bin/gitlab").read())
