...
class Credential:
    def __init__(
        self,
        client: Union[ClientType, BaseDocument],
        user: Any = None,
...
class StaticClientAuth(BaseAuth):
    ...
    def verify(
        ...
        try:
            return Credential(client=self.client_model.validate(client_dict))
...
class StaticSecretAuth(BaseAuth):
    ...
    def verify(
        ...
        if client_dict.client_secret != client_secret:
            raise exceptions.APIException(
                status_code=self.error_status_code,
                error="permission_denied",
                detail="client_sercet not match",
        ...
        try:
            return Credential(
                client=self.client_model.validate(client_dict), server_call=True
            )
        except ValidationError as exc:
            raise exceptions.APIException(
                status_code=411,
