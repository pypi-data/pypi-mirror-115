...
def apply(path,image='CANDI_HC_001.nii.gz',output_name='test.nii.gz',model='raw_nu_NKI_freesurfer',permute=False):
    ...
    DATA = 'data'
    MODEL = 'model'
    prediction_dir = os.path.join(WORKING_PATH,'src','SubBrainSegment','result')
    print(WORKING_PATH)
    output_label_map = True
    config['model_file'] =  os.path.join(WORKING_PATH,'src','SubBrainSegment', MODEL,model + '_unet_model.h5')
