...
def run(args=None):
    ...
    suite1 = ParserDataToCase.parser_yaml_create_cases(yaml_dir) or unittest.TestSuite()
    # load json TestCase
    json_dir = os.path.join(dir, 'case_json')
    suite2 = ParserDataToCase.parser_json_create_cases(json_dir) or unittest.TestSuite()
    # load py TestCase
    case_dir = os.path.join(dir, 'case_py')
    suite3 = unittest.defaultTestLoader.discover(case_dir) if os.path.isdir(case_dir) else unittest.TestSuite()
    ...
    runner = TestRunner(suite=suite,
                        ...
                        **result)
    res = runner.run(thread_count=getattr(settings, 'THREAD_COUNT', 1))
    print(res)
