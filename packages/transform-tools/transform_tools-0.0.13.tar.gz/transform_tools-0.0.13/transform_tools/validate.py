try:
    ...
    # We support down to Python 3.6 so this is needed to prevent breaking change
    import importlib_resources as pkg_resources
import linter_configs
import logging
import json
import os
import requests
...
def lint_transform_configs(config_dir: str):
    """Run the python yamllinter on all metric configs"""
    linter_config = pkg_resources.read_text(linter_configs, "yaml_linter_config.yaml")
    conf = YamlLintConfig(content=linter_config)
    max_level = 0
    for _, fdata in read_config_files(config_dir).items():
        problems = linter.run(fdata, conf, "")
        level = show_problems(problems, "stdin", args_format="colored", no_warn=False)
        max_level = max(max_level, level)
    if max_level == linter.PROBLEM_LEVELS["error"]:
        raise Exception("Python yamllint linter failed. See console output above for errors")
...
def upload_configs(  # noqa: D
    ...
    # run python yaml linter in order to assert configs are following correct yaml syntax.
    # this is to avoid adhoc debugging for config writers later.
    lint_transform_configs(config_dir_to_use)
...
if __name__ == "__main__":
    ...
    upload_configs(mode, TRANSFORM_API_KEY, REPO, BRANCH, COMMIT, TRANSFORM_CONFIG_DIR, API_URL)
    print("success")
