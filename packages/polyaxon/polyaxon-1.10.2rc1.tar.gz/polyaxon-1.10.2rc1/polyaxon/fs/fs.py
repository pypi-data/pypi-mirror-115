...
def _get_fs_from_type(
    connection_type: V1ConnectionType, asynchronous: bool = False, use_listings_cache: bool=False, **kwargs
):
    ...
    if not connection_type or connection_type.kind in {
        ...
        return LocalFileSystem(auto_mkdir=kwargs.get("auto_mkdir", False), use_listings_cache=use_listings_cache)
    if connection_type.kind == V1ConnectionKind.WASB:
        from polyaxon.fs.azure import get_fs

        return get_fs(context_path=context_path, asynchronous=asynchronous, use_listings_cache=use_listings_cache, **kwargs)
