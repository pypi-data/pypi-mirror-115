...
class Script:
    ...
    @classmethod
    def asm_number_to_bsv_bytes(cls, word):
        '''Convert an ASM number to bytes attempting to emulate the bsv library.'''
        value = int(word)
        if abs(value) <= 2147483647:
            if word[0] == '0':

    @classmethod
    def asm_word_to_bytes(cls, word, emulate_bsv=False):
        '''Convert an ASM word to bytes, either a 1-byte opcode or the data bytes.

        If emulate_bsv is True then attempt to emulate the bsv javascript library.'''
        ...
        if word.isdigit() or word[0] == '-' and word[1:].isdigit():
            if emulate_bsv:
                return asm_number_to_bsv_bytes(word)
            value = int(word)
            if abs(value) <= 2147483647:
    ...
    @classmethod
    def from_asm(cls, asm, bsv_lib=False):
        ...
        return cls(b''.join(asm_word_to_bytes(word, emulate_bsv) for word in asm.split()))
