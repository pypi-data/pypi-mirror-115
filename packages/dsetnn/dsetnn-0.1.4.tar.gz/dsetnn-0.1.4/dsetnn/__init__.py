...
def gaussian_symmetric_image(generated_heatmap, padding_target=None, dilation_iters=0, return_filled=False, make_plots=False, normalize=True):
    ...
    extended_gaussian_heatmap = torch.nn.functional.pad(generated_heatmap, default_padding)
    ext_h, ext_w = extended_gaussian_heatmap.shape[-2:]
    original_center = default_padding[2], default_padding[0]

    gaussian_center = generated_heatmap.argmax()
    gaussian_center = torch.Tensor( [gaussian_center//w, gaussian_center%w] )

    x_mask = torch.arange(0, w)
    y_mask = torch.arange(0, h)
    ...
    not_filled_values = extended_gaussian_heatmap<=0
    
    if make_plots:
        plt.imshow(extended_gaussian_heatmap)
        plt.title('Extended heatmap before dilation')
        plt.show()
    
    if dilation_iters>0:
        dilation_kernel = torch.ones((1,1,3,3))
        dilated_gaussian_heatmap = extended_gaussian_heatmap.clone().unsqueeze(0).unsqueeze(0)
        ...
        if make_plots:
            ...
            plt.imshow(dilated_gaussian_heatmap)
            plt.show()
    
        max_heatmap_border = maximum_along_border(extended_gaussian_heatmap, not_filled_values, dilation_kernel)
        max_dilated = dilated_gaussian_heatmap[not_filled_values].max()
    ...
    filled_values = extended_gaussian_heatmap>0
    
    if make_plots:
        fig, ax = plt.subplots(1, 3, figsize=(15, 15))
        ax[0].imshow(generated_heatmap)
        ax[0].set_title('Original heatmap')
        ax[1].imshow(filled_values)
        ax[1].set_title('Filled pixels')
        ax[2].imshow(extended_gaussian_heatmap)
...
def batched_gaussian_symmetric_image(heatmaps, padding_target=None, dilation_iters=0, normalize=True):
    ...
    for batch_num in range(batches):
        ...
        for location_num in range(locations):
            extended_heatmap = gaussian_symmetric_image(heatmaps[batch_num, location_num], padding_target=padding_target, dilation_iters=dilation_iters, normalize=normalize)
