...
#
__QUESTIONS_LABEL__ = ["question", "says"]
__WELCOME_SAYING_LABELS__ = ["sys_reply_q1", "sys_welcome"]
__REFUSE_SAYING_LABELS__ = ["sys_reply_q2", "sys_refuse"]
__COMPLETE_SAYING_LABELS__ = ["sys_reply_complete", "sys_complete"]
__DISSABLE_WELCOME_LABEL__ = ["__DISSABLE_Q1__", "DISSABLE_WELCOME"]
...
class QAStage(Stage):
    ...
    def __init__(self, data):
        ...
        if get_value_from_dict_by_multi_name(d=data, names=__DISSABLE_WELCOME_LABEL__, default=False):
...
class REStage(Stage):
    ...
    def __init__(self, data):
        ...
        # if data.get("__DISSABLE_Q1__", False):
        if get_value_from_dict_by_multi_name(d=data, names=__DISSABLE_WELCOME_LABEL__, default=False):
            self.is_first_access = self.disabel_is_first_access
    ...
    def find_new_stages(self, kwargs):
        ...
        for filter_tuple in self.stages_filter:
            ...
            if symbol is None:
                ...
                assert len(symbol) == len(text), "變數比較條件與限定結果的數量不一致"
