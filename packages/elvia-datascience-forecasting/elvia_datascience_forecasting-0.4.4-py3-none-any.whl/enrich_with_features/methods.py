...
from enrich_with_features.errors import NotEnoughDataError, UnspecifiedTimeSpanError, DataNotFound
...
def ingest_data(  # NOSONAR
    ...
    if inference:
        ...
        inference_df = pd.DataFrame(
            index=pd.date_range(
                start=last_day,
                end=last_day+timedelta(hours=forecast_horizon-1), 
                freq='H',
                tz='utc'),
            columns=['trafo_id','value_wh']                    
        )
        # Append the new hours and set the index as a column again
        hist_cons_df = hist_cons_df.append(inference_df).reset_index(drop=True)
    ...
    # The logic is present in the function add_metadata
    hist_cons_df['substation_id'] = hist_cons_df.trafo.apply(add_metadata, df_metadata=df_metadata)
...
def enrich(
    ...
    :param bool time_zone: Weather data comes with timezone UTC. It can be set to Oslo-timezone by writing 'time_zone = True'. Defaults to False.
    :param bool deployment: If this function is used in deployment step, it should be set as 'True'. Default False.
    :param logging.Logger logger: Logger object, optional
    :param enrich_with_features.data_models.EndpointRequest endpoint_request: Object describing an HTTP request, optional
    :return: A pandas dataframe without index of 'date_tz' and feature of 'trafo'
...
def _cast_type_df(
    ...
    if 'station_id' in df.columns:
        ...
        log_counter = add_debug_trace(
                'DataNotFound: Could not find column station_id in hist_cons_df', 
                logger, endpoint_request, log_counter)
        raise DataNotFound 
