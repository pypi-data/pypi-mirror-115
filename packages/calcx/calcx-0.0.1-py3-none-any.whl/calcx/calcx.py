...
def _format_data(df, date_column='date', nv_column='sawnav', index_column=None, compute_index=False):
    ...
    df = df[[date_column, nv_column]].rename(index=str, columns=dict_columns)
    # 按选取的列进行筛选
    list_columns = dict_columns.values()
    # 排序
    df = df.ix[:, list_columns].sort_values(by=['date'], ascending=True).reset_index(drop=True)
...
class CalcxTarget:
    def __init__(self, df, date_column='date', nv_column='sawnav', index_column=None, compute_index=False, adjustment_factor=None):
