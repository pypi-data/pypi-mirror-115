...
from ...basic.os.file import check_file_exist
from ...basic.validation import input as input_checker
...
class INIAccessor:
    ...
    def load(self):
        if check_file_exist(self.file_path) is False:
            self.ini_file()
    ...
    def ini_file(self):
        ...
        for section_key in self.default_params.keys():
            for param_key in self.default_params.get(section_key).keys():
                ...
                config.set(section_key, param_key, input_checker.ensure_str(param_key, self.default_params.get(section_key)))
    ...
    def set(self, param_key: str, param_value: Any):
        param_key = param_key.lower()
