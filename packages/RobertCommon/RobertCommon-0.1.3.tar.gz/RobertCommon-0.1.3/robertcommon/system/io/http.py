...
from ...basic.error.utils import E_INTERNAL, InputDataError
        ...
        for i in range(0, retry):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return Response(
                        input.ensure_bool(
                            'success', response_json, default_to=False),
                        input.ensure_str('code', response_json),
                        input.ensure_str('msg', response_json),
                        response_json.get('data'))

                raise Exception(f'Unexpected result: {response.status_code} '
                                f'{response.text[:max_log_size]}')
            except Exception as e:
                last_error = e
                if log_error:
                    logutils.log_unhandled_error()

        last_error_msg = f'Failed to post JSON request to beopweb after ' \
                         f'all retries! {last_error.__str__()}'
        if log_error:
