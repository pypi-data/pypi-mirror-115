...
#        mb_func.menu.add_command(label='Wiener',command=self.wiener)
    ...
    def loadimage(self, event=None):
        ifilename = str(askopenfilename(filetypes=[("tiff", "*.tiff"),
                                              ...
                                              ("allfiles", "*")]))
        if ifilename != None:
            self.ui.image = Image.open(ifilename).convert(
                'L').resize((self.ui.W, self.ui.H), Image.ANTIALIAS)
    ...
    def loadbg(self, event=None):
        ifilename = str(askopenfilename(filetypes=[("tiff", "*.tiff"),
                                              ...
                                              ("allfiles", "*")]))
        if ifilename != None:
            self.ui.bgimage = Image.open(ifilename).convert('L')
            self.ui.bubgimage = self.ui.bgimage
            self.ui.refresh()
            self.ui.vidnum = 0

    def flipimv(self):
        self.ui.image = self.ui.image.transpose(Image.FLIP_TOP_BOTTOM)
        self.ui.refresh()

    def flipimh(self):
        self.ui.image = self.ui.image.transpose(Image.FLIP_LEFT_RIGHT)
        self.ui.refresh()

    def transposeim(self):
        self.ui.image = self.ui.image.transpose(Image.TRANSPOSE)
        self.ui.refresh()

    def flipbgv(self):
        if self.ui.bgimage != None:
            self.ui.bgimage = self.ui.bgimage.transpose(Image.FLIP_TOP_BOTTOM)
            self.ui.vflip = True
            self.ui.refresh()
    ...
    def flipbgh(self):
        if self.ui.bgimage != None:
            self.ui.hflip = True
            self.ui.bgimage = self.ui.bgimage.transpose(Image.FLIP_LEFT_RIGHT)
            self.ui.refresh()

    def transposebg(self):
        if self.ui.bgimage != None:
            self.ui.transpose = True
            self.ui.bgimage = self.ui.bgimage.transpose(Image.TRANSPOSE)
            self.ui.refresh()
