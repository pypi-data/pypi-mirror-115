...
from fateslist.classes import APIResponse, Ratelimit, Bot, User
import aiohttp
...
class BaseHTTP():
    user_agent = f"fateslist.py/{cfg.version}"       
    ...
    async def request(
        ...
        if auth:
            if not self.logged_in:
                raise ValueError("Not logged in to Fates List yet")
...
class BotHTTP(BaseHTTP):    
    async def set_stats(
        ...
        return await self.request(
            ...
            json={
                ...
                "shard_count": shard_count if shard_count else None
            },
            auth=True
        )
    ...
    async def get_bot(self):
        ...
        return Bot(self.id, api_res.json)
...
class UserHTTP(BaseHTTP):         
    async def get_user(self, debug: bool = False):
        ...
        return User(self.id, api_res.json)
