...
def dsfs_ratecalc(data,channel):
    """
    ...
    * None.
    ...
    if (logtype == 'cxdh'):
        for string in day_strs:
            ...
            savename = string[0:8]+'_'+logtype+'_'+level+'.csv'
            if os.path.exists(savename):
                ...
                temp = csvw(savename,combo_data,level=level)
    ...
    if ((logtype == 'xrfs')|(logtype == 'xray')):
        for string in hour_strs:
            for i in np.arange(4,dtype=int):
                ...
                savename = string[0:10]+minstr+'_'+logtype+'_'+level+'.csv'
                if os.path.exists(savename):
                    load_data = np.loadtxt(savename,delimiter=',')
                    combo_data = np.concatenate((data[date_bool],load_data),axis = 0)
                    combo_data = combo_data[np.argsort(combo_data[:,1])]
    ...
    if (logtype == 'dsfs'):
        for string in hour_strs:
            for i in np.arange(4,dtype=int):
                ...
                minstr = '00' if i == 0 else str(15*(i))
                savename = string[0:10]+minstr+'_'+logtype+'_'+level+'.csv'
                ratename = string[0:10]+minstr+'_'+logtype+'_rate_'+level+'.csv'
                if os.path.exists(savename):
                    load_data = np.loadtxt(savename,delimiter=',')
                    combo_data = np.concatenate((data[date_bool],load_data),axis = 0)
                    combo_data = combo_data[np.argsort(combo_data[:,1])]
                    ...
                    if (level != 3):
                        ...
                        temp = csvw(ratename,combo_data,level=level)
