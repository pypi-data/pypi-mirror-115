...
class TimeKeeper():
    def __init__(self, supress:str) -> datetime:
        '''
    ...
    def log(self) -> datetime:
        '''
        returns difference as relativedelta value of difference between last log and 2nd last log entry
        '''
        self.logs.append(datetime.datetime.now())
        diff = relativedelta(self.logs[-1], self.logs[-2])
        if self.supress == 'y':
            pass
        else:
    ...
    def get_log(self, log_idx:int) -> datetime:
        '''
        returns difference as relativedelta value of difference between given log and start time
        '''
        diff = relativedelta(self.logs[log_idx], self.start_time)
        if self.supress == 'y':
            pass
        else:
    ...
    def end(self) -> datetime:
        ...
        diff = relativedelta(self.end_time, self.start_time)
        if self.supress == 'y':
            pass
        else:
