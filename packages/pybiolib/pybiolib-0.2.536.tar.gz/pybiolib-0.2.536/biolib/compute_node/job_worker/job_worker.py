...
class _JobWorker:
    ...
    def _handle_save_job_wrapper(self, package: bytes):
        ...
        if job['app_version']['modules'] is not None and BiolibDockerClient.is_docker_running():
    ...
    def _run_app_version(self, app_version_id: str, module_input_serialized: bytes) -> bytes:
        job = BiolibJobApi.create(app_version_id)
    ...
    def _run_job(self, job: Job, module_input_serialized: bytes) -> bytes:
        ...
        root_job_id = job['public_id']
        while job['caller_job'] is not None:
            root_job_id = job['caller_job']
