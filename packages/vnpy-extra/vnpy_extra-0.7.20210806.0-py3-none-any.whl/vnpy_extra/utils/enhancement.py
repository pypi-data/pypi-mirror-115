...
PATTERN_PUT_OPTION = re.compile(r'\w+-?\d+-?P-?\d+')
PATTERN_CALL_OPTION = re.compile(r'\w+-?\d+-?C-?\d+')
...
PATTERN_OPTION_TYPE_EXCHANGE_SPLIT = re.compile(r"(?<=\D)\d{3,4}-?[C|P]-?\d+\.(?=\D{3,4})", re.IGNORECASE)
PATTERN_INSTRUMENT_TYPE_OPTION = re.compile(r'\D+(?=\d{3,4}-?[C|P]-?\d+)', re.IGNORECASE)
