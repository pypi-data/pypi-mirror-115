...
class SymbolsInfo(Model):
    ...
    @staticmethod
    def get_or_create_curr_symbols(symbols: typing.Union[str, object, int], create_if_not_exist=False):
        """将合约转化为当期主力合约"""
...
class StrategyBacktestStats(Model):
    ...
    def get_balance_df(self, naming_column_by_key=True) -> typing.Optional[pd.DataFrame]:
        """获取回测的利润曲线"""
        ...
        # if naming_column_by_key:
        #     key = self.get_stats_key()
        #     profit_df.rename(columns={
        #         'profit': f'{key.stg_info_id}_{key.symbols_info_id}_{key.id_name}_{key.cross_limit_method}'
        #     }, inplace=True)
        return balance_df
