...
class DashProxy(dash.Dash):
    ...
    def __init__(self, *args, transforms=None, **kwargs):
        ...
        # TODO: Move again.
        self.transforms.append(ReactiveTransform())
        self.reactive_vars = []
    ...
    def output(self, outputs, f, **kwargs):
        # Create output objects.
        outputs = _as_list(outputs)
        outputs = [Output(*[t[0], t[1]] if type(t) is tuple else [t, 'children']) for t in outputs]
        # Derive inputs objects. TODO: Add state?
        ...
        for key in params:
            if type(params[key]) is tuple:
                inputs.append(Input(params[key][0], params[key][1]))
            else:
                inputs.append(Input(params[key], 'data' if params[key][0] in self.reactive_vars else 'value'))
        # Assemble callback objects.
        callback = self._collect_callback(outputs, inputs, **kwargs)
        callback["f"] = lambda *args: f(*args)
        self.callbacks.append(callback)

    def reactive(self, id, f):
        # TODO: Inject the Store.
        self.reactive_vars.append(id)
        self.output((id, 'data'), f, reactive=True)
...
class StateTransform(DashTransform):
    ...
    def layout(self, layout, layout_is_function):
        if layout_is_function or not self.initialized:
            children = _as_list(layout.children) + self.stores
            layout.children = children
            self.initialized = True
        return layout


class NoOutputTransform(StateTransform):

    def _apply(self, callbacks):
        for callback in callbacks:
            if len(callback[dd.Output]) == 0:
                output_id = _get_output_id(callback)
                store = dcc.Store(id=output_id)
                callback[dd.Output] = [dd.Output(output_id, "data")]
...
# region Shiny transform

class ReactiveTransform(StateTransform):

    def _apply(self, callbacks):
        for callback in callbacks:
            reactive = callback['kwargs'].pop('reactive', False)
            if reactive:
                store = dcc.Store(id=callback[Output][0].component_id)
                self.stores.append(store)
        return callbacks
