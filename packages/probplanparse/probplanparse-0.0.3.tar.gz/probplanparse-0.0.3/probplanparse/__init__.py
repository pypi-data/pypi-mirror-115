...
def get_report():
    ...
    if path_to_result.endswith(".tar.bz2"):
        # finding and opening the folder
        os.system('mkdir result-temp')
        os.system(f'tar -xvf {path_to_result} -C ./result-temp > /dev/null')
    ...
    elif path_to_result.endswith("report.txt"):
...
def main():
    path_to_report = get_report()
    parse_report(path_to_report)
