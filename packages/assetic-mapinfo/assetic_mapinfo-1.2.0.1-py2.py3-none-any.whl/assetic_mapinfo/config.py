...
from .settings.assetic_mapinfo_config import LayerConfig
...
class Config(object):
    ...
    def __init__(self):
        ...
        #self.logger = logging.getLogger("assetic_mapinfo")
...
class Initialise(object):
    def __init__(self, configfile=None, inifile=None, logfile=None
        ...
        msg = "Initiated Assetic_MapInfo. Version {0}".format(__version__)
        ...
        assetic_mapinfo_logger = logging.getLogger(__name__).parent
        for handle in assetic_mapinfo_logger.handlers:
            if isinstance(handle, logging.FileHandler):
                assetic_mapinfo_logger.removeHandler(handle)
                # now attach the handler defined in the xml config file
                assetic_mapinfo_logger.addHandler(assetic_sdk_handle)
                assetic_mapinfo_logger.setLevel(config.asseticsdk.logger.level)
                break
