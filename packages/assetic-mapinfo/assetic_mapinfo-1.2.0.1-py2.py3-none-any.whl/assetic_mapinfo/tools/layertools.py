...
class LayerTools(object):
    ...
    def __init__(self, layerconfig=None):
        ...
        if layerconfig is None:
            if not self.config.layerconfig:
                ...
                self.logger.error(
                    "Assetic MapInfo package missing layer configuration")
                return
        ...
        # Instantiate mapinfo desktop tools
        self._mi_common_util = CommonUtil
    ...
    def get_layer_config(self, table_fields, layer_name, purpose):
        """
        For the given layer get the config settings. Depending on purpose not
        all config is required, so only get relevant config
        :param table_row: is the layer to process (not layer name but
        ArcMap layer)
        :param purpose: one of 'create','update','delete','display'
        """
        lyr_config_list = [
            j for j in self._assetconfig if j["layer"] == layer_name]
        if len(lyr_config_list) == 0:
            if purpose not in ["delete"]:
                msg = "No configuration for layer {0}".format(layer_name)
                self.commontools.new_message(msg)
            return None, None, None
            ...
            if fields is not None:
                ...
                if len(missing_fields) > 0:
                    msg = "Fields [{0}] is defined in configuration but is " \
                          ...
                          "".format(', '.join(missing_fields), layer_name)
                    self.commontools.new_message(msg)
                    self.logger.warning(msg)
                    msg = "Fields in layer {0} are: {1}".format(
                        layer_name, actuallayerflds)
                    self.logger.warning(msg)
                    return None, None, None
        ...
        idfield = None
        if purpose in ["delete", "display"]:
            # get the Assetic unique ID column
            assetid = None
    ...
    def __fetch_records_from_table__(self, tab_path):
        """
        Get dataframe from tab file
        :param tabPath: the file and path of tab fille
        ...
        field_names = []
        table_lr = None
        self.commontools.new_message(f"Tab file to fetch {tab_path}")
        try:
            if tab_path and os.path.isfile(tab_path):
                ...
                if table_ds:
                    ...
                    self.commontools.new_message("Tab file not opened")
            else:
                self.logger.error(f"Failed to find table: {tab_path}")
        except Exception as e:
            self.logger.error("Failed to read table: {}".format(e))

        return rows, field_names, table_alias

    # def _ind_update_assets(self, table_row, layer_name, dialog):
    #     """
    #     Invidividually updates each asset using the passed in row and layer
    #     information.
    #
    ...
    #     else:
    #         # get number of recor
    #         # ds to process for use with timing dialog
    #         if sel_set:
    ...
    def create_asset(self, tab_file, layer_name=None, query=None):
        """
        For the given layer create new assets for the selected features only if
        features have no assetic guid.

        :param tab_file: is the layer to process as a tab file, usually a
        tmp copy of a selection set
        :param layer_name: optionally set layer name, else get table alias
        as name
        ...
        if not self._is_valid_config:
            ...
            return list()
        self.logger.debug("create_asset. file={0}".format(tab_file))

        # get table file as a rows matching field list
        rows, field_names, alias = self.__fetch_records_from_table__(
            tab_file)
        if not rows or len(rows) == 0:
            self.logger.info(f"No data found to process for {tab_file}")
            return list()
        else:
            num_rows = len(rows)
        ...
        for row in rows:
            ...
            result_code, id_dict = self._new_asset(row, lyr_config, fields)
            ...
            if result_code in \
                ...
                self.logger.debug(
                    "Update of asset ID {0}, updating MapInfo row."
                    "".format(suc))
                processed_ids.append(id_dict)
        ...
        msg = ("Finished {0} Asset Creation: {1} Assets created ({2} partially"
               ...
               .format(layer_name, str(prog_cnter['pass_cnt'])
        ...
        self.commontools.new_message(msg)
        self._mi_common_util.do('Note \"{0}\"'.format(msg))
    ...
    def individually_update_rows(self, rows, lyr_config, fields, lyrname):
        ...
        self.commontools.new_message(message, "Assetic Integration")
        self._mi_common_util.do('Note \"{0}\"'.format(message))

    def update_assets(self,  tab_file, layer_name=None, query=None):
        """
        ...
        # get configuration for layer
        lyr_config, fields, idfield = self.get_layer_config(
            field_names, layer_name, "update")

        if lyr_config is None:
            return

        if len(rows) > self._bulk_threshold:
            chk, valid_rows = self.bulk_update_rows(rows, layer_name
                                                    , lyr_config)
        else:
            self.individually_update_rows(rows, lyr_config, fields
                                          , lyrname=layer_name)

    def display_in_assetic(self, tab_file, layer_name=None):
        """
    ...
    def display_asset(self, tab_file, layer_name, lyr_config=None,
                      idfield=None):
        """
        Open assetic and display the first selected feature in layer
        :param tab_file: source mapinfo file
        ...
        # get configuration for layer
        lyr_config, fields, idfield = self.get_layer_config(
            field_names, layer_name, "display")
        self.logger.debug("Got layer config from xml")
        if lyr_config is None:
            self.logger.error("Returning early as layerfile has been "
                              "misconfigured. See log output.")
    ...
    def display_fl(self, lyr, lyr_config=None, idfield=None):
        ...
        # Now launch Assetic
        self.apihelper.launch_assetic_functional_location(flid)
    ...
    def _new_asset(self, row, lyr_config, fields):
        ...
        id_to_update = dict()
        id_to_update["rowid"] = row["rowid_tmp"]
        id_to_update["assetic_ids"] = dict()
        ...
        if "id" in corefields and corefields["id"] in fields:
            ...
            if "asset_id" in corefields and corefields["asset_id"] in fields:
                ...
                return self.results["error"], id_to_update
        ...
        if not newasset:
            ...
            return self.results["skip"], id_to_update
        ...
        if addr or geojson:
            ...
            if chk > 0:
                ...
                return self.results['partial'], id_to_update

        if asset_repr.error_code in [2, 4, 16]:
            # component (2), or dimension (4) or Fl (16) error
            return self.results['partial'], id_to_update

        return self.results['success'], id_to_update
    ...
    def _update_asset(self, row, lyr_config, fields):
        ...
        errcode = self._attach_functionallocation(
        ...
        if lyr_config["upload_feature"]:
            chk = self.upload_features(
                row, complete_asset_obj, lyr_config, fields)
