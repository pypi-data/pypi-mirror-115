...
class BertModel(PreTrainModel):
    """Bert models"""

    def __init__(self, config, with_pooler=True, seq_length=4, name='bert', **kwargs):
        """
        ...
        self._embeddings = self.create_embedding_layer(self._config)
        self._encoder = self.create_encoder_layer(self._config)
        if with_pooler:
    ...
    def mapping(self, pretrained_model_path: str):
        ...
        if pretrained_model_path.endswith('.ckpt'):
            ...
            self.logger.warning('your file does not have file extension, it will default as checkpoint file.')
            init_variables = tf.train.list_variables(pretrained_model_path)
            weight_value_tuples = self.ckpt_mapping(name_to_variable, init_variables,
                                                    pretrained_model_path)
...
class BertMRC(BertModel):
    """Bert for machine reading comprehension, it bases on version which was provided by Google"""

    def __init__(self, config, custom_loss_fn=None, label_smoothing=None, **kwargs):
        """
        ...
        """
        super().__init__(config, with_pooler=False, **kwargs)
...
class BertPreTrainingModel(BertModel):
