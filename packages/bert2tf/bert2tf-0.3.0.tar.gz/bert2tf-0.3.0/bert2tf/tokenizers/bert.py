...
class WordpieceTokenizer(BaseTokenizer):
    ...
    def __init__(self, vocab_file_path, unk_token='[UNK]', max_input_chars_per_word=200):
...
class BertTokenizer(BaseTokenizer):
    ...
    def __init__(self, vocab_file_path, do_lower_case=True, cls_token='[CLS]', sep_token='[SEP]', pad_token='[PAD]'):
        super().__init__(vocab_file_path)
        self.basic_tokenizer = BasicTokenizer(do_lower_case=do_lower_case)
        self.wordpiece_tokenizer = WordpieceTokenizer(vocab_file_path=vocab_file_path)
    ...
    def encode(self, text1, text2=None, maxlen=None, *args, **kwargs):
        if maxlen:
            if not text2 and maxlen < 3:
                raise ValueError('if you want to set max length for encoding one sentence, '
                                 'you must set max length greater than 2')
            if not text2 and maxlen < 5:
                raise ValueError('if you want to set max length for encoding two sentences, '
                                 'you must set max length greater than 4')

        if text2:
            input_tokens, input_masks, segment_ids = self._encode_double(text1, text2, maxlen=maxlen)
        else:
            input_tokens, input_masks, segment_ids = self._encode_single(text1, maxlen=maxlen)

        while maxlen and len(input_tokens) < maxlen:
    ...
    def _encode_single(self, text, maxlen=None):
        tokens = self.tokenize(text)

        if maxlen is not None and len(tokens) > maxlen - 2:
    ...
    def _encode_double(self, text1, text2, maxlen):
        while maxlen is not None and len(text1) + len(text2) > maxlen - 3:
            text1 = text1[:len(text1) - 1]
            if len(text1) + len(text2) > maxlen - 3:
                text2 = text1[:len(text2) - 1]

        tokens1 = self.tokenize(text1)

        input_tokens = [self.cls_token] + tokens1 + [self.sep_token]
        segment_ids = [0] * (len(input_tokens))

        tokens2 = self.tokenize(text2)
        input_tokens = input_tokens + tokens2 + [self.sep_token]
        segment_ids = segment_ids + [1] * (len(input_tokens) - len(segment_ids))

        input_masks = [1] * len(input_tokens)
        return input_tokens, input_masks, segment_ids
