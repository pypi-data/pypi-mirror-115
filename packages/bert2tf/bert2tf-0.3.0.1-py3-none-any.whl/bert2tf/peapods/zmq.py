...
from ..helper import colored, use_uvloop
from ..logging import default_logger
from ..logging.base import get_logger
from ..proto import bert2tf_pb2, add_envelope
...
class Zmqlet:
    ...
    def send_idle(self, msg: Optional['bert2tf_pb2.Message'] = None):
        """Use `in_sock` to send idle signal to head pea, used for Router-Dealer"""
        if msg:
            msg.request.control.command = bert2tf_pb2.Request.ControlRequest.IDLE
        else:
        ...
        send_message(self.in_sock, msg, **self.send_recv_kwargs)
    ...
    def send_message(self, msg: bert2tf_pb2.Message):
        ...
        send_message(self.out_sock, msg, **self.send_recv_kwargs)
        self.msg_sent += 1
        if self.in_sock_type == zmq.DEALER:
            self.send_idle()
    ...
    def init_socket(self) -> Tuple[zmq.Context, zmq.Socket, zmq.Socket, zmq.Socket]:
        ...
        ctrl_sock, ctrl_addr = _init_socket(ctx, SocketType.PAIR_CONNECT, None, port=self.args.port_ctrl)
...
class ZmqStreamlet(Zmqlet):
    ...
    def register_pollin(self):
        ...
        try:
            ...
            self.logger.error('it requires "tornado" as a base dependency, '
                              'we use its I/O event loop for non-blocking sockets. try pip install tornado')
            raise
    ...
    def start(self, callback):
        ...
        self._in_sock_callback = lambda x: _callback(x)
        self.in_sock.on_recv(self._in_sock_callback)
        self.ctrl_sock.on_recv(lambda x: _callback(x))
        if self.out_sock_type == zmq.ROUTER:
            self.out_sock.on_recv(lambda x: _callback(x))
...
class AsyncZmqlet(Zmqlet):
    ...
    async def recv_message(self, callback: Callable = None) -> Union[None, Dict]:
...
def send_message(sock: zmq.Socket, msg: bert2tf_pb2.Message, timeout: int = -1, **kwargs):
    try:
        if timeout > 0:
...
def recv_message(sock: zmq.Socket, timeout: int = -1, **kwargs) -> bert2tf_pb2.Message:
    try:
        if timeout > 0:
...
def send_ctrl_message(address: str, cmd: 'bert2tf_pb2.Request.ControlRequest', timeout: int):
    ...
    with zmq.Context() as ctx:
        ctx.setsockopt(zmq.LINGER, 0)
        sock, _ = _init_socket(ctx, SocketType.PAIR_BIND, None, host=address)
        req = bert2tf_pb2.Request()
