...
from .. import __stop_msg__, __default_host__
from ..client.python import request
from ..helper import routes2str, get_random_identity, pb2array, use_uvloop
from ..logging.base import get_logger
from ..proto import bert2tf_pb2, bert2tf_pb2_grpc, add_route, add_envelope
...
class GatewayPea:
    ...
    class _Pea(bert2tf_pb2_grpc.Bert2tfRPCServicer):
        ...
        #     import asyncio
        #     onrecv_task = []
        #     self.args.identity = get_random_identity()
        #     with AsyncZmqlet(self.args) as zmqlet:
        #         try:
        #             for request in request_iterator:
        #                 asyncio.create_task(
        #                     zmqlet.send_message(
        #                         add_envelope(request, 'gateway', zmqlet.identity)))
        #                 onrecv_task.append(asyncio.create_task(zmqlet.recv_message(callback=self.handle)))
        #         except StopIteration:
        #             pass
        #
        #         while not zmqlet.msg_sent == zmqlet.msg_recv != 0:
        #             self.logger.info(f'send: {zmqlet.msg_sent} '
        #                              f'recv: {zmqlet.msg_recv} '
        #                              f'pending: {zmqlet.msg_sent - zmqlet.msg_recv}')
        #             for r in asyncio.as_completed(onrecv_task):
        #                 yield await r

        async def CallUnary(self, request, context):
            with AsyncZmqlet(self.args) as zmqlet:
                await zmqlet.send_message(add_envelope(request, 'gateway', zmqlet.identity))
...
class RESTGatewayPea(BasePea):
    ...
    def _create_http_server(self):
        ...
        app = Application([(f'/{self.args.url}', PredictHandler, dict(args=self.args))])
    ...
    def loop_body(self):
        ...
        self.server.bind(self.args.http_port)
    ...
    def set_ready(self, *args, **kwargs):
        self.logger.success(f'REST gateway is listening at {__default_host__}:{self.args.http_port}/{self.args.url}')
        super().set_ready(*args, **kwargs)
...
class PredictHandler(RequestHandler):
    ...
    async def prepare(self):
        try:
            self._inputs = json.loads(self.request.body.decode('utf-8'))['inputs']
            self.set_status(200)
            ...
            self.logger.debug(
                'remote addr: %s\trequest body: %s is a bad json' % (
                    self.request.remote_ip, self.request.body.decode("utf-8")))

    async def post(self):
        if self.get_status() == 200:
            req = getattr(request, 'predict')(self.inputs)
            self._req = await self._p_servicer.CallUnary(req, None)

            if self.req.status.code == bert2tf_pb2.Status.SUCCESS:
                self.write({'outputs': [pb2array(blob).tolist() for blob in self.req.predict.data.blob]})
            else:
                self.set_status(500, reason=f'internal server error: {self.req.status.description}')
