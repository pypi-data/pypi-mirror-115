...
class IniTestFile(pytest.File):
    ...
    def collect(self):
        ...
        for name in config.sections():
            ...
            spec["workingdir"] = self.rootdir / (".out_" + self.rootname)
            spec["bulk"] = bulkfile
            ref = spec["reference"]
            # -----------------------------------------------------------------
...
class IniItem(pytest.Item):
    def __init__(self, name, parent, spec):
        ...
        if not self.parent.mystran_run_status:
            # first test for this class, a few things to set up
            self.parent.mystran_run_status = self.run_mystran()
        # ---------------------------------------------------------------------
        # process OP2
    ...
    def run_mystran(self):
        ...
        # =====================================================================
        try:
            self.spec["workingdir"].mkdir()
        except FileExistsError:
            shutil.rmtree(self.spec["workingdir"])
            self.spec["workingdir"].mkdir()
        target = shutil.copy(self.spec["bulk"], self.spec["workingdir"])
        cmd = f"{MYSTRAN_BIN} {target}"
