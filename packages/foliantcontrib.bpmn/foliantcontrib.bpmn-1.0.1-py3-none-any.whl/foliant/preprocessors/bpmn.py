'''
BPMN diagrams preprocessor for Foliant documenation authoring tool.
'''
...
class Preprocessor(BasePreprocessorExt):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger = self.logger.getChild('bpmn')
    ...
    def _get_command(self,
        ...
        components = [options['converter_path']]
        ...
        components.append(f'{diagram_src_path}:{diagram_path}')
    ...
    @allow_fail('Error while processing bpmn tag.')
    def _process_diagrams(self, block) -> str:
        '''
        Process bpmn tag.
        Save BPMN diagram body to .bpmn file, generate an image from it,
        ...
        self.logger.debug(f'Processing BPMN diagram, options: {options}, body: {body}')

        body_hash = md5(f'{body}'.encode())
        body_hash.update(str(options.options).encode())

        diagram_src_path = self._cache_path / f'{body_hash.hexdigest()}.bpmn'
        ...
        if result.returncode == 0:
            ...
            self._warning(f'Processing of BPMN diagram failed.',
