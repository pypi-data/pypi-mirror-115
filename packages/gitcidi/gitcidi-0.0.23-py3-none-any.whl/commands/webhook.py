...
@click.option('--serverurl', '-e', default="host file ", prompt='Enter the host  ', help='enter the host ')
def create_webhook(path,serverurl):
    ...
    username=parsed_yaml_file["user_name"]
    token=parsed_yaml_file["access_token"]
    repo=parsed_yaml_file["repository"]
    access_token = token# "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"
    OWNER = username#"AkashAi7" # github account name
    REPO_NAME = repo#"demosession1"# github repository name
    EVENTS = ["*"]      # Events on github
    HOST = serverurl#"343328894880.ngrok.io"  # ngrok tunnel
    config = {
