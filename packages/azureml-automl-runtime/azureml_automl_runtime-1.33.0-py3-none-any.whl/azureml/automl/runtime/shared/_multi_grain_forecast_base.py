...
class _MultiGrainForecastBase:
    ...
    def get_params(self, deep: bool = True) -> Dict[str, Any]:
        param_dict = self.timeseries_param_dict.copy()
        for key in param_dict:
            if isinstance(param_dict[key], pd.DateOffset):
                param_dict[key] = param_dict[key].freqstr
        return {
            "timeseries_param_dict": param_dict
        }
