...
def read_ref_workbook(workbook_fpath):
    ...
    for sheet_name in workbook.sheet_names:
        ...
        df = df.replace('nan', '')
        df['Authors'] = auto_correct_author_string(df['Authors'])
...
def check_ref_workbook(workbook_dict):
    ...
    for sheet_name, df_ref in workbook_dict.items():
        ...
        if sel_ref_1:
            print('Incorrect Format of Authors in {}: '.format(sheet_name))
            print(tabulate(df_ref.loc[sel_ref_1, ['Authors']], tablefmt='psql'))
        if sel_ref_2:
            print('Incorrect Format of Year in {}: '.format(sheet_name))
