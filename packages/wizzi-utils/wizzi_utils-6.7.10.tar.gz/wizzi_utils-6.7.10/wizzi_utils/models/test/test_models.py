...
def tr_run(
    ...
    fps.start()
    detections = model.track_frame(cv_img, ack=DEBUG_DETECTIONS, tabs=1, frame_idx=frame_idx)
    fps.update(ack_progress=True, tabs=1, with_title=True)
...
def tr_Model_video_test(
    ...
    if cam is not None:
        ...
        model = tr.Cv2Tracker(model_obj=tr.Trackes[model_name])
...
def test_all():
    ...
    if TR_VID_TEST:
        vid_path = cvtt.get_vid_from_web(name=mtt.WOMAN_YOGA)
        tr_Model_video_test(
            model_name='CSRT',  # slower but better
            vid_path=vid_path,
            work_every_x_frames=1,
