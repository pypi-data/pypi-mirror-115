...
def add_fps_and_name(cv_img, fps, name):
    cvt.add_header(cv_img, header=name, loc=pyplt.Location.BOTTOM_LEFT.value, bg_font_scale=1)
...
def od_run(
    ...
    add_fps_and_name(cv_img, fps, name=model.model_name)
    return
