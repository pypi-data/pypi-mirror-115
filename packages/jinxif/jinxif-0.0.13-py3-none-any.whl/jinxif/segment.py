...
def _relabel_numba(ai_cell_labels, ai_cellid_relabel):
    '''
    version: 2021-08-03
    ...
    if (ai_cellid_relabel.max() < 2**8):
        ai_cell_labels = ai_cell_labels.astype(np.unit8)
        ai_cellid_relabel = ai_cellid_relabel.astype(np.unit8)
    elif (ai_cellid_relabel.max() < 2**16):
        ai_cell_labels = ai_cell_labels.astype(np.unit16)
        ai_cellid_relabel = ai_cellid_relabel.astype(np.unit16)
    elif (ai_cellid_relabel.max() < 2**32):
        ai_cell_labels = ai_cell_labels.astype(np.unit32)
        ai_cellid_relabel = ai_cellid_relabel.astype(np.unit32)
    else:
        ai_cell_labels = ai_cell_labels.astype(np.unit64)
        ai_cellid_relabel = ai_cellid_relabel.astype(np.unit64)
...
def match_nuccell_labels(
    ...
    ai_cell_relabeled = _relabel_numba(ai_cell_labels=ai_cell_labels, ai_cellid_relabel=ai_cellid_relabel)
...
def segment_nuc_dapi(
    ...
    if (a_mask.max() < 2**8):
        a_mask = a_mask.astype(np.unit8)
    elif (a_mask.max() < 2**16):
        a_mask = a_mask.astype(np.unit16)
    elif (a_mask.max() < 2**32):
        a_mask = a_mask.astype(np.unit32)
    else:
        a_mask = a_mask.astype(np.unit64)
...
def segment_spawn(
    ...
    for s_folder in sorted(os.listdir(s_regdir)):
        ...
        if b_found:
            print(f'Processing {s_task} {s_folder} ...')
            ...
            for s_slide_pxscene in sorted(set(df_img.slide_scene)):
                ...
                # nuc, cell, nuccell segmentation
                    ...
                    # BUE 20210624: should provide proper regex fot dapi
                    s_tiff_dapi = df_img.loc[(df_img.loc[:,'round'] == 'R1') & (df_img.color == 'c1') & (df_img.slide_scene == s_slide_pxscene),:].index[0]
...
def nuccell_zprojlabel_imgs(
    ...
    for s_file in sorted(os.listdir(s_ipath)):
        ...
        if not (o_match is None):
            ...
            ax[0,0].set_title('cell z projection')
            ax[0,0].imshow(io.imread(s_ipath + config.d_nconv['s_format_png_cellprojection'].format(s_slide_pxscene, s_seg_markers, i_cell_diam)))  # bue: s_seg_markers == s_z_label
            ax[0,1].set_title('nuc z projection')
            ax[0,1].imshow(io.imread(s_ipath + config.d_nconv['s_format_png_nucprojection'].format(s_slide_pxscene, i_nuc_diam)))
            ax[0,2].set_title('nuc label')
            ax[0,2].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuc'].format(s_slide_pxscene, i_nuc_diam)))
            ax[1,0].set_title('cell label - original')
            ax[1,0].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_cell'].format(s_slide_pxscene, s_seg_markers, i_cell_diam)))
            ax[1,1].set_title('cell label - nuc ok')
            ax[1,1].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuccellmatched'].format(s_slide_pxscene, s_seg_markers, i_nuc_diam, i_cell_diam)))
            ax[1,2].set_title('cell label - nuc feat ok')
            try:
                ax[1,2].imshow(io.imread(s_ipath + config.d_nconv['s_format_tiff_celllabel_nuccellmatchedfeat'].format(s_slide_pxscene, s_seg_markers, i_nuc_diam, i_cell_diam)))
...
def nuccell_zprojlabel_imgs_spawn(
    ...
    for s_slide in sorted(es_slide):
        ...
        print(f'nuccell_label_imgs_spawn: {s_slide}')
