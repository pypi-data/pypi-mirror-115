...
def auto_thresh(
    ...
    for s_folder in sorted(os.listdir(s_afsubdir)):
        ...
        if s_folder.startswith(s_slide):
            print(f'process {s_folder} ...')
            b_found = True
            s_imgdir = s_format_afsubdir.format(s_afsubdir, s_folder)
            df_img = basic.parse_tiff_reg(s_wd=f'{s_afsubdir}{s_folder}/')
    ...
    if b_found:
        ...
        df_thresh.to_csv(s_afsubdir + config.d_nconv['s_format_csv_threshold'].format(s_slide))
        print(f'write file: {s_afsubdir}{config.d_nconv["s_format_csv_threshold"].format(s_slide)}')
...
def auto_thresh_spawn(
    ...
    for s_slide in sorted(es_slide):
        ...
        print(f'auto_thresh_spawn: {s_slide}')

        # set run commands
        s_pathfile_template = 'template_autothresh_slide.py'
        s_pathfile = f'autothresh_slide_{s_slide}.py'
        ...
        if b_parallel and (s_type_processing == 'slurm'):
            ...
            s_pathfile_sbatch = f'autothresh_slide_{s_slide}.sbatch'
            config.slurmbatch(
            ...
            # Jenny this is cool! Popen rocks.
            s_file_stdouterr = 'slurp-autothresh_slide_{s_slide}.out'.replace('-','')
            time.sleep(4)
...
def load_thresh_df(
    ...
    for s_slide in sorted(es_slide):
        print(f'Loading: {config.d_nconv["s_format_csv_threshold"].format(s_slide)} ...')
        df_img = pd.read_csv(s_afsubdir + config.d_nconv['s_format_csv_threshold'].format(s_slide), index_col=0)
...
def apply_thresh(
    ...
    input:
        ...
        df_img_thersh:  dataframe with threshold values.
...
def markerpositive_scatterplots(
    ...
    for s_slidepxscene in sorted(set(dfb_thresh.slide_scene)):
        ...
        if b_found:
            ...
            df_xy = pd.read_csv(s_segdir + config.d_nconv['s_format_csv_controidxy'].format(s_slide), index_col=0)
            ...
            fig.savefig(s_opath+ f'{s_slidepxscene}_{".".join(ls_save)}_thresh_scatter.png', facecolor='white')
            plt.close()
