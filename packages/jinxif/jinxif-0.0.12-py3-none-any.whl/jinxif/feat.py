...
def extract_features(
    ...
    df_feat.to_csv(s_path_seg + config.d_nconv['s_format_csv_raw_centroid_shape_meanintenisty'].format(s_slide))
    #break
...
def filter_features(
    ...
    for s_slide in sorted(df_mi.slide.unique()):
        ...
        s_ofile = config.d_nconv['s_format_csv_patched_shape_meanintenisty'].format(s_slide, s_filter_dapi)
        print(f'write file: {s_ofile}')
        df_mi.loc[df_mi.slide == s_slide].to_csv(s_segdir + s_ofile)
...
def load_cellpose_features_df(
    ...
    df_mi = pd.DataFrame()
    for s_slide in sorted(es_slide):
        print('Loading:', config.d_nconv['s_format_csv_raw_centroid_shape_meanintenisty'].format(s_slide))
        df_mi_slide = pd.read_csv(s_format_segdir_cellpose.format(s_segdir, s_slide) + config.d_nconv['s_format_csv_raw_centroid_shape_meanintenisty'].format(s_slide), index_col=0)
