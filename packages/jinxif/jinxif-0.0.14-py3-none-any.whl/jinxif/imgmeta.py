    ...
    for s_slide in sorted(set(df_img.slide)):
        ...
        for s_slidemscene in  sorted(set(df_img_slide.slide_mscene)):
            ...
            for s_image in df_img_mscene.index:
                ...
                se_meta = pd.Series(l_meta, name=s_image, index=config.d_nconv['ls_color_order'][0:len(l_meta)])
    ...
    for s_slide  in sorted(es_slide):
        ...
        if b_sceneposition:
            ...
            fetch_meta_slide_sceneposition(
                s_slide = s_slide,
                s_czidir_original = s_format_czidir_original.format(s_czidir, s_slide),
                s_metadir = s_metadir,
            )
...
def load_exposure_df(
    ...
    df_exposure = df_load.loc[:, df_load.columns.isin(config.d_nconv['ls_color_order'])]
    ...
    df_img = df_load.loc[:, ~df_load.columns.isin(config.d_nconv['ls_color_order'])]
...
def exposure_matrix(
    ...
    for s_slide in sorted(es_slide):
        ...
        df_all = df_all.append(df_load.loc[:,['slide_mscene','exposure_time_ms']])
    df_all = df_all.pivot(columns='slide_mscene')
