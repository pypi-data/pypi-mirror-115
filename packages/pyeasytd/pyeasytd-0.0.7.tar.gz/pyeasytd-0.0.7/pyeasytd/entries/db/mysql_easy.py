...
class MysqlEasyConnection:
    ...
    def insert_dict_auto_add_table_and_column(self, table, data, mode='into',
        ...
        if not self.exists_table(table):
            # 通过数据生成表
            self.__data_generate_table(table, data.copy(), value_list_split, primary_name, engine=engine, charset=charset)
    ...
    def insert_dict(self, table, data, mode='into', value_list_split=','):
        ...
        sql = f'{pre_sql} `{table}` ({keys}) values({",".join(["%s" for i in range(len(data))])})'
        cursor = conn.cursor()
        try:
    ...
    def insert_dicts(self, table, data, once_length=200, mode='into', values_list_split=','):
        ...
        for i in range(int((len(data) - 1) / once_length) + 1):
            ...
            sql = f'{pre_sql} {table} ({keys}) values '
            values = []
    ...
    def __data_generate_table(self, table, data, split, primary, engine, charset):
        '''
        dict数据生成mysql表
        :param table: 表名
        :param data: dict数据
        ...
        if BasicCheckUtil.non_none(data.get(primary)):
            ...
            elif type(data.get(primary_type)) != int:
