...
def setDryRun(enabled):
    global _DRY_RUN
    _DRY_RUN = bool(enabled)


def enableSafeZone(folder=None, includeSubfolders=True):
    """
...
def strict():
    """
    Can only be enabled. Effectively does a enableSafeZone('*') and enableConfirm()
    to make everything read-only. Wait, what? So confirmation mode sort of overrides safe zone right?
    Yeah, I guess so. If you just want to confirm everything, leave safe zone disabled but enable confirm.
    Hmmm, put more thought into this.
    To "disable" strict mode, call disableSafeZone() and disableConfirm(). This function exists mostly
    as a shortcut so you can easily just slap import safefs;safefs.strict() at the top of your program to make
