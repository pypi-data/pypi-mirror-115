...
def guess_delimiter(line: str, delimiters: Iterable[str] = ("\t", "\0", "|", ",")) -> str:
    ...
    Parameters
    ----------
    line : line of delimited text.
    delimiters : delimiters to check.
...
def dict_to_yaml(content: dict) -> str:
    ...
    Parameters
    ----------
    content: the dictionary content to convert to the yaml format.
...
def standardize_string(value: str) -> str:
    ...
    Parameters
    ----------
    value: the string to be 'standardized'.
...
def check_if_dependency_is_installed(dependency: str) -> bool:
    ...
    Parameters
    ----------
    dependency: the dependency to be verified.
...
def check_if_image_is_allowed_on_platform(registry_url: Optional[str], docker_image_name: Optional[str]) -> None:
    ...
    registry_url: The registry the provided image should belong to.
    docker_image_name: The docker image to validate against the url.
...
def get_system_requirements_from_file(file_path: Optional[Path]) -> List[str]:
    ...
    file_path: the path to the system's requirements file.
...
def check_if_requirement_is_present(file_path: Path, requirement: str) -> bool:
    ...
    requirement: the requirement to validate against the provided requirements file
    ...
    if file_path and file_path.exists():
        ...
        for entry in split_lines:
            if not entry.startswith("#") and requirement in entry:
                return True
    return False
...
def merge(x: T, *args: Mapping[str, Any], **kwargs: Any) -> T:
    ...
    x: the initial, mutable dictionary.
    args: the arguments to merge into the 'x' dictionary.
    kwargs: the keyword arguments to merge into the 'x' dictionary.
...
def get_bytes_as_human_readable(input_bytes_data: Union[Optional[int], Optional[float]]) -> str:
    ...
    input_bytes_data: the input int that corresponds to the bytes value.
...
def get_iec_to_si_format_as_human_readable(input_data: str) -> str:
    ...
    input_data: the input str in iec format. ex: 256Mi
...
def parse_pydantic_errors(validation_error: ValidationError) -> str:
    ...
    validation_error: the ValidationError to prettify.
...
def get_files_from_dir(file_type: str, input_dir: str) -> List:
    ...
    input_dir: the directory to read the files from.
...
def get_system_information(pretty_keys: bool = False) -> dict:
    ...
    Parameters
    ----------
    pretty_keys: indicates whether it should yield a dictionary with raw data or a 'prettified' one.
...
def ansi_escape_string(value: str) -> str:
    ...
    Parameters
    ----------
    value : the string to ansi escape
