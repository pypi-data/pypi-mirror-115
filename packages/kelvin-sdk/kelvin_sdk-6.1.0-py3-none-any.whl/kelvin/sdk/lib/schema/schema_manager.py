...
def generate_base_schema_template(project_creation_parameters_object: ProjectCreationParametersObject) -> dict:
    ...
    Parameters
    ----------
    project_creation_parameters_object : the app creation parameters object used to generate the default schema.
    ...
    app_name = project_creation_parameters_object.app_name
    ...
    if project_creation_parameters_object.app_type == ProjectType.kelvin:
        ...
        language_block.update(project_creation_parameters_object.kelvin_app_lang.default_template(app_name=app_name))
        # 2 - Create the interface block
        app_type_obj.update(
            {
                "kelvin": {"language": language_block},
            }
        )
    else:
        app_type_obj.update(
...
def validate_app_schema_from_app_config_file(
    ...
    Parameters
    ----------
    app_config :the alternative app configuration to the app_config_file_path.
    app_config_file_path :the path to the app configuration.
...
def validate_schema(
    ...
    schema_file_path: The path to the schema file to validate the file against.
    full_schema_errors: Indicates whether or not it should log the complete stack trace.
...
def _validate_schema(content: dict, schema: Optional[dict] = None, schema_path: Optional[KPath] = None) -> bool:
    ...
    schema : the schema, as a dict, to validate the content against.
    schema_path : the path to the schema to validate the content against.
...
def _get_and_persist_app_schema(schema_url: str, schema_version: str) -> Tuple[dict, KPath]:
    ...
    Parameters
    ----------
    schema_url :  the url to retrieve the schema from.
    schema_version : the latest schema version.
...
def _get_schema_version_file_path(schema_version: str) -> KPath:
    ...
    Parameters
    ----------
    schema_version : The version corresponding to the generated schema file path.
