...
def appregistry_show(app_name: str, should_display: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    app_name: the name with version of the application.
    should_display: specifies whether or not the display should output data.
    ...
    try:
        ...
        complete_app_info = {}
        if app_data_display_object:
            complete_app_info["app"] = app_data_display_object.parsed_data
        if app_version_display:
            complete_app_info["app_versions"] = app_version_display.parsed_data
        if workload_display:
            complete_app_info["app_workloads"] = workload_display.parsed_data
...
def appregistry_delete(app_name_with_version: str, ignore_destructive_warning: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    app_name_with_version: the name with version of the app to be deleted.
...
def appregistry_upload(app_dir_path: str, upload_datatypes: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    app_dir_path: the path to the application's dir.
    upload_datatypes: If specified, will upload locally defined data types.
...
def appregistry_download(app_name_with_version: str, override_local_tag: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    app_name_with_version: the app with version to be downloaded.
    override_local_tag: specifies whether or not it should override the local tag.
    ...
    try:
        ...
        docker_manager.pull_docker_image_from_registry(
...
def retrieve_appregistry_data(query: Optional[str] = None, should_display: bool = True) -> DisplayObject:
    ...
    Parameters
    ----------
    query: the query to search specific applications.
