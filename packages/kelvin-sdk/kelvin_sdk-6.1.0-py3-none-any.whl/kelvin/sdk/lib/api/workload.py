...
from kelvin.sdk.lib.models.types import DelimiterStyle, StatusDataSource, WorkloadFileType
...
def workload_list(
    ...
    node_name : the name of the node to filter the workloads.
    app_name : the name of the app to filter the workloads.
    enabled : indicates whether it should filter workloads by their status.
...
def workload_deploy(workload_deployment_request: WorkloadDeploymentRequest) -> OperationResponse:
    ...
    Parameters
    ----------
    workload_deployment_request: the deployment object that encapsulates all the necessary parameters for deploy.
...
def workload_update(
    ...
    Parameters
    ----------
    workload_name: the name for the workload to update.
    app_version: the the version of the app.
    workload_title: the title for the  workload.
    app_config: the path to the app configuration file.
...
def workload_logs(
    ...
    ----------
    workload_name: the name of the workload.
    tail_lines: the number of lines to retrieve on the logs request.
    output_file: the file to output the logs into.
    follow: a flag that indicates whether it should trail the logs, constantly requesting for more logs.
...
def workload_undeploy(workload_name: str, ignore_destructive_warning: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    workload_name: the name of the workload to be stopped and deleted.
...
def workload_start(workload_name: str) -> OperationResponse:
    ...
    Parameters
    ----------
    workload_name: the workload to start on the platform.
...
def workload_stop(workload_name: str, ignore_destructive_warning: bool = False) -> OperationResponse:
    ...
    Parameters
    ----------
    workload_name: the workload to stop on the platform.
    ignore_destructive_warning: indicates whether it should ignore the destructive warning.
...
def workload_deploy_bulk(
    ...
    Parameters
    ----------
    filename: The filename to load the configurations from.
    file_type: The type of the workload file.
    output_filename: The output file into which the results will be written.
    ignore_failures: Ignore deployment failures and automatically continue.
    skip_successes: Skip previous successes.
    dry_run: Validate inputs only.
...
def retrieve_workload_metrics_data(workload_metrics_data: WorkloadMetrics, title: str = "") -> DisplayObject:
    ...
    Parameters
    ----------
    workload_metrics_data : the WorkloadMetrics object.
    title : the title to associate to the the node metrics detail info.
...
def retrieve_workload_and_workload_status_data(
    ...
    node_name: the name of the node to filter the workloads.
    app_name: the name of the app to filter the workloads.
    app_version: the version of the app to filter the workloads.
    enabled: indicates whether it should filter workloads by their status.
    query: the query to query specific workloads.
    source: the status data source from where to obtain data.
...
def _combine_workload_and_workload_status_data(
    ...
    Parameters
    ----------
    workloads: the list of workloads to combine.
    workloads_status: the list containing the status for each existing workload.
...
def _retrieve_workload_logs(
    ...
    Parameters
    ----------
    client: the Kelvin SDK Client object used to retrieve data.
    workload_name: the name of the workload.
    tail_lines: the number of lines to retrieve on the logs request.
    output_file: the file to output the logs into.
    follow: a flag that indicates whether it should trail the logs, constantly requesting for more logs.
...
def _extract_last_date_from_log_entries(entry: List) -> Optional[str]:
    ...
    Parameters
    ----------
    entry: the log entries to retrieve the data from.
...
def _get_parsed_workload_status(workload_status_item: Optional[WorkloadStatusItem] = None) -> str:
    ...
    Parameters
    ----------
    workload_status_item: the Workload status item containing all necessary information.
...
def _deploy_bulk_workload(
    ...
    Parameters
    ----------
    workload_data: workload deployment information.
    ignore_failures: continue if any deployment fails.
    skip_successes: skip any deployment already marked with success.
    skip_successes: validate inputs only.
