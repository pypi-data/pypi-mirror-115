...
class PrecipitateBase:
    ...
    def _resetArrays(self):
        ...
        self.numberOfNucleations = np.zeros((len(self.phases), self.steps))  #Number of nucleates
    ...
    def _divideTimestep(self, i, dt):
        ...
        self.numberOfNucleations = np.append(self.numberOfNucleations, np.zeros((len(self.phases), 1)), axis=1)
    ...
    def setNucleationDensity(self, grainSize = 100, aspectRatio = 1, dislocationDensity = 5e12, bulkN0 = None):
        ...
        #Temporary - set self.N0 to some value that isn't None so we
        #   know that nucleation density was set by user before simulation
        self.N0 = 0
    ...
    def _getNucleationDensity(self):
        ...
        self.N0 = self.GBareaN0 + self.GBedgeN0 + self.GBcornerN0 + self.dislocationN0 + self.bulkN0
    ...
    def _printOutput(self, i):
        ...
        for p in range(len(self.phases)):
            print('\t{}\t{:.3e}\t{:.4f}\t\t{:.4e}'.format(self.phases[p], self.numberOfNucleations[p,i], 100*self.betaFrac[p,i], self.avgR[p,i]))
        print('')
    ...
    def plot(self, axes, variable, bounds = None, *args, **kwargs):
        ...
        *args, **kwargs - extra arguments for plotting
        ...
        labels = {
            ...
            'Driving Force': 'Driving Force (J/mol)',
        ...
        if variable == 'Temperature':
            ...
            if self.numberOfElements == 1:
                ...
                axes.set_ylabel('Matrix Composition (at.% ' + self.solutes[0] + ')')
            else:
                for i in range(self.numberOfElements):
                    axes.semilogx(self.time, self.xComp[:,i], label=self.solutes[i], *args, **kwargs)
                axes.legend(self.solutes)
                axes.set_ylabel(labels[variable])
            ...
            if variable == 'Volume Fraction':
                ...
                plotVariable = np.cbrt(self.betaFrac / self.numberOfNucleations)
                ...
                plotVariable = self.numberOfNucleations
            ...
            if variable == 'Total Volume Fraction':
                ...
                totalN = np.sum(self.numberOfNucleations, axis=0)
                totalN[totalN == 0] = 1
                totalR = np.sum(self.avgR * self.numberOfNucleations, axis=0)
                ...
                totalN = np.sum(self.numberOfNucleations, axis=0)
                totalN[totalN == 0] = 1
                totalAR = np.sum(self.avgAR * self.numberOfNucleations, axis=0)
                ...
                plotVariable = np.sum(self.numberOfNucleations, axis=0)
