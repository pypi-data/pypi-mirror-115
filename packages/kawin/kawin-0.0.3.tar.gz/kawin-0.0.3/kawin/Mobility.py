...
class MobilityModel(Model):
    ...
    def mobility_callables(self, additional_statevars = None):
        '''
        Create callable functions for mobility

        Parameters
        ----------
        additional_statevars : list
            List of additional variables to include in callables

        Returns
        -------
        Dictionary of mobility callables for each element
        '''
        additional_statevars = set(additional_statevars) if additional_statevars is not None else set()
        sv = get_state_variables({self.phase_name: self})
        sv |= additional_statevars
        sv = sorted(sv, key=str)
        callables = {}
        for c in self.mobility:
            callables[c] = make_callable(self.mobility[c], sv + self.site_fractions)
        return callables

    def diffusivity_callables(self, simple = False, additional_statevars = None):
        '''
        Create callable functions for diffusivity
        ...
        Returns
        -------
        Dictionary of diffusivity callables for each element
        '''
        additional_statevars = set(additional_statevars) if additional_statevars is not None else set()
        sv = get_state_variables({self.phase_name: self})
        sv |= additional_statevars
        sv = sorted(sv, key=str)
        callables = {}
        for c in self.diffusivity:
            if simple:
                callables[c] = make_callable(self.diffusivity[c], sv + self.diff_site_fractions[c])
            else:
                callables[c] = make_callable(exp(self.diffusivity[c]), sv + self.diff_site_fractions[c])
        return callables
    ...
    def build_mobility(self, dbe):
        ...
        for c in self.components:
            if c.name != 'VA':
                ...
                diff[c.name] = (self.mob_models['DF'][c.name] + self.mob_models['DQ'][c.name]) / (v.R * v.T)

        return mob, diff

def tracer_diffusivity(model, eq, mobility_callables = None, mobility_correction = None):
    '''
    ...
    if mobility_callables is None:
        mobility_callables = model.mobility_callables()

    R = 8.314
    T = eq.T.values.ravel()[0]
    ...
    return R * T * np.array([mobility_correction[elements[A]] * mobility_callables[elements[A]](*values) for A in range(len(elements))])

def mobility_matrix(model, eq, mobility_callables = None, mobility_correction = None):
    '''
    Mobility matrix
    ...
    values = [v for s, v in symbolMap]
    elements = eqsub.component.values.ravel()
    X = eqsub.X.values.ravel()
    ...
    mob = np.array([X[A] * mobility_correction[elements[A]] * mobility_callables[elements[A]](*values) for A in range(len(elements))])
...
def chemical_diffusivity(model, eq, free_energy_callables, mobility_callables = None, mobility_correction = None, returnHessian = False):
    '''
    ...
    model : Model
    eq : Dataset from pycalphad equilibrium result
    free_energy_callables : dict
        Pre-computed free energy callables for phase
    mobility_callables : dict (optional)
        Pre-computed mobility callables for each element
    mobility_correction : dict (optional)
        Factor to multiply mobility by for each given element (defaults to 1)
    ...
    '''
    dmudx = partialdMudX(model, eq, free_energy_callables)
    mobMatrix = mobility_matrix(model, eq, mobility_callables, mobility_correction)
...
def interdiffusivity(model, eq, refElement, free_energy_callables, mobility_callables = None, mobility_correction = None, returnHessian = False):
    '''
    ...
    Dkj, hessian = chemical_diffusivity(model, eq, free_energy_callables, mobility_callables, mobility_correction, returnHessian)
    elements = eq.component.values.ravel()
...
def interdiffusivity_from_diff(model, eq, refElement, diffusivity_callables, diffusivity_correction = None):
    '''
    ...
    for a in range(len(elements) - 1):
        ...
        Daa = diffusivity_correction[elements[eleIndex]] * diffusivity_callables[elements[eleIndex]](*values)
        Dnkj[a, a] = Daa
...
def inverseMobility(model, eq, refElement, free_energy_callables, mobility_callables = None, mobility_correction = None, returnOther = True):
    '''
    ...
    refElement : str
        Reference element n
    free_energy_callables : dict
        Pre-computed free energy callables
    mobility_callables : dict (optional)
        Pre-computed mobility callables for each element
    mobility_correction : dict (optional)
        Factor to multiply mobility by for each given element (defaults to 1)
    ...
    '''
    Dnkj, _ = interdiffusivity(model, eq, refElement, free_energy_callables, mobility_callables, mobility_correction, False)
    totalH = dMudX(model, eq, refElement, free_energy_callables)
...
def inverseMobility_from_diffusivity(model, eq, refElement, free_energy_callables, diffusivity_callables, diffusivity_correction = None, returnOther = True):
    '''
    Inverse mobility matrix for determining interfacial composition
    ...
    (interdiffusivity, hessian, inverse mobility)
        Interdiffusivity and hessian will be None if returnOther is False
    '''
    Dnkj = interdiffusivity_from_diff(model, eq, refElement, diffusivity_callables, diffusivity_correction)
    totalH = dMudX(model, eq, refElement, free_energy_callables)
