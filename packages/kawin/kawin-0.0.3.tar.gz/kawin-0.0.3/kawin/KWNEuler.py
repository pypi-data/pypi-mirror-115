...
class PrecipitateModel (PrecipitateBase):
    ...
    def createLookup(self, i = 0):
        ...
        for p in range(len(self.phases)):
            self.PSDXalpha.append(np.zeros(self.bins + 1))
            self.PSDXbeta.append(np.zeros(self.bins + 1))
    ...
    def _postCheckDT(self, i):
        ...
        if not any(checks):
            ...
            self.avgAR[:,i] = 0
            self.nucRate[:,i] = 0
            self.numberOfNucleations[:,i] = 0
            self.betaFrac[:,i] = 0
            self.dGs[:,i] = 0

            self.prevFConc[0] = copy.copy(self.prevFConc[1])

            self._divideTimestep(i, (self.time[i] - self.time[i-1]) / 2)
    ...
    def _massBalance(self, i):
        ...
        for p in range(len(self.phases)):
            ...
            if Ntot[p] > 0:
                ...
                self.numberOfNucleations[p, i] = Ntot[p]
                self.avgAR[p, i] = ARsum[p] / Ntot[p]
            else:
                self.avgR[p, i] = 0
                self.numberOfNucleations[p, i] = 0
                self.avgAR[p, i] = 0
