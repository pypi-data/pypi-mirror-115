...
def test_DG_binary():
    ...
    '''
    dg, _ = AlZrTherm.getDrivingForce(0.004, 673.15)
    assert_allclose(dg, 6345.930428, atol=1e-1)
...
def test_DG_binary_output():
    ...
    '''
    dg, xP = AlZrTherm.getDrivingForce(0.004, 673.15, returnComp=True)
    dgarray, xParray = AlZrTherm.getDrivingForce([0.004, 0.005], [673.15, 683.15], returnComp=True)
...
def test_DG_ternary():
    ...
    '''
    dg, _ = NiCrAlTherm.getDrivingForce([0.08, 0.1], 1073.15)
    assert_allclose(dg, 244.012026, atol=1e-2)
...
def test_DG_ternary_output():
    ...
    '''
    dg, xP = NiCrAlTherm.getDrivingForce([0.08, 0.1], 1073.15, returnComp=True)
    dgarray, xParray = NiCrAlTherm.getDrivingForce([[0.08, 0.1], [0.085, 0.1], [0.09, 0.1]], [1073.15, 1078.15, 1083.15], returnComp=True)
    assert np.isscalar(dg) or (type(dg) == np.ndarray and dg.ndim == 0)
...
def test_DG_order():
    ...
    '''
    dg1, _ = NiCrAlTherm.getDrivingForce([0.08, 0.1], 1073.15)
    dg2, _ = NiAlCrTherm.getDrivingForce([0.1, 0.08], 1073.15)
    dg3, _ = AlCrNiTherm.getDrivingForce([0.08, 0.82], 1073.15)
    assert_allclose(dg1, dg2, atol=1e-2)
    assert_allclose(dg2, dg3, atol=1e-2)
...
def test_IC_binary():
    ...
    assert_allclose(xm2, 0.0233507, atol=1e-5)
...
def test_Mob_binary():
    ...
    assert_allclose(dnkj, 1.280344e-20, rtol=0.001)
...
def test_Mob_ternary():
    ...
    assert_allclose(dnkj, [[5.26834399e-17, 2.41155166e-17], [6.18753651e-18, 8.81684280e-18]], rtol=0.001)
...
def test_Mob_order():
    ...
    dnkj2[:,[0,1]] = dnkj2[:,[1,0]]
    dnkj2[[0,1],:] = dnkj2[[1,0],:]
    assert_allclose(dnkj1, dnkj2, rtol=0.001)

def test_Curv_ternary():
    '''
    Checks that order of elements does not matter for curvature calculations
    '''
    n1, d1, g1, b1, ca1, cb1 = NiCrAlTherm.curvatureFactor([0.08, 0.1], 1073.15)
    n2, d2, g2, b2, ca2, cb2 = NiAlCrTherm.curvatureFactor([0.1, 0.08], 1073.15)
    ...
    assert_allclose(n1, n2, rtol=0.001)
    assert_allclose(d1, d2, rtol=0.001)
    assert_allclose(g1, g2, rtol=0.001)
    assert_allclose(b1, b2, rtol=0.001)
    assert_allclose(ca1, ca2, rtol=0.001)
    assert_allclose(cb1, cb2, rtol=0.001)

def test_IC_ternary():
    '''
    Checks that order does not matter for growth and interfacial composition calcualtions
    '''
    g1, ca1, cb1 = NiCrAlTherm.getGrowthAndInterfacialComposition([0.08, 0.1], 1073.15, 900, 1e-9, 1000)
    g2, ca2, cb2 = NiAlCrTherm.getGrowthAndInterfacialComposition([0.1, 0.08], 1073.15, 900, 1e-9, 1000)

    ca2[[0,1]] = ca2[[1,0]]
    cb2[[0,1]] = cb2[[1,0]]

    assert_allclose(g1, -1.66241802e-09, rtol=0.001)
...
def test_IC_ternary_output():
    ...
    '''
    g, ca, cb = NiCrAlTherm.getGrowthAndInterfacialComposition([0.08, 0.1], 1073.15, 900, 1e-9, 1000)
    garray, caarray, cbarray = NiCrAlTherm.getGrowthAndInterfacialComposition([0.08, 0.1], 1073.15, 900, [0.5e-9, 1e-9, 2e-9], [2000, 1000, 500])
