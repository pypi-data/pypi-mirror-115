...
def _deserialize_np_array(aws_credentials: AwsCredentialsFetcher, value):
    ...
    return np.load(
        DataTransfer.download_to_bytes(global_client().get_download_config(), aws_credentials, value)  # type: ignore
    )
...
def _download_df(
    ...
    for col_num in range(len(is_col_json)):  # pylint: disable=consider-using-enumerate
        if is_col_json[col_num]:
            df.iloc[:, col_num] = df.iloc[:, col_num].apply(
                lambda x: None if x is None else json.loads(x)
            )
    return df
