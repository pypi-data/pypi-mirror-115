...
class Command():
    ...
    def update_timestamps(self, pathname, delta_mtime=None):
    ...
    def add_package(self, name, files, or_modes={}, and_modes={},
                version=PKG_VERSION, release=PKG_RELEASE, cache_dir=None,
                delta_mtime=None):
        """Add a package."""
        cache_dir = self.cache_dir if cache_dir is None else cache_dir
        if not os.path.isdir(cache_dir):
            os.makedirs(cache_dir)
        pkg_path = os.path.join(cache_dir, '%s-%s-%s-%s.pkg.tar.%s' %
                    (name, version, release, os.uname().machine, EXTENSION))
        with temp_cwd():
            self.add_files(files, or_modes=or_modes, and_modes=and_modes)
            with tarfile_open(pkg_path, EXTENSION, mode='w') as tar:
                tar.add(ROOT_SUBDIR)
        self.update_timestamps(pkg_path, delta_mtime)
        return pkg_path
...
class UpdateTestCase(CommandsTestCase):
    ...
    def test_signature_file(self):
        ...
        path = os.path.join(self.cmd.cache_dir,
                    'package-%s-%s-%s.pkg.tar.%s.sig' %
                    (PKG_VERSION, PKG_RELEASE, os.uname().machine, EXTENSION))
        with open(path, 'w') as f:
            f.write('signature')
        # Ensure that the sig file is more recent than the package file,
        # otherwise the sig file may be ignored anyway.
        self.cmd.update_timestamps(path, delta_mtime=120)
