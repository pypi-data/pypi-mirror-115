...
for i in os.getenv("PATH").split(separator):
    ...
    cp = os.path.join(i, "gswin64c.exe")
...
if GSPATH is None:
    print("gs / gswin32c / gswin64c not in path")
    sys.exit(1)
...
class Crop(ABC):
    ...
    def pdf2tif(self, fp):
        ...
        if False:
            os.system("C:\\PROGRA~2\\IrfanView\\i_view32.exe " + outf)
...
class DHL(Crop):
    ...
    def save(self):
        ofs = 0
        timg = Image.new("RGB", (Crop.mmtopixels(62*10, 300), Crop.mmtopixels(180*10, 300)), (255,255,255))
        ...
        # sicherheitscode
        sc_sec = self.sections[11]
        sc = sc_sec.data.rotate(360, expand=True)
        sc = sc.resize((round(sc.width * 0.6), round(sc.height * 0.6)), Image.LANCZOS)
        # center in x axis
        center_x = (timg.width/2) - (sc.width/2)
        timg.paste(sc, (round(center_x), ofs-sc.height))
        ofs += sc_sec.margin

        # strich unter sicherheitscode
        timg.paste(strich, (0, ofs-30))
        ofs += strich.height
