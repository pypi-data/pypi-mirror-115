...
class OptimizationCockpitWidget:
    ...
    def display_geometry_graph(self, index, row=3, col=2):
        ...
        geometry = CosThetaGeometry.with_roxie_relative_angle_definition_dict(fit_input, self.cadata)
        geometry.build_blocks()

        go_scatter = OptimizationCockpitWidget.create_plotly_scatter_blocks(geometry)
    ...
    @staticmethod
    def create_plotly_scatter_blocks(geometry):
        # ToDo: This should come from the geometry class
        idx = 1
        go_scatter = []
        for block in geometry.blocks:
            for area in block.areas:
                ...
                go_scatter.append(go.Scatter(x=x, y=y, fill="toself", fillcolor='white', marker=dict(size=1),
                                             name='turn' + str(idx), line=go.scatter.Line(color='blue')))
                idx += 1
