...
class TestRectangularGeometry(unittest.TestCase):
    ...
    def test_build_geometry_rectangle_with_csv(self, mock_show=None):
        # arrange
        csv_path = create_resources_file_path('resources/geometry/roxie/SMC/three_cable_coil.csv')
        cadata_path = create_resources_file_path('resources/geometry/roxie/SMC/roxie-SMC.cadata')
        cadata = CableDatabase.read_cadata(cadata_path)

        # act
        geometry = GeometryFactory.init_with_csv(csv_path, cadata)
        geometry.build_blocks()
        geometry.plot_blocks()
    ...
    def test_to_roxie_df_with_csv(self):
        ...
        pd.testing.assert_frame_equal(roxie_ref.reset_index(drop=True), geometry.to_roxie_df().reset_index(drop=True))
