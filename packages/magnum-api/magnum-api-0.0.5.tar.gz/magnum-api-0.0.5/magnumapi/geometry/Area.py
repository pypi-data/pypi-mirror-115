...
class Area:
    """ Area class implementing a 2D area made of connected lines

    """
    def __init__(self, lines: List[Line]) -> None:
        """ Constructor of an Area class from a list of Line objects
    ...
    @staticmethod
    def of_lines(lines: List[Line]) -> "Area":
        """ Static method creating an Area object from a list of Line objects. Line objects are copied to preserve
        static behaviour and avoid side effects.

        :param lines: list of Line objects
        :return: An area with copy of input Line objects
        """
        return Area(lines=[line.copy() for line in lines])
    ...
    def copy(self) -> "Area":
        ...
        """
        return Area(lines=[line.copy() for line in self.lines])
    ...
    def translate(self, vector: Point) -> "Area":
        ...
        """
        return Area(lines=[line.translate(vector) for line in self.lines])
    ...
    def rotate(self, angle: float) -> "Area":
        ...
        :return: rotated area
        """
        return Area(lines=[line.rotate(angle) for line in self.lines])
