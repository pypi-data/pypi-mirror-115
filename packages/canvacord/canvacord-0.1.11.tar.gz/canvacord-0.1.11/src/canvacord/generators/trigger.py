...
async def trigger(user):
        avatar = await getavatar(user).resize((320, 320)).convert('RGBA')
        triggered = Image.open(await getbackground("triggered"))
        tint = Image.open(await getbackground("red")).convert('RGBA')
