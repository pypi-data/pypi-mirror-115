...
async def trigger(user, username, currentxp, lastxp, nextxp, level, rank, background=None):
        avatar = getavatar(user).resize((320, 320)).convert('RGBA')
        triggered = Image.open(getbackground("triggered"))
        tint = Image.open(getbackground("red")).convert('RGBA')
