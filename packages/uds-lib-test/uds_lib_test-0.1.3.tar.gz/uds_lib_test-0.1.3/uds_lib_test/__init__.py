...
def transaction_create(company_id, api_key, total, cash, points, code=None,
...
def customers_list(company_id, api_key, max=10, offset=0):
    auth = partner_auth(company_id, api_key)
    try:
        params = 'max=' + str_parse(max) + '&offset=' + str_parse(offset)
        con = HTTPSConnection('api.uds.app', timeout=60)
...
def customer_get(company_id, api_key, participant_id):
    auth = partner_auth(company_id, api_key)
    try:
        con = HTTPSConnection('api.uds.app')
        con.request(
...
def items_list(company_id, api_key, max=10, offset=0, node_id=None):
    auth = partner_auth(company_id, api_key)
    try:
        params = "max=" + str_parse(max) + "&offset=" + str_parse(offset) + \
...
def item_or_category_get(company_id, api_key, goods_id):
    auth = partner_auth(company_id, api_key)
    try:
        con = HTTPSConnection('api.uds.app', timeout=60)
        con.request(
...
def order_get(company_id, api_key, order_id):
    auth = partner_auth(company_id, api_key)
    try:
        con = HTTPSConnection('api.uds.app', timeout=60)
        con.request(
...
def generate_complete_order_code(company_id, api_key, order_id):
    auth = partner_auth(company_id, api_key)
    try:
        con = HTTPSConnection('api.uds.app', timeout=60)
        con.request(
