...
def get_order_by_number(orders: Stream, number: str):
...
class Connection(object):
    ...
    def get_employee(self):
        ...
        if not response.ok:
            ...
            employees = Stream\
                .create(root.xpath(f"//div[@class='tooltip']/../..")) \
                .map(lambda n: {
    ...
    def get_contract_orders(self) -> Stream:
        ...
        if not response.ok:
            ...
            root = etree.fromstring(response.content, etree.HTMLParser())
            self.contract_orders = Stream\
                .create(root.xpath("//table//td/div[@class='tooltip']/../..")) \
                .map(lambda n: {
                "id": n.xpath("td//tr/td[text()='id:']/following-sibling::td")[0].text,
                "order": {
    ...
    def get_sub_orders(self) -> Stream:
    ...
    def get_report(self, contract, start_day, end_day):
        ...
        if not response.ok:
            ...
            return Stream\
                ...
                "comment": trim_whitespace(n.xpath("td[6]/textarea")[0].text),
                "duration": (float(safe_xpath(n, "td[8]/select[1]/option[@selected]", "0.0")) * 60.0 +
                             float(safe_xpath(n, "td[8]/select[2]/option[@selected]", "0.0"))) / 60.0,
            })
