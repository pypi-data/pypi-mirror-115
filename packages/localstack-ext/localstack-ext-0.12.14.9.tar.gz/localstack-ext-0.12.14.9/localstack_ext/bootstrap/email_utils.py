...
SENT_EMAILS=[]
EMAIL_BLACKLIST=quxFC()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",quxFb(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=quxFc,from_name=quxFc,smtp_host=quxFc,smtp_user=quxFc,smtp_pass=quxFc,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if quxFE(recipients,quxFt)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=quxFc):
 ...
 result["From"]=formataddr((quxFb(Header(from_name,"utf-8")),from_email))
 return result
