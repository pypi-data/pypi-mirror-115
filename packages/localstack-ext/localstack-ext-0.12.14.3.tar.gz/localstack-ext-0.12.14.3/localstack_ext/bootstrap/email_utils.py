...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DLOkQ()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DLOkw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DLOkj,from_name=DLOkj,smtp_host=DLOkj,smtp_user=DLOkj,smtp_pass=DLOkj,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DLOkC(recipients,DLOkf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DLOkj):
 ...
 result["From"]=formataddr((DLOkw(Header(from_name,"utf-8")),from_email))
 return result
