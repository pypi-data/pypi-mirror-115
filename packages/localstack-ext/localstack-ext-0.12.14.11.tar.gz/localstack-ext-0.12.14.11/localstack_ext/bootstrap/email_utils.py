...
SENT_EMAILS=[]
EMAIL_BLACKLIST=hEeQm()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",hEeQG(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=hEeQc,from_name=hEeQc,smtp_host=hEeQc,smtp_user=hEeQc,smtp_pass=hEeQc,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if hEeQA(recipients,hEeQo)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=hEeQc):
 ...
 result["From"]=formataddr((hEeQG(Header(from_name,"utf-8")),from_email))
 return result
