...
class CloudPodManager(UyJzi):
 BACKEND="_none_"
 def __init__(self,pod_name=UyJzx,config=UyJzx):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise UyJzp("Not implemented")
 def pull(self):
  raise UyJzp("Not implemented")
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert UyJzO(statuses)==UyJzO(["running"])
 ...
 def get(cls,pod_name):
  ...
  raise UyJzp('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @UyJzR
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise UyJzp("Working with local cloud pods requires $DATA_DIR configuration")
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self):
  ...
  data_dir=self.data_dir()
  zip_data_content=create_zip_file(data_dir,get_content=UyJzu)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(UyJzH(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise UyJzp("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
 def pull(self):
  data_dir=self.data_dir()
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise UyJzp("Unable to push cloud pod to API (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b"HEAD" not in repo or repo.refs.allkeys()==UyJzO([b"HEAD"])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=UyJzu)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise UyJzp('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
  ...
  for folder in PERSISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=UyJzu)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get("url"))
  return client,path
 def local_repo(self):
  self.clone_dir=UyJzt(self,"clone_dir",UyJzx)
...
class PodConfigManagerMeta(UyJzl):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=UyJzx
   for manager in cls.CHAIN:
    try:
     tmp=UyJzt(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif UyJzh(tmp,UyJzK)and UyJzh(result,UyJzK):
       result.extend(tmp)
    except UyJzp:
     pass
   if result is not UyJzx:
    return result
   raise UyJzp('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(UyJzi,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @UyJzR
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise UyJzp('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(UyJzi):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=UyJzu).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=UyJzO([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=UyJzS):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except UyJzp:
   if safe:
    return{}
   raise UyJzp('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(UyJzi):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise UyJzp("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise UyJzp("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise UyJzp("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
