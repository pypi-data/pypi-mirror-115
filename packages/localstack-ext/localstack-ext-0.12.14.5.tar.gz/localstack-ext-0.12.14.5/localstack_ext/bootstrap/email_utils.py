...
SENT_EMAILS=[]
EMAIL_BLACKLIST=WAPKC()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",WAPKS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=WAPKH,from_name=WAPKH,smtp_host=WAPKH,smtp_user=WAPKH,smtp_pass=WAPKH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if WAPKe(recipients,WAPKv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=WAPKH):
 ...
 result["From"]=formataddr((WAPKS(Header(from_name,"utf-8")),from_email))
 return result
