...
SENT_EMAILS=[]
EMAIL_BLACKLIST=hiPNm()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",hiPNS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=hiPNE,from_name=hiPNE,smtp_host=hiPNE,smtp_user=hiPNE,smtp_pass=hiPNE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if hiPNW(recipients,hiPNG)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=hiPNE):
 ...
 result["From"]=formataddr((hiPNS(Header(from_name,"utf-8")),from_email))
 return result
