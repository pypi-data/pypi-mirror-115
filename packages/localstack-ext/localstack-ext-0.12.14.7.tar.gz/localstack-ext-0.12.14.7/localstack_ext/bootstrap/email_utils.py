...
SENT_EMAILS=[]
EMAIL_BLACKLIST=UWSra()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",UWSrB(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=UWSrm,from_name=UWSrm,smtp_host=UWSrm,smtp_user=UWSrm,smtp_pass=UWSrm,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if UWSry(recipients,UWSrV)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=UWSrm):
 ...
 result["From"]=formataddr((UWSrB(Header(from_name,"utf-8")),from_email))
 return result
