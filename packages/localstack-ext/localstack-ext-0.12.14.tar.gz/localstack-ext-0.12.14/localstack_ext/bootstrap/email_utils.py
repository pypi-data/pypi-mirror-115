...
SENT_EMAILS=[]
EMAIL_BLACKLIST=IDfoM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",IDfoK(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=IDfoG,from_name=IDfoG,smtp_host=IDfoG,smtp_user=IDfoG,smtp_pass=IDfoG,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if IDfoB(recipients,IDfoA)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=IDfoG):
 ...
 result["From"]=formataddr((IDfoK(Header(from_name,"utf-8")),from_email))
 return result
