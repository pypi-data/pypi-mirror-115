...
SENT_EMAILS=[]
EMAIL_BLACKLIST=uknaO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",uknaU(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=uknae,from_name=uknae,smtp_host=uknae,smtp_user=uknae,smtp_pass=uknae,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if uknaf(recipients,uknao)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=uknae):
 ...
 result["From"]=formataddr((uknaU(Header(from_name,"utf-8")),from_email))
 return result
