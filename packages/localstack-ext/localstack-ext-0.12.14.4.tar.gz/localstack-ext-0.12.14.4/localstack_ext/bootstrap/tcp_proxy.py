...
class AttrDict(VEHFv):
 def __init__(self,*args,**kwargs):
  VEHFo(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and VEHFn(socket,"IPV6_V6ONLY"):
...
class Half(VEHFb):
 def __init__(self,opt,sock,addr,VEHFg):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.VEHFg=VEHFg
  self.name="peer" if self.VEHFg else "client"
  self.queue=[]
  self.dest=VEHFc
  self.err=VEHFc
  self.ready=VEHFx
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":VEHFx,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":VEHFx,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":VEHFx,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":VEHFx,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
