...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qJMOy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qJMOg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qJMOl,from_name=qJMOl,smtp_host=qJMOl,smtp_user=qJMOl,smtp_pass=qJMOl,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qJMOY(recipients,qJMOW)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qJMOl):
 ...
 result["From"]=formataddr((qJMOg(Header(from_name,"utf-8")),from_email))
 return result
