...
def register_localstack_plugins():
 ...
 if jkNne([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  from localstack_ext.services.kms import kms_listener,kms_starter
  ...
  if "No module named" not in jkNnu(e):
   print("ERROR: %s"%e)
...
def api_key_configured():
 return jkNnW(os.environ.get("LOCALSTACK_API_KEY"))
