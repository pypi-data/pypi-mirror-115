...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DYKRU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DYKRq(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DYKRI,from_name=DYKRI,smtp_host=DYKRI,smtp_user=DYKRI,smtp_pass=DYKRI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DYKRe(recipients,DYKRp)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DYKRI):
 ...
 result["From"]=formataddr((DYKRq(Header(from_name,"utf-8")),from_email))
 return result
