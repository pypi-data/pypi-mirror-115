...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SEIGy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SEIGa(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SEIGn,from_name=SEIGn,smtp_host=SEIGn,smtp_user=SEIGn,smtp_pass=SEIGn,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SEIGx(recipients,SEIGl)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SEIGn):
 ...
 result["From"]=formataddr((SEIGa(Header(from_name,"utf-8")),from_email))
 return result
