...
SENT_EMAILS=[]
EMAIL_BLACKLIST=EaPKB()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",EaPKC(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=EaPKU,from_name=EaPKU,smtp_host=EaPKU,smtp_user=EaPKU,smtp_pass=EaPKU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if EaPKc(recipients,EaPKm)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=EaPKU):
 ...
 result["From"]=formataddr((EaPKC(Header(from_name,"utf-8")),from_email))
 return result
