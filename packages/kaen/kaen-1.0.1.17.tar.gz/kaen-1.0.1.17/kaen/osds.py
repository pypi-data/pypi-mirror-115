...
#         eager_load_batches = True if protocol in ('file') and eager_load_batches is None else eager_load_batches
        ...
        if index_shards is None:   
            ...
            if shards_glob is None and protocol not in ('http', 'https'):
                ...
                if self.fs.isfile(glob) or (not self.fs.isfile(glob) and not self.fs.isdir(glob)):
                    ...
                    #remove trailing / characters
                    maybe_shards_glob = f"{remove_trailing_forward_slash(glob)}/.meta/shards/"                
                else:
                    logging.warn(f"failed to construct a shards dataframe path for glob={glob}")
                    maybe_shards_glob = None
                
                maybe_shards = self.fs.glob(maybe_shards_glob)
                if not isinstance(maybe_shards, list) or not len(maybe_shards):
                    logging.warn(f"shards_glob is not specified at initialization, failed to find shard metadata at default location {maybe_shards_glob}")
                else:
                    [shards_glob] = maybe_shards
                    shards_glob = f"{protocol}://{shards_glob}"
                    logging.warn(f"shards_glob is not specified at initialization, defaulting to shards_glob={shards_glob}")

            if shards_glob is None:
                index_shards = True
                logging.warn("shards_glob is not specified at initialization, attempting to proceed with index_shards=True which can take a while for large objects.")
            else:
                index_shards = False
        else:
    ...
    def __len__(self):
        if self.shards_df is not None:
            return self.shards_df.iloc[-1]['end_idx']
        else:
            return -1
        ...
        while self.max_shards:
            try:
                ...
                for row_start_idx, row_end_idx in zip(row_start_indicies, row_end_indicies):
                    print(f"*** {row_start_idx=}, {row_end_idx=} ***")
