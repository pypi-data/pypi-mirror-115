...
def visualize_alignment_by_db(db_path, output_path, lang_name_from="ru", lang_name_to="de", batch_size=0, size=(260, 300), batch_ids=[], plt_show=False, transparent_bg=False, show_info=False, show_regression=False):
        ...
        if i in batch_ids or len(batch_ids) == 0:
            ...
            try:
                ...
                save_pic(align_matrix, lang_name_to, lang_name_from,
                        output_path, batch_number=i, interval_x=(x_min, x_max), interval_y=(y_min, y_max), size=size, plt_show=plt_show, transparent=transparent_bg, shift=shift, window=window, show_info=show_info, show_regression=show_regression)
            except Exception as e:
                logging.error(e, exc_info=True)


def save_pic(align_matrix, lang_name_to, lang_name_from, output_path, batch_number, interval_x, interval_y, size=(260, 260), plt_show=False, transparent=False, shift=None, window=None, show_info=False, show_regression=False):
