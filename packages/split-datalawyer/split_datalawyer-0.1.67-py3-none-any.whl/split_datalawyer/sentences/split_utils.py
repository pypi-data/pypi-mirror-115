...
def _ends_with_parenthesis(text: str) -> bool:
    if len(__PARENTHESIS_ENDING__.findall(text)) > 0:
...
def sanitize_split(texto, quote_chars=None, trechos=None, split_parenthesis_ending: bool = False) -> List[str]:
...
def split_by_sentence(texto, usar_ponto_virgula=False, split_parenthesis_ending: bool = False):
    ...
    if type(texto) == str:
        ...
        # return [sentence for paragrafo in sentence_tokenize(sentence_tokenizer, texto) for sentence in
        #         sanitize_split(paragrafo)]
        ...
        for subtexto in texto:
            ...
            retorno.extend(split_by_sentence(subtexto, usar_ponto_virgula, split_parenthesis_ending))
        return retorno


def split(texto, usar_ponto_virgula=False, split_parenthesis_ending: bool = False):
    segmentos_por_quebra = split_by_break(texto)
    return split_by_sentence(segmentos_por_quebra, usar_ponto_virgula, split_parenthesis_ending)
