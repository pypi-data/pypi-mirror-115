...
from ..base import (get_expiry, validate_attachment_url, validate_email,
                    validate_template)
from ..common import Platform, Waterfall
...
class Email:
    ...
    def __init__(
            ...
            expiry: int = None,
            platform: Platform = Platform.Waahjobs
    ):
        """
        Parameters:
    ...
    def __set_sender(self, sender: EmailRecipient = None):
        if sender:
            return sender.proto
        if self.platform == Platform.Waahjobs:
            return EmailRecipient('noreply@olx.com', 'Waahjobs').proto
        else:
