...
class Template(object):
    ...
    def replace(self, eid, replacement, index=0, append_children=False):
        """
        ...
        if index < len(elems):
            ...
            if append_children:
                ...
                parent_elem.children.remove(elem)
            replacement_type = type(replacement)
            if replacement_type in (Element, TextElement):
                self.check_element(replacement, True)
                if append_children:
                    parent_elem.children.extend(replacement.children)
                else:
                    parent_elem.children.insert(current_pos, replacement)
                replacement.parent = elem.parent
            elif replacement_type == Template:
                if append_children:
                    children = replacement.root.children[0].children
                else:
                    children = replacement.root.children

                for child in children:
                    parent_elem.children.insert(current_pos, child)
