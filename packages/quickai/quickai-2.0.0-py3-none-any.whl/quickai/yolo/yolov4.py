...
def decode_tflite(
    ...
    for idx, dxdy in enumerate(conv_raw_dxdy):
        dxdy = ((tf.sigmoid(dxdy) * XYSCALE[i]) -
...
def filter_boxes(box_xywh, scores, score_threshold=0.4,
...
def compute_loss(
    ...
    prob_loss = respond_bbox * \
        tf.nn.sigmoid_cross_entropy_with_logits(labels=label_prob, logits=conv_raw_prob)
