...
def upload_to_run(
    ...
    with TemporaryDirectory() as td:
        ...
        categorical_features_path = os.path.join(td, ModelAnalysisFileNames.CATEGORICAL_FEATURES)
        with open(categorical_features_path, 'w+') as categorical_features_file:
            json.dump(settings.categorical_column_names, categorical_features_file)
