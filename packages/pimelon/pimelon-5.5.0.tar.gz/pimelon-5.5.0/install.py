...
tmp_pine_repo = os.path.join('/', 'tmp', '.pine')
...
def check_environment():
	...
	if message:
		log("Pine's CLI needs these to be defined!", level=3)
...
def check_system_package_managers():
	if 'Darwin' in os.uname():
		if not shutil.which('brew'):
			raise Exception('''
			Please install brew package manager before proceeding with pine setup. Please run following
...
def import_with_install(package):
	# copied from https://discuss.monakerp.com/u/nikunj_patel
	# https://discuss.monakerp.com/t/easy-install-setup-guide-for-monak-installation-on-ubuntu-20-04-lts-with-some-modification-of-course/62375/5
...
def install_prerequisites():
	# pre-requisites for pine repo cloning
	run_os_command({
		'apt-get': [
			'sudo apt-get update',
...
def install_pine(args):
	# clone pine repo
	if not args.run_travis:
		clone_pine_repo(args)
	...
	extra_vars.update(melon_user=args.user)

	extra_vars.update(user_directory=get_user_home_directory(args.user))

	if os.path.exists(tmp_pine_repo):
		repo_path = tmp_pine_repo
	else:
		repo_path = os.path.join(os.path.expanduser('~'), 'pine')
	...
	melon_branch = 'version-5'
	monak_branch = 'version-5'

	if args.version:
		if args.version <= 10:
			melon_branch = "{0}.x.x".format(args.version)
			monak_branch = "{0}.x.x".format(args.version)
		else:
			melon_branch = "version-{0}".format(args.version)
			monak_branch = "version-{0}".format(args.version)
	# Allow override of melon_branch and monak_branch, regardless of args.version (which always has a default set)
	if args.melon_branch:
		melon_branch = args.melon_branch
	if args.monak_branch:
		monak_branch = args.monak_branch

	extra_vars.update(melon_branch=melon_branch)
	extra_vars.update(monak_branch=monak_branch)

	pine_name = 'pimelon' if not args.pine_name else args.pine_name
	extra_vars.update(pine_name=pine_name)

	# Will install Monak production setup by default
	if args.without_monak:
		log("Initializing pine {pine_name}:\n\tMelon Branch: {melon_branch}\n\tMonak will not be installed due to --without-monak".format(pine_name=pine_name, melon_branch=melon_branch))
	else:
		log("Initializing pine {pine_name}:\n\tMelon Branch: {melon_branch}\n\tMonak Branch: {monak_branch}".format(pine_name=pine_name, melon_branch=melon_branch, monak_branch=monak_branch))
	run_playbook('site.yml', sudo=True, extra_vars=extra_vars)

	if os.path.exists(tmp_pine_repo):
		shutil.rmtree(tmp_pine_repo)


def clone_pine_repo(args):
	'''Clones the pine repository in the user folder'''
	branch = args.pine_branch or 'develop'
	repo_url = args.repo_url or 'https://github.com/amonak/pine'

	if os.path.exists(tmp_pine_repo):
		log('Not cloning already existing Pine repository at {tmp_pine_repo}'.format(tmp_pine_repo=tmp_pine_repo))
		return 0
	elif args.without_pine_setup:
		clone_path = os.path.join(os.path.expanduser('~'), 'pine')
		log('--without-pine-setup specified, clone path is: {clone_path}'.format(clone_path=clone_path))
	else:
		clone_path = tmp_pine_repo
		# Not logging repo_url to avoid accidental credential leak in case credential is embedded in URL
		log('Cloning pine repository branch {branch} into {clone_path}'.format(branch=branch, clone_path=clone_path))

	success = run_os_command(
		{'git': 'git clone --quiet {repo_url} {pine_repo} --depth 1 --branch {branch}'.format(
			repo_url=repo_url, pine_repo=clone_path, branch=branch)}
	)
...
def get_passwords(args):
	...
	and creates passwords.txt in the pine user's home directory
	"""
	log("Input MySQL and Melon Administrator passwords:")
	ignore_prompt = args.run_travis or args.without_pine_setup
...
def run_playbook(playbook_name, sudo=False, extra_vars=None):
	...
	if os.path.exists(tmp_pine_repo):
		...
		cwd = os.path.join(os.path.expanduser('~'), 'pine')

	playbooks_locations = [os.path.join(cwd, 'pine', 'playbooks'), os.path.join(cwd, 'playbooks')]
...
def parse_commandline_args():
	...
	parser = argparse.ArgumentParser(description='Melon Installer')
	...
	args_group.add_argument('--production', dest='production', action='store_true', default=False, help='Setup Production environment for pine')
	parser.add_argument('--site', dest='site', action='store', default='monakweb', help='Specify name for your first Monak site')
	...
	parser.add_argument('--user', dest='user', help='Install pimelon for this user')
	parser.add_argument('--pine-branch', dest='pine_branch', help='Clone a particular branch of pine repository')
	parser.add_argument('--repo-url', dest='repo_url', help='Clone pine from the given url')
	parser.add_argument('--melon-repo-url', dest='melon_repo_url', action='store', default='https://github.com/amonak/melon', help='Clone melon from the given url')
	parser.add_argument('--melon-branch', dest='melon_branch', action='store', help='Clone a particular branch of melon')
	parser.add_argument('--monak-repo-url', dest='monak_repo_url', action='store', default='https://github.com/amonak/monak', help='Clone monak from the given url')
	parser.add_argument('--monak-branch', dest='monak_branch', action='store', help='Clone a particular branch of monak')
	parser.add_argument('--without-monak', dest='without_monak', action='store_true', default=False, help='Prevent fetching Monak')
	# direct provision to install versions
	parser.add_argument('--version', dest='version', action='store', default='12', type=int, help='Clone particular version of melon and monak')
	...
	parser.add_argument('--without-pine-setup', dest='without_pine_setup', action='store_true', default=False, help=argparse.SUPPRESS)
	# whether to overwrite an existing pine
	parser.add_argument('--overwrite', dest='overwrite', action='store_true', default=False, help='Whether to overwrite an existing pine')
	# set passwords
	...
	parser.add_argument('--pine-name', dest='pine_name', help='Create pine with specified name. Default name is pimelon')
...
if __name__ == '__main__':
	...
	log("Pine + Melon + Monak has been successfully installed!")
