...
def dense_lucaskanade(R, **kwargs):
    ...
    for n in range(nr_fields - 1):
        ...
        p0 = ShiTomasi_features_to_track(
            prvs, max_corners_ST, quality_level_ST, min_distance_ST, block_size_ST
        )

        # get sparse u, v vectors with Lucas-Kanade tracking
        x0, y0, u, v = LucasKanade_features_tracking(
        ...
        max_speed_thr = np.min(
            (max_speed, q2 + nr_IQR_outlier * (q3 - q1))
        )  # [px/timesteps]
    ...
    X, Y, UV = interpolate_sparse_vectors(
        x, y, u, v, domain_size, epsilon=epsilon, smooth=smooth, nchunks=nchunks
    )

    return UV


def ShiTomasi_features_to_track(
    R, max_corners_ST, quality_level_ST, min_distance_ST, block_size_ST
):
...
def interpolate_sparse_vectors(
    ...
    for i, subgrid in enumerate(subgrids):
        ...
        if i == 0:
            rbfiu = scipy.interpolate.Rbf(
                x, y, u, function=function, epsilon=epsilon, smooth=smooth
            )
            rbfiv = scipy.interpolate.Rbf(
                x, y, v, function=function, epsilon=epsilon, smooth=smooth
            )
    ...
    if testinterpolation:
        ...
        plt.quiver(
            X_, np.flipud(Y_), UV_[0, :, :], -UV_[1, :, :], angles="xy", scale=100
        )
