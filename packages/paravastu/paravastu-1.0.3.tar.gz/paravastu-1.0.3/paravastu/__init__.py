...
def EuclideanDistance(coords1, coords2):
    if type(coords1) and type(coords2) is list:
        coords1 = numpy.array(coords1)
        coords2 = numpy.array(coords2)
    if type(coords1) and type(coords2) is tuple:
        coords1 = numpy.array(list(coords1))
        coords2 = numpy.array(list(coords2))
...
def GetHydrogenBondDistances(PDBDataFrame, ResidueStart, ResidueStop, Orientation = "Parallel"):
    ...
    if Orientation == "Parallel":
        ...
        PDBDataFrame=PDBDataFrame[PDBDataFrame["residue_number"].isin(range(ResidueStart, ResidueStop+1))]
        PDBDataFrame=PDBDataFrame[PDBDataFrame["atom_name"].isin(["HN", "O"])]
        ...
        for SegmentPair in SegmentIDPairs:
            ...
            HNArray = PDBDataFrame[(PDBDataFrame["atom_name"] == "HN") & (PDBDataFrame["segment_id"] == SegmentPair[0])][["x_coord", "y_coord", "z_coord"]]
            COArray = PDBDataFrame[(PDBDataFrame["atom_name"] == "O") & (PDBDataFrame["segment_id"] == SegmentPair[1])][["x_coord", "y_coord", "z_coord"]]
            ...
            HNArray2 = PDBDataFrame[(PDBDataFrame["atom_name"] == "HN") & (PDBDataFrame["segment_id"] == SegmentPair[1])][["x_coord", "y_coord", "z_coord"]]
            COArray2 = PDBDataFrame[(PDBDataFrame["atom_name"] == "O") & (PDBDataFrame["segment_id"] == SegmentPair[0])][["x_coord", "y_coord", "z_coord"]]
...
def CheckAllPotentialClashes(PDBDataFrame, Save = False, FileName = None):
    ...
    for count, clash in enumerate(list(combinations_with_replacement("HCON", 2))):
        FindPotentialClashes(PDBDataFrame, clash[0], clash[1], distances[count])
        if Save:
            plt.savefig(f"{FileName} {clash[0]}-{clash[1]} Potential Clashes.png", facecolor = "w")
        plt.show()
