...
def execute_objective(parallel_arguments, parameters, id_):#arguments):
    ...
    #     parallel_arguments["train_args"]= {**parallel_arguments["train_args"], "fit_inputs" :fit_inputs}
    #     parallel_arguments["test_args"]= {**parallel_arguments["train_args"], "val_inputs" : val_inputs}
    ...
    for i, fit_input in enumerate(fit_inputs):
        ...
        if train_args["ODE_order"]:
            #I want to delete this if statement.
            #1
            ...
            #     #1
            #     results = RC.fit(**fit_inputs, train_score = True, **train_args)
            #     #2
            #     train_scores = results["scores"]
            #     #3
            ...
            #     val_scores, pred_, id_ = RC.test(**val_inputs,  **test_args)
            ...
            _ = RC.fit(**fit_inputs, **parallel_arguments["train_args"])
            #train_score = process_score(train_score)
            
            val_score, pred_, id_ = RC.test(**val_input, **parallel_arguments["test_args"])
    ...
    if id_ == 0:
        return float(total_score), {"pred": pred_.to("cpu"), "val_y" : val_input["y"]}, id_
    else:
...
class EchoStateNetworkCV:
    ...
    def eval_objective(self, parameters, plot_type = "error", *args):
        ...
        #self.parallel_arguments["windowsOs"] = self.windowsOS

        


        #params = [ param in enumerate(parameter_lst)]
        
        #data_args = [(cv_samples, self.parallel_arguments, params, self.windowsOS, i) for i, params in enumerate(parameter_lst)]

        #combine fit_inputs and train_args into one dictionary
        #combine test_inputs and val_inputs into one dictionary
        ...
        #     
        #     fit_inputs = cv_sample[0]
        #     val_inputs = cv_sample[1]
        ...
        parallel_args_id = ray.put(self.parallel_arguments)
        ...
        # else:
        #     results = ray.get(execute_objective.remote(self.parallel_arguments, cv_samples[0], parameter_lst[0], 0))
