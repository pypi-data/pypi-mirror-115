...
@ray.remote(num_gpus=int(torch.cuda.is_available()))
def execute_objective(parallel_arguments, cv_samples, parameters, id_):#arguments):
    ...
    windowsOS = parallel_arguments["windowsOs"]
    ...
    for i, cv_sample in enumerate(cv_samples):
        ...
        if ODE_order:
            ...
            val_score, pred_, id_ = RC.test(**val_inputs, **parallel_arguments["test_args"])
    ...
    total_score = total_score / (len(cv_samples) * len(val_scores))
    if id_ == 0:
        return float(total_score), {"pred": pred_.to("cpu"), "val_y" : val_inputs["y"]}, id_
    else:
