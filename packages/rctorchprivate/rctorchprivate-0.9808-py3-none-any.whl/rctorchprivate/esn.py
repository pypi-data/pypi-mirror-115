...
def check_x(X, y, tensor_args = {}, supervised = False):
    ...
    try:
        if X is None:
            X = ones((y.shape[0],1) **tensor_args)
            #print("1a")
    except:
...
class EchoStateNetwork(nn.Module):
    ...
    def __init__(self, n_nodes = 1000, bias = 0, connectivity = 0.1, leaking_rate = 0.99, spectral_radius=0.9, #<-- important hyper-parameters
                 ...
                 enet_alpha = None, gamma_cyclic = None, beta = None): #<-- this line is backprop arguments
        super().__init__()
        ...
        self.LinOut = Linear(self.n_nodes + self.n_outputs, self.n_outputs)
    ...
    def train_state_vanilla(self, t, X, state, y, output = False, retain_grad = False):
        ...
        #preactivation = input_vector + recurrent_vec + self.bias * self.betas[t-1,:] 
        preactivation = self.preactivation(t, input_vector, recurrent_vec, self.bias, self.beta)
    ...
    def fit(self, y = None, X=None, burn_in=0, input_weight=None, verbose = False , 
            ...
            backprop_f = None, epochs = None, beta = None):
        # if verbose:
        #     if nl:
        #         print("nonlinear!")
        #     else:
        ...
        with no_grad():
            ...
            if self.ODE_order: 
                ...
                X = check_x(X, y, self.dev, supervised = True).to(self.device)
    ...
    def test(self, y, X=None, y_start=None, steps_ahead=None, scoring_method='nmse', 
        ...
        if not self.ODE_order:
            ...
            if y.device != self.device:
                y = y.to(self.device)
            final_t =y.shape[0]
            X = check_x(X , y, self.dev, supervised = True).detach().clone().to(self.device).requires_grad_(True)
