...
def execute_objective(arguments):
    ...
    # Take the parallelism away from the batches, and give it to the trust_regions.
    
    RC = EchoStateNetwork(**declaration_args, **parameters, id_ = id_)
    cv_samples_ = []
    for i, cv_sample in enumerate(cv_samples):
        cv_samples_.append((cv_sample["tr_x"],    cv_sample["tr_y"], 
                            cv_sample["val_x"],   cv_sample["val_y"],
                            cv_sample["tr_b"], cv_sample["val_b"]))
    ...
    for i, cv_sample in enumerate(cv_samples_):
        #print(i)
        train_x, train_y, validate_x, validate_y, tr_beta, val_beta = cv_sample

        cv_sample_score = 0

        fit_inputs_dict = {"X": train_x,
                   "y": train_y,
                   "beta": tr_beta}

        val_inputs_dict = {"X": validate_x,
                           "y": validate_y,
                           "beta": val_beta}
        ...
        if ODE_order:
            if multiple_ICs:
                ...
                #                 "ydots"  : ydots,
                #                 "losses" : Ls}
                dictt = RC.fit(**fit_inputs_dict, 
                                train_score = True, **train_args)
                train_scores = dictt["scores"]
                val_scores, pred_, id_ = RC.test(**val_inputs_dict, **test_args)
                ...
                results = RC.fit(**fit_inputs_dict, train_score = True, **train_args)
            ...
            _ = RC.fit(**fit_inputs_dict, **parallel_arguments["train_args"])
            #train_score = process_score(train_score)
            
            val_score, pred_, id_ = RC.test(**val_inputs_dict, **parallel_arguments["test_args"])
    ...
    #del cv_samples;
    nn = len(cv_samples_) * len(val_scores)
...
class EchoStateNetworkCV:
    ...
    def __init__(self, bounds, subsequence_prop,  model=EchoStateNetwork, initial_samples=50, #subsequence_length,
        ...
        self.subsequence_prop = subsequence_prop
    ...
    def _define_tr_val(self, inputs):
        ...
        train_x, validate_x = if_split(self.x, 
                                        start_index, 
                                        train_stop_index,
                                        validate_stop_index )

        train_beta, validate_beta = if_split(self.beta, 
                                             start_index, 
                                             train_stop_index, 
                                             validate_stop_index )
        beta_dict = {"tr_b" : train_beta, "val_b": validate_beta}


        xy_dict = {"tr_x" : train_x, "tr_y": train_y, "val_x": validate_x, "val_y" : validate_y }
        xy_dict = {**xy_dict, **beta_dict}
        return xy_dict
    ...
    def eval_objective(self, parameters, plot_type = "error", *args):
        ...
        if self.interactive : #and preds[0]["id_"] == 1
            self._train_plot_update(pred_ = preds[0]["pred"], validate_y = preds[0]["val_y"], 
    ...
    def optimize(self, n_trust_regions = 1, max_evals = None, y = None, x=None, store_path=None, 
        ...
        if n_trust_regions > 1:
            assert isinstance(max_evals, int), "you must enter n_outputs (int)"
    ...
    def _turbo_1(self):
        ...
        while not self.state.restart_triggered:  
            ...
            #can be parallelized:
            Y_next = self.eval_objective( (X_next, 0)) #tensor()#.unsqueeze(-1)
    ...
    def _turbo_initial_samples(self, X_init, dim, turbo_state_id):
        ...
        if len(X_init) > self.batch_size:
            ...
            for i in range(clean_batches):
                ...
                Y_batch = self.eval_objective( (X_batch, turbo_state_id ))
    ...
    def _turbo_m(self):
        ...
        while self.n_evals < self.max_evals: #not self.state.restart_triggered: 
            ...
            for i in range(self.n_trust_regions):
                ...
                #can be parallelized:
                Y_next = self.eval_objective( (X_next, i)) #tensor()#.unsqueeze(-1)
                                        #[self.eval_objective(x.view(1,-1)) for x in X_next],
                print('Trust region', i ,': Y_next', Y_next)
                #print("self.state", self.state)
