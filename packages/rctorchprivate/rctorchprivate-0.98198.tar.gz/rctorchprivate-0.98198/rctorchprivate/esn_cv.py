...
def execute_objective(parallel_arguments, parameters, id_):#arguments):
    ...
    if id_ == 0:
        if ode:
            ...
            return float(total_score), {"pred": results["ys"][best_idx].to("cpu"), "val_y" : results["ydots"][best_idx]}, id_
        else:
            return float(total_score), {"pred": pred_, "val_y" : val_input["y"]}, id_
...
def eval_objective_remote(parallel_args_id, parameters, dtype = None, device = None, plot_type = "error",  *args):
        ...
        pred, y = preds[0]["pred"], preds[0]["val_y"]
        ...
        return -Scores_, { "pred" : pred, "y": y, "trust_region_id": trust_region_id}
...
class EchoStateNetworkCV:
    ...
    def updates(self, scores, trust_region_id, pred, y):
        ...
        if self.interactive:
            self._train_plot_update(pred_ = pred, validate_y = y, 
                steps_displayed = pred.shape[0])
    ...
    def get_cv_samples(self):
        ...
        #ray.put(self.parallel_arguments["fit_inputs"])
        #ray.put(self.parallel_arguments["val_inputs"])
