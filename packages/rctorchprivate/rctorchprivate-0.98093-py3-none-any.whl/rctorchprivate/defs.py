...
def multi_attractor_loss(X , y, ydot, out_weights, force_t = None, 
    ...
    ydot, pdot = p, -torch.sin(y) - (1/5)*torch.sin(-5*y)

    #ydot, pdot = ydot[:,0].view(-1,1), ydot[:,1].view(-1,1)
    
    #with paramization
    L =  (ydot - p)**2 + (pdot + y   - force_t)**2 #+ lam * y**3 
    ...
    y0, p0 = init_conds
    ham = hamiltonian2(y, p)
    ham0 = 1.2 + (1/2)*p0**2 -np.cos(y0)-(1/5)*np.cos(5*y0) #ham0 = hamiltonian2(y0, p0)
    L_H = (( ham - ham0).pow(2)).mean()
    assert L_H >0
