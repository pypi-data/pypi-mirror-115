...
def upgrade_step_to_current(step):
    """
    ...
    if step_type in STEP_UPGRADES_FUNCTION_MAPPING:
        step_upgrades = STEP_UPGRADES_FUNCTION_MAPPING[step_type]
        if step_version in step_upgrades:
            return upgrade_step_to_current(step_upgrades[step_version](step))
...
def upgrade_steps_for_old_format(saved_analysis):
    ...
    new_steps = []
    for step_idx, step in steps.items():
        new_steps.append(upgrade_step_to_current(step))
...
def upgrade_saved_analysis_to_current_version(saved_analysis):
    ...
    # NOTE: in the future, when upgrading steps, put the upgrading here!

    return new_format_saved_analysis
