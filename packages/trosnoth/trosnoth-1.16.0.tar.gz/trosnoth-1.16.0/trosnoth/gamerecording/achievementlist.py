...
from trosnoth.const import (
    SHOT_HIT, GRENADE_HIT, SHOXWAVE_HIT, MINIMAP_FULLY_DISRUPTED,
...
class QuickKillSmall(QuickKillBase, NoLimit):
    ...
    name = 'Double Kill'
    description = 'Kill two enemies within five seconds (no grenades or mines)'
...
class QuickKillMedium(QuickKillBase, NoLimit):
    ...
    name = 'Triple Kill'
    description = 'Kill three enemies within five seconds (no grenades or mines)'
...
class MinimapDisruptionTag(OncePerPlayerPerGame):
    ...
    def taggedZone(self, zone, previous_owner):
        if previous_owner and previous_owner.get_minimap_status() == MINIMAP_FULLY_DISRUPTED:
            self.achievementTriggered()
...
class ChokepointSpam(Streak, OncePerPlayerPerGame):
    ...
    description = 'Kill three players without moving or dying (no items)'
    streakTarget = 3
...
class MutualKill(NoLimit):
    ...
    def died(self, killer, *args, **kwargs):
        if not killer or killer == self.player:
            return
