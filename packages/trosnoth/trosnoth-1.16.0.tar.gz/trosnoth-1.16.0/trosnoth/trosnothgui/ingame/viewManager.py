...
from trosnoth.trosnothgui.ingame.sprites import PlayerSprite, CrosshairDrawer
...
class ViewManager(framework.Element):
    ...
    def __init__(self, app, parent, universe, target=None, replay=False):
        ...
        self.outer_crosshair = CrosshairDrawer(universe, app.theme, universe.getTime)
        self.inner_crosshair = CrosshairDrawer(
            universe, app.theme, universe.getTime, frame_offset=16)
        self.sRect = None
    ...
    def add_sprites_for_player(self, player, visible_players, extras):
        ...
        if player.is_juggernaut():
            self.outer_crosshair.update(player, focus, area)
            self.inner_crosshair.update(player, focus, area)
            extras.add(self.outer_crosshair)
            extras.add(self.inner_crosshair)
