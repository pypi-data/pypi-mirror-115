...
def red(code: str) -> tuple:
    ...
    red_price = eval_red(code=code)['red_price']
...
def blue(code: str) -> tuple:
    ...
    def 이자보상배율평가(이자보상배율: tuple) -> int:
        ...
        if math.isnan(최근분기) or 최근분기 <= 1:
            ...
            c104.chg_page(page='c104y')
            전년대비 = c104.find_cmp(title='이자보상배율')
    ...
    def 순부채비율평가(순부채비율: tuple) -> int:
        ...
        if math.isnan(최근분기) or 최근분기 >= 80:
            ...
            _, 최근연도 = corps_eval.get_recent(dict_y)
            c104.chg_page(page='c104y')
            전년대비 = c104.find_cmp(title='순부채비율')
...
def growth(code: str) -> tuple:
    ...
    grwoth_dict = eval_growth(code)

    logger.debug(pprint.pformat(grwoth_dict, width=200))
    ...
    def 매출액증가율평가(매출액증가율: tuple) -> int:
        ...
        if math.isnan(최근연도):
            ...
            if 최근연도 < 최근분기:
                ...
                p1 += MAX / 10
            # 나머지는 성장률 기반 점수 배정
            p1 += MAX / 2 if 최근분기 > MAX else 최근분기 / 2
        elif 최근연도 <= 0 < 최근분기:
        ...
        if math.isnan(평균매출액증가율):
            ...
            p2 += -(MAX / 2) if 평균매출액증가율 < -MAX else 평균매출액증가율 / 2
        else:
            p2 = MAX / 2 if 평균매출액증가율 > MAX else 평균매출액증가율 / 2

        logger.info(f'매출액증가율평가 point : {p1 + p2}')

        return int(p1 + p2)

    p1 = 매출액증가율평가(grwoth_dict['매출액증가율'])
