...
from tunnelclient.tests import fakes
from tunnelclient.tests.unit.osc.v2 import constants


class FakeTunnelClient(object):
    def __init__(self, **kwargs):
        self.vpn_servers = mock.Mock()
        self.vpn_servers.resource_class = fakes.FakeResource(None, {})
        self.auth_token = kwargs['token']
        self.management_url = kwargs['endpoint']


class TestTunnelClient(utils.TestCommand):

    def setUp(self):
        super().setUp()
        self.app.client_manager.vpn_server = FakeTunnelClient(
