...
class ListAmphora(lister.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        data = self.app.client_manager.vpn_server.amphora_list(**attrs)
...
class ShowAmphora(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        data = self.app.client_manager.vpn_server.amphora_show(
            amphora_id=parsed_args.amphora_id,
        )

        rows = const.AMPHORA_ROWS
        formatters = {
            'vpnservers': v2_utils.format_list,
            'amphorae': v2_utils.format_list,
        }
...
class ConfigureAmphora(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.amphora_configure(
            amphora_id=amp_id)

        if parsed_args.wait:
            amphora = self.app.client_manager.vpn_server.amphora_show(
                amp_id)
            lb_id = amphora.get('vpnserver_id')
...
class FailoverAmphora(command.Command):
    ...
    def take_action(self, parsed_args):
        attrs = v2_utils.get_amphora_attrs(self.app.client_manager,
                                           parsed_args)
        amp_id = attrs.pop('amphora_id')
        amphora = self.app.client_manager.vpn_server.amphora_show(amp_id)
        self.app.client_manager.vpn_server.amphora_failover(
            amphora_id=amp_id)
...
class ShowAmphoraStats(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        data = self.app.client_manager.vpn_server.amphora_stats_show(
            amphora_id=parsed_args.amphora_id
        )
...
class DeleteAmphora(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.amphora_delete(
            amphora_id=parsed_args.amphora_id)

        if parsed_args.wait:
            v2_utils.wait_for_delete(
                status_f=self.app.client_manager.vpn_server.amphora_show,
