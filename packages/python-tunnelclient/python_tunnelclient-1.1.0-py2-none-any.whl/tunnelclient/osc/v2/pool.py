...
class CreatePool(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parent_group.add_argument(
            '--vpnserver',
            metavar='<vpn_server>',
            help="Vpn server to add the pool to (name or ID)"
        )
        parser.add_argument(
            '--session-persistence',
        ...
        parser.add_argument(
            ...
            type=lambda s: s.upper(),  # case insensitive
            help="Vpn servering algorithm to use."
        )
    ...
    def take_action(self, parsed_args):
        ...
        body = {"pool": attrs}
        data = self.app.client_manager.vpn_server.pool_create(
            json=body)
        ...
        if parsed_args.wait:
            v2_utils.wait_for_active(
                status_f=(self.app.client_manager.vpn_server.
                          vpn_server_show),
                res_id=data['pool']['vpnservers'][0]['id']
            )
            data = {
                'pool': (
                    self.app.client_manager.vpn_server.pool_show(
                        data['pool']['id']))
            }
...
class DeletePool(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.pool_delete(
            pool_id=pool_id)

        if parsed_args.wait:
            v2_utils.wait_for_delete(
                status_f=self.app.client_manager.vpn_server.pool_show,
                res_id=pool_id
            )
...
class ListPool(lister.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        attrs = v2_utils.get_pool_attrs(self.app.client_manager, parsed_args)
        data = self.app.client_manager.vpn_server.pool_list(**attrs)
        formatters = {'vpnservers': v2_utils.format_list,
                      'members': v2_utils.format_list,
                      'listeners': v2_utils.format_list}
...
class ShowPool(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        if uuidutils.is_uuid_like(parsed_args.pool):
            try:
                data = self.app.client_manager.vpn_server.pool_show(
...
class SetPool(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            choices=ALGORITHM_CHOICES,
            type=lambda s: s.upper(),  # case insensitive
            help="Set the vpn servering algorithm to use."
        )
        admin_group = parser.add_mutually_exclusive_group()
        admin_group.add_argument(
            '--enable',
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.pool_set(
            pool_id, json=body)
...
class UnsetPool(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        pool_id = v2_utils.get_resource_id(
            self.app.client_manager.vpn_server.pool_list,
