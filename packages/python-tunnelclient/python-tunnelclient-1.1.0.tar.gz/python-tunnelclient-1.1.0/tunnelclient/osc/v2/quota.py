...
class ListQuota(lister.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        attrs = v2_utils.get_listener_attrs(self.app.client_manager,
                                            parsed_args)
        data = self.app.client_manager.vpn_server.quota_list(**attrs)
...
class ShowQuota(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        if uuidutils.is_uuid_like(parsed_args.project):
            try:
                data = self.app.client_manager.vpn_server.quota_show(
...
class ShowQuotaDefaults(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        rows = const.QUOTA_ROWS
        data = self.app.client_manager.vpn_server.quota_defaults_show()
...
class SetQuota(command.ShowOne):
    ...
    def _check_attrs(attrs):
        args = ['health_monitor', 'listener', 'vpn_server', 'member',
    ...
    def get_parser(self, prog_name):
        ...
        quota_group.add_argument(
            ...
            dest='vpn_server',
            metavar='<vpn_server>',
            help=('New value for the vpn server quota limit. Value -1 '
                  'means unlimited.')
        )
        quota_group.add_argument(
            '--member',
    ...
    def take_action(self, parsed_args):
        ...
        data = self.app.client_manager.vpn_server.quota_set(project_id,
                                                               json=body)
...
class ResetQuota(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.quota_reset(
            project_id=project_id)
...
class UnsetQuota(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--vpnserver',
            action='store_true',
            help="Reset the vpn server quota to the default."
        )
        parser.add_argument(
            '--listener',
            action='store_true',
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.vpn_server.quota_set(
