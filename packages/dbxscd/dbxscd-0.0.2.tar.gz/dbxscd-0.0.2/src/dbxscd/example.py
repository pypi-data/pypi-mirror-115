...
SCD1(spark, df, target_table_name_scd1, target_partition_keys, key_cols, current_time)
display(spark.sql(f"select * from {target_table_name_scd1}"))

# COMMAND ----------

target_table_name_scd2 = 'default.table_scd2'
SCD2(spark, df, target_table_name_scd2, target_partition_keys, key_cols, current_time)
display(spark.sql(f"select * from {target_table_name_scd2}"))

# COMMAND ----------

target_table_name_scd3 = 'default.table_scd3'
SCD3(spark, df, target_table_name_scd3, target_partition_keys, key_cols, current_time)
