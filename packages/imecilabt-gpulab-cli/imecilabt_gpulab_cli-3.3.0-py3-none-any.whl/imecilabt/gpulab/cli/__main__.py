...
import requests
import yaml
from build.lib.imecilabt.gpulab.cli.__main__ import _command_array_to_str
...
def localhost_mode_cli(ctx):
    """GPULab client version 3.3.0 (in LOCALHOST_MODE!)
    ...
    if r.ok:
        ...
        if show_ansible:
            ...
            if sftp_command == 'gftp':
                ...
                if show:
                    click.echo('Short: '+_command_array_to_str(command_short))
                    click.echo('Full:  '+_command_array_to_str(command_full))
                else:
                    # Note: This REPLACES the current python process!
