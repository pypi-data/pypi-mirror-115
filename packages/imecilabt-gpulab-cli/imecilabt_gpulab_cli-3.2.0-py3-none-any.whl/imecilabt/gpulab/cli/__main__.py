...
def _command_array_to_str(command: List[str]) -> str:
    return ' '.join([_sh_quote_arg(c) for c in command])


def _add_gpulab_ssh_config_hack(
        ctx,
        target_hostname: str, target_identity: str,
        proxy_hostname: Optional[str], proxy_port: Optional[int],
        proxy_username: Optional[str], proxy_identity: Optional[str],
) -> str:
    """
    Adds a new ~/.ssh/config Host that can be used to force the SSH proxy easily.
    :param target_hostname:
    :param proxy_hostname:
    :return: the fake hostname
    """
    # we need to be sure the correct identity is used
    target_identity_id = _get_short_file_short_hash_id(target_identity)
    proxy_identity_id = _get_short_file_short_hash_id(proxy_identity) if proxy_identity else None

    if proxy_hostname:
        assert proxy_port
        assert proxy_username
        assert proxy_identity
        id = f'{proxy_username}.{proxy_identity_id}.{proxy_hostname}.{target_identity_id}'
        fake_hostname = f'gpulabproxyhack.{_get_str_short_hash_id(id)}.{target_hostname}'
    else:
        fake_hostname = f'gpulabnoproxyhack.{target_identity_id}.{target_hostname}'
    ...
    if os.path.exists(ssh_config_filename):
        with open(ssh_config_filename, 'r') as f:
            for line in f:
                if line.startswith(f'Host={fake_hostname}'):
                    return fake_hostname

    created = os.path.exists(ssh_config_filename)
    with open(ssh_config_filename, 'a') as f:
        f.write(f'\n')
        f.write(f'# This entry is added by GPULab. Feel free to delete it. GPULab will rewrite it if needed.\n')
        f.write(f'Host={fake_hostname}\n')
        f.write(f'   Hostname {target_hostname}\n')
        if proxy_hostname:
            f.write(f'   ProxyCommand ssh -i "{proxy_identity}" -oPort={proxy_port} "{proxy_username}@{proxy_hostname}" -W %h:%p\n')
        f.write(f'   IdentityFile "{target_identity}"\n')
        f.write(f'   IdentitiesOnly yes\n')
        f.write(f'   PreferredAuthentications publickey\n')
        f.write(f'\n')
    if created:
        os.chmod(ssh_config_filename, 0o700)
    if ctx.obj[VERBOSE_DEBUG]:
        click.secho(f'Appended Host={fake_hostname} to {ssh_config_filename}')
    return fake_hostname
...
def localhost_mode_cli(ctx):
    """GPULab client version 3.2.0 (in LOCALHOST_MODE!)
...
@click.option('--auto-sftp-command', 'sftp_command', flag_value='auto', default=True,
              help="Use sftp command: automatic (Default)")
@click.pass_context
@require_ctx_loggedin_api_client
def sftp(ctx, job_id, show, proxy, sftp_command):
    if sftp_command == 'auto' and _has_gftp():
        sftp_command = 'gftp'
    else:
        sftp_command = 'sftp'
...
def actual_ssh(ctx, job_id, show, show_ansible, proxy, *, sftp_command=None):
    ...
    assert sftp_command is None or sftp_command in ('sftp', 'gftp')
    ...
    if r.ok:
        ...
        if show_ansible:
            ...
            if sftp_command == 'gftp':
                if proxy == 'yes':
                    fake_hostname = _add_gpulab_ssh_config_hack(
                        ctx,
                        target_hostname=job.state.resources.ssh_host,
                        target_identity=ctx.obj[CERTFILE],
                        proxy_hostname=job.state.resources.ssh_proxy_host,
                        proxy_port=job.state.resources.ssh_proxy_port,
                        proxy_username=job.state.resources.ssh_proxy_username,
                        proxy_identity=ctx.obj[CERTFILE],
                    )
                else:
                    fake_hostname = _add_gpulab_ssh_config_hack(
                        ctx,
                ...
                if ctx.obj[VERBOSE_DEBUG]:
                    click.secho(f'Configured fake hostname for use with gftp: '
                ...
                command = [
                    ...
                    'ssh2://{}@{}:{}'.format(
                        ...
                        fake_hostname,  # instead of: job.state.resources.ssh_host,
                ...
                if show:
                    click.echo(_command_array_to_str(command))
                else:
                    if ctx.obj[VERBOSE_DEBUG]:
                ...
                c = 'ssh' if sftp_command is None else 'sftp'
                command = [
                    c,
                    '-i', ctx.obj[CERTFILE],
                ...
                if proxy == 'yes':
                    ...
                    command.append('-oProxyCommand=ssh -i {} {} {}@{} -W %h:%p'
                                   .format(_sh_quote_arg(ctx.obj[CERTFILE]), _ssh_port_command(proxy_port), proxy_username,
                                           proxy_host))
