...
# Users should not be dealing with this
from phantom.app import APP_SUCCESS

from phantom_dev.action_handler import ActionHandler, SmartConnector, contains
    ...
    def on_poll(
        ...
        for container_index in range(container_count):
            ...
            for artifact_index in range(artifact_count):
                ...
                dummy_artifacts.append(artifact)

            container = {
                'name': f'dummy container {container_index}',
                'artifacts': dummy_artifacts,
            }

            dummy_containers.append(container)

        # Needing to use APP_SUCCESS directly runs against the design goal of
        # phantom-dev.
        # Unfortunately, the planned object-oriented approach to container
        # management for now precludes wrapping this in a helper method in
        # order to avoid further cluttering the connector interface.
        # This at least presents an example of how to deal with many of the
        # base API methods should their use be necessary.
        status, message, responses = self.save_containers(dummy_containers)
        if status != APP_SUCCESS:
            raise RuntimeError(message)

        self.logger.info('Saved new containers with responses %s', responses)
        return responses
