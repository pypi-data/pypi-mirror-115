...
def start(update, context):
    start_string = f'''
a telegram bot that mirrors magnet/torrent/ddl links from the internet.
'''
    buttons = button_build.ButtonMaker()
    buttons.buildbutton("Owner", "https://t.me/orsixtyone")
    buttons.buildbutton("Channel", "https://t.me/torrentmirrorer")
    buttons.buildbutton("Bot Commands", "https://telegra.ph/Commands-for-ChizuBot-05-03")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    LOGGER.info('UID: {} - UN: {} - MSG: {}'.format(update.message.chat.id, update.message.chat.username, update.message.text))
    uptime = get_readable_time((time.time() - botStartTime))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        if update.message.chat.type == "private" :
            sendMessage(f"Hey, I've been alive for <code>{uptime}</code>.", context.bot, update)
        else :
            sendMarkup(start_string, context.bot, update, reply_markup)
    else :
        sendMarkup(f"Oops! not a Authorized user.\nYou have to use this bot in @ChizuDrive, not in PMS", context.bot, update, reply_markup)
...
'''
    ...
    help_string = f'''
Click the button below to see the full list of available commands.
'''
    buttons = button_build.ButtonMaker()
    buttons.buildbutton("Commands", "https://telegra.ph/Commands-for-ChizuBot-05-03")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
...
botcmds = [
        ...
        (f'{BotCommands.ExecHelpCommand}','Run Shell CMDS [owner only]]'),
