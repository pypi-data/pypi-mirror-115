...
class AriaDownloadHelper(DownloadHelper):
    ...
    def add_download(self, link: str, path, listener, filename, uid, cid, hlink):
        if is_magnet(link):
        ...
        with download_dict_lock:
            ...
            LOGGER.info(f"#SADL Started aria2 DL. GID: {download.gid} DIR: {download.dir} ")
            LOGGER.info(f'UID: {uid} - CID {cid} - Link (if available): {hlink}')
