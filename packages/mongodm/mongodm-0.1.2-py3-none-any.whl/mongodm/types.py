...
from .utils import Q, processing_kwargs
...
class Document(BaseModel):
    ...
    @classmethod
    def update_one(cls, query: Q, *args: Any, **kwargs: Any) -> UpdateResult:
        ...
        """
        return cls.collection.update_one(query, {"$set": kwargs})

    @classmethod
    def update_many(cls, query: Q, *args: Any, **kwargs: Any) -> UpdateResult:
        ...
        Returns:
            UpdateResult: Update result from Pymongo.
        """
        return cls.collection.update_many(query, {"$set": kwargs})

    def update(self, *args: Any, **kwargs: Any) -> UpdateResult:
        """Update instance.
        ...
        if ObjectId.is_valid(self.id):
            return self.collection.update_one({"_id": self.id}, kwargs)
        lookup_field = self.Meta.lookup_field
        lookup_value = getattr(self, lookup_field)
        return self.collection.update_one({lookup_field: lookup_value}, kwargs)
