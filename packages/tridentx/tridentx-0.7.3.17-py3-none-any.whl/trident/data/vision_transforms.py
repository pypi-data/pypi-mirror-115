...
class ImageMosaic(VisionTransform):
    ...
    def _apply_coords(self, coords, spec: TensorSpec):
        ...
        height,width=self.output_size
        box0 =coords.copy()
        box0[:, 0] = np.clip(box0[ :, 0], 0, center[1])
        box0[:, 1] = np.clip(box0[:, 1], 0, center[0])

        idxs = np.array([(0, 1), (2, 1), (0, 3), (2, 3)]).flatten()


        box1 = self.memory_cache[other_idxes[0]][spec].copy()[:,:4]
        box1 = np.asarray(box1).reshape(-1, 4)[:, idxs].reshape(-1, 2)
            #.reshape(coords.shape[0],-1,2)
        box1[:, 0] = np.clip(box1[:, 0], img1_offsetx,img1_offsetx+width-center[1])
        box1[:, 1] = np.clip(box1[:, 1], img1_offsety,img1_offsety+center[0])


        box2 = self.memory_cache[other_idxes[1]][spec].copy()[:,:4]
        box2= np.asarray(box2).reshape(-1, 4)[:, idxs].reshape(-1, 2)
        box2[:, 0] = np.clip(box2[:, 0], img2_offsetx,img2_offsetx+center[1])
        box2[:, 1] = np.clip(box2[:, 1], img2_offsety,img2_offsety+height-center[0])


        box3 = self.memory_cache[other_idxes[2]][spec].copy()[:,:4]
        box3 = np.asarray(box3).reshape(-1, 4)[:, idxs].reshape(-1, 2)
        box3[:, 0] = np.clip(box3[ :, 0], img3_offsetx,img3_offsetx+width-center[1])
        box3[ :, 1] = np.clip(box3[:, 1], img3_offsety,img3_offsety+height-center[0])

        box_list=np.concatenate([box0,box1,box2,box3],axis=0)
        box_list=box_list.reshape(-1,4)
        hw = np.clip(box_list[..., 2:] - box_list[..., :2], 0.0, None)
        area=hw[..., 0] * hw[..., 1]
        mask_box=area>0
        mask_box=box_list[mask_box,:]
        return mask_box
