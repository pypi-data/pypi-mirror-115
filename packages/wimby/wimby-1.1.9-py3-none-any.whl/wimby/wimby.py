    ...
    def dump(self, yesterday={}):
        if self.movers.count() > 0:
            ...
            if isinstance(yesterday, dict):
                for rank, tokens in yesterday.items():
                    for token in tokens:
                        comparison.update({token: rank})
            ...
            for rank in ranking:
                ...
                for token in tokens:
                    previous = float(comparison.get(token, 0))
                    change = 0
                    if previous > 0:
                        change = ((rank - previous) / previous) * 100
                    ...
                    buy_score = self.momentum.get(token, {}).get("buy", 0) * 100
                    sell_score = self.momentum.get(token, {}).get("sell", 0) * 100
                    hold_score = max((buy_score, sell_score))
                    if pattern.strip() != "":
                        print(f"    {pattern}")
                    if change == 0:
                        print(f"    Movement: {fmovement}")
                    else:
                        print(f"    Movement: {fmovement} ({change:,.2f}%)")
                    if not max((hold_score, buy_score, sell_score)) == 0:
                        if hold_score <= 60:
