...
class ProwlerAudit(
    ...
    def __init__(
        ...
        *,
        ...
        :param prowler_options: Options to pass to Prowler command, make sure at least -M junit-xml is used for CodeBuild reports. Use -r for the region to send API queries, -f to filter only one region, -M output formats, -c for comma separated checks, for all checks do not use -c or -g, for more options see -h. For a complete assessment use "-M text,junit-xml,html,csv,json", for SecurityHub integration use "-r region -f region -M text,junit-xml,html,csv,json,json-asff -S -q" Default: '-M text,junit-xml,html,csv,json,json-asff -S -q'
...
class ProwlerAuditProps:
    ...
    def prowler_options(self) -> builtins.str:
        ...
        assert result is not None, "Required property 'prowler_options' is missing"
