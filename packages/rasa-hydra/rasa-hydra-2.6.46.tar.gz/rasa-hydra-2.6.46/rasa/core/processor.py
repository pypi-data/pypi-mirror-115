        ...
        if message_type == "SPOKEN" and len(partials) > 0 and (
            ...
            for partial in partials:
                ...
                if parse_data["intent"]["name"] and parse_data["intent"]["confidence"] > confidence_threshold and (
                    ...
                    red_parse_data_partials = redactor.redact_event_complete(parse_data)
    ...
    def convert_messge_to_digits(self, message):
        ...
        if '/greet' not in pci_msg and '/disconnect' not in pci_msg and '/restart' not in pci_msg:
            if len(pci_msg) > 0 and pci_msg[-1] in string.punctuation and (pci_msg[-1] not in ['#', '*']):
                pci_msg = pci_msg[:-1]
            pci_msg = pci_msg.replace(',', ' ').replace('for ', 'four ').replace('to ', 'two ').replace('too ', 'two ').replace('won ', 'one ').replace('ate ', 'eight ').replace('hate ', 'eight ')
