...
def get_data_array(csv_data_input, t_data_input, y_data_input, series_name_input,
                   filter_input=False, filter_by_input=None):
    if filter_input:
        filter_name, filter_value = str(filter_by_input).split("=")
        filter_value = int(filter_value)
        if str(series_name_input).lower() == "displacement":
            data_array_output = []
            row_count = 0
                ...
                for row in csv_reader:
                    if row_count == 0:
                        ...
                        if filter_check == filter_value:
                            y_position = float(row[y_pos])
                            record_time = float(row[t_pos])
                            input_time = record_time
                            data_array_output.append([input_time, y_position])

            return data_array_output

        elif str(series_name_input).lower() == "velocity":
            data_array_output = []
            row_count = 0
            with open(csv_data_input, "r") as csv_file:
                csv_reader = (csv.reader(csv_file, delimiter=','))
            ...
            with open(csv_data_input, "r") as csv_file:
                ...
                for row in csv_reader:
                    if row_count == 0:
                        ...
                        y_position = float(row[y_pos])
                        record_time = float(row[t_pos])
                        input_time = record_time - initial_time
                        data_array_output.append([input_time, y_position])
            ...
            with open(csv_data_input, "r") as csv_file:
                ...
                for row in csv_reader:
                    if row_count == 0:
                        ...
                        y_position = int(float(row[y_pos]))
                        record_time = float(row[t_pos])
                        input_time = 0
                        initial_time = record_time
                        initial_value = y_position
                        y_diff = 0
                        data_array_output.append([input_time, y_diff])
                        row_count += 1
                    else:
                        y_position = int(float(row[y_pos]))
                        record_time = float(row[t_pos])
                        input_time = record_time - initial_time
                        y_diff = initial_value - y_position
                        initial_value = y_position
                        data_array_output.append([input_time, y_diff])
                        row_count += 1
...
def main():
    ...
    if draw_able and data:
        ...
        draw_data_array = []
        max_video_length = frame_count_to_time(frame_count, frame_rate)
        ...
        for info in series_array:
            ...
            if parent_index is not None:
                ...
                if str(display_type).lower() == "pen":
                    if info_filter:
                        pen_data_arr = get_data_array(csv_data, t_data, y_data, series_name, info_filter, filter_by)
                    else:
                        pen_data_arr = get_data_array(csv_data, t_data, y_data, series_name)
                    series_data_array = []
                    drawing_speed = 0.1
                    number_of_display = int(max_video_length / drawing_speed)
                    lower_range = 0
                    upper_range = lower_range + drawing_speed
                    # max_data_per_draw_speed = int((axes_pos_w / t_w) * drawing_speed)
                    max_width_to_draw = int(axes_pos_w)
                    small_data_array = []
                    # last_value_for_dis_arr = 0
                    for i in range(1, (number_of_display + 1), 1):
                        raw_small_data_array = []
                        for data in pen_data_arr:
                            if lower_range <= data[0] < upper_range:
                                raw_small_data_array.append(data)
                        small_data_array.extend(raw_small_data_array)
                        another_small_data_array = small_data_array[:]
                        lower_range = upper_range
                        upper_range += drawing_speed
                        series_data_array.append([lower_range, another_small_data_array])
                        if len(small_data_array) >= max_width_to_draw:
                            small_data_array.clear()
                    draw_data_array.append([series_name, display_type, series_data_array])

                elif str(display_type).lower() == "refresh":
                    if info_filter:
                        re_data_arr = get_data_array(csv_data, t_data, y_data, series_name, info_filter, filter_by)
                    else:
                        re_data_arr = get_data_array(csv_data, t_data, y_data, series_name)
                    re_series_data_array = []
                    collect_speed = 0.1
                    max_width_to_draw = int(axes_pos_w)
                    # max_data_per_draw_speed = int((axes_pos_w / t_w) * collect_speed)
                    number_of_display = math.ceil(max_video_length / collect_speed)
                    re_small_data_array = []
                    re_lower_range = 0
                    re_upper_range = re_lower_range + collect_speed
                    for i in range(1, (number_of_display + 1), 1):
                        raw_re_small_data_array = []
                        for data in re_data_arr:
                            if re_lower_range <= data[0] < re_upper_range:
                                raw_re_small_data_array.append(data)
                        small_tem_arr = raw_re_small_data_array[:]
                        raw_re_small_data_array.clear()
                        re_small_data_array.extend(small_tem_arr)
                        re_lower_range = re_upper_range
                        re_upper_range += collect_speed
                        # print(re_lower_range)
                        # print(re_upper_range)
                        t_w_count = 1
                        if (re_lower_range == (t_w * t_w_count)) or i == number_of_display:
                            another_array = re_small_data_array[:]
                            # print(another_array)
                            re_series_data_array.append([re_lower_range, another_array])
                            t_w_count += 1
                        if len(re_small_data_array) >= max_width_to_draw:
                            re_small_data_array.clear()
                    # print(re_series_data_array)
                    draw_data_array.append([series_name, display_type, re_series_data_array])
            else:
                # draw_able = False
                pass
        ...
        while True:
            ...
            if ret:
                ...
                for i, info in enumerate(series_array):
                    ...
                    display_data_array = []
                    for inf in display_arr:
                        if inf[0] >= check_timer:
                            display_data_array = inf[1]
                            break
                    ...
                    axes_info = axes_array[parent_index]
                    if info_name == "displacement" and parent_index is not None:
                        ...
                        time_in_pixel_value = round((t_width / ax_w), 2)
                        # start_x = ax_x
                        # start_y = ax_y + int(ax_h / 2)
                        start_x = ax_x
                        start_y = ax_y + int(ax_h / 2)
                        # first_step = 0
                        value_to_draw = False
                            ...
                            for ind, number in enumerate(display_data_array):
                                ...
                                if math.isnan(number[1]):
                                    ...
                                    if not value_to_draw:
                                        scaled_num = number_to_scale(lower_limit, upper_limit, ax_h, number[1])
                                        scaled_step = int(number[0] / time_in_pixel_value)
                                        start_x = ax_x + scaled_step
                                        start_y = int(ax_y + ax_h - scaled_num)
                                        value_to_draw = True
                                    else:
                                        scaled_num = number_to_scale(lower_limit, upper_limit, ax_h, number[1])
                                        scaled_step = int(number[0] / time_in_pixel_value)
                                        end_x = ax_x + scaled_step
                                        end_y = int(ax_y + ax_h - scaled_num)
                                        # first_step += scaled_step
                                        cv2.line(clone_frame, (start_x, start_y), (end_x, end_y),
                                                 line_color_array[i], line_thickness, cv2.LINE_AA)
                                        if need_to_draw_pointer and ind == len_array - 1:
                                            cv2.circle(clone_frame, (end_x, end_y), pointer_radius, pointer_color, -1)
                                        start_x = end_x
                                        start_y = end_y

                    if info_name == "velocity" and parent_index is not None:
                        ax_pos = axes_info["position"]
                        ax_x = ax_pos["x"]
                        ax_y = ax_pos["y"]
                        ax_w = ax_pos["width"]
                        ...
                        zero_line_start_x = ax_x
                        zero_line_y_scale = int(number_to_scale(lower_limit, upper_limit, ax_h, 0))
                        zero_line_y = int(ax_y + ax_h - zero_line_y_scale) + 1
                        zero_line_end_x = ax_x + ax_w
                        time_in_pixel_value = round((t_width / ax_w), 2)
                        start_x = ax_x
                        start_y = ax_y + int(ax_h / 2)
                        ...
                        if len(display_data_array) > 0:
                            for ind, number in enumerate(display_data_array):
                                scale_for_check_timer = int(ax_w / t_width)
                                # check_timer_for_pointer = 0
                                # if check_timer <= t_width:
                                #     check_timer_for_pointer = check_timer * scale_for_check_timer
                                # if check_timer > t_width:
                                #     reminder_check_timer = check_timer % t_width
                                #     check_timer_for_pointer = reminder_check_timer * scale_for_check_timer
                                if zero_display:
                                    cv2.line(clone_frame, (zero_line_start_x, zero_line_y),
                                             (zero_line_end_x, zero_line_y),
                                             line_color_array[i], zero_line_thick)
                                if math.isnan(number[1]):
                                    ...
                                    if not value_to_draw:
                                        ...
                                        cv2.line(clone_frame, (start_x, start_y), (end_x, end_y),
                                                 line_color_array[i], line_thickness, cv2.LINE_AA)
                                        if need_to_draw_pointer and round(number[0], 2) == round(check_timer, 2):
                                            cv2.circle(clone_frame, (end_x, end_y), pointer_radius, pointer_color,
                                                       -1)
                                        start_x = end_x
                                        start_y = end_y
                ...
                print("\r")
                print(f"The whole process took {(end_whole_t - whole_t)} seconds.")
