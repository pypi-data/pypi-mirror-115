...
class StepBatchTest(transformations_testlib.TransformationsTest):
  ...
  def test_batch_episode_consistent_with_batch_steps(self):
    steps_dataset = tf.data.Dataset.from_tensor_slices(self.steps)
    episode = rlds_types.build_episode(steps=steps_dataset, metadata={})

    batched_steps = flexible_batch.batch(steps_dataset, size=2)
    batched_episode = flexible_batch.batch_episode(episode, size=2)

    self.expect_equal_datasets(batched_steps, batched_episode[rlds_types.STEPS])

  def test_unbatch_episode(self):
    steps_dataset = tf.data.Dataset.from_tensor_slices(self.steps)
    episode = rlds_types.build_episode(steps=steps_dataset, metadata={})
    batched_episode = flexible_batch.batch_episode(episode, size=2)
    unbatched_episode = flexible_batch.unbatch_episode(batched_episode)

    self.expect_equal_datasets(unbatched_episode[rlds_types.STEPS],
