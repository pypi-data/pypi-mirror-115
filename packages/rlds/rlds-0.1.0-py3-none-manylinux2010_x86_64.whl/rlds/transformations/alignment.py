...
def shift_keys(steps: tf.data.Dataset,
               keys: List[str],
               shift: int,
               batch_size: int = flexible_batch.DEFAULT_BATCH,
               enable_batch_optimization: bool = True) -> tf.data.Dataset:
  ...
  Args:
    ...
    batch_size: batch size to use when applying the transformation. It should be
      larger than `shift`.
    enable_batch_optimization: if True, it assumes that the full episode fits in
      memory (and batch_size >= episode size).
  ...
  if enable_batch_optimization:
    return batched_helpers.batched_map_full_dataset(steps, realign_batch,
                                                    batch_size)
  else:
