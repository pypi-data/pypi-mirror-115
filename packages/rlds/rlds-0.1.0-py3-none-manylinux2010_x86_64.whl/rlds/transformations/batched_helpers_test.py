...
class StepBatchTest(transformations_testlib.TransformationsTest):
  ...
  def test_batched_map_full_dataset(self):
    steps_dataset = tf.data.Dataset.from_tensor_slices(self.steps)
    ...
    expected_result = steps_dataset.map(increase_reward)
    result = batched_helpers.batched_map_full_dataset(steps_dataset,
                                                      increase_reward)

    self.expect_equal_datasets(result, expected_result)

  def test_batched_reduce_full_dataset(self):
    steps_dataset = tf.data.Dataset.from_tensor_slices(self.steps)

    def sum_reward(step):
      return np.sum(step[rlds_types.REWARD])

    expected_result = steps_dataset.reduce(
        0., lambda accum, step: accum + step[rlds_types.REWARD])
    result = batched_helpers.batched_reduce_full_dataset(
        steps_dataset, sum_reward)
