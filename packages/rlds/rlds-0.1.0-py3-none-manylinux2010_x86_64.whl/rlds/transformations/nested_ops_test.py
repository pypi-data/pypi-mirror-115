...
class NestedOpsTest(transformations_testlib.TransformationsTest):
  ...
  def setUp(self):
    ...
    def episode_generator():
      for i in range(0, 2):
        if i == 0:
          yield {rlds_types.STEPS: self.steps1_dataset}
        else:
          yield {rlds_types.STEPS: self.steps2_dataset}
      return

    self.episodes_dataset = tf.data.Dataset.from_generator(
        episode_generator,
        output_signature={
            rlds_types.STEPS:
                tf.data.DatasetSpec.from_value(self.steps1_dataset)
        })
  ...
  def test_nested_map(self):
    ...
    normalized_ds = nested_ops.map_nested_steps(self.episodes_dataset,
                                                normalize_step)
  ...
  def test_total_sum_tfdata(self):
    expected_sum = {
        rlds_types.OBSERVATION: {
            'field0': [6., 6.],
            'field1': [6., 12.],
    ...
    zero_step = {
        ...
        rlds_types.ACTION: (0, 0, 0),
    }

    obs_action = nested_ops.map_nested_steps(
        self.episodes_dataset, lambda step:
        {k: step[k] for k in [rlds_types.OBSERVATION, rlds_types.ACTION]})

    total_sum = nested_ops.sum_nested_steps(
        obs_action, zero_step, optimization_batch_size=0)

    self.expect_nested_dict_equality(total_sum, expected_sum)

  def test_total_sum_batched(self):
    expected_sum = {
        rlds_types.OBSERVATION: {
            'field0': [6., 6.],
            'field1': [6., 12.],
    ...
    total_sum = nested_ops.sum_nested_steps(obs_action, zero_step)

    self.expect_nested_dict_equality(total_sum, expected_sum)

  def test_sum_episode_tfdata(self):
    expected_sum = {
        rlds_types.OBSERVATION: {
            'field0': [1., 3.],
            'field1': [3., 3.],
    ...
    total_sum = nested_ops.sum_dataset(
        obs_action,
        dataset_concat.zeros_from_spec(obs_action.element_spec),
        optimization_batch_size=0)

    self.expect_nested_dict_equality(total_sum, expected_sum)

  def test_sum_episode_batched(self):
    expected_sum = {
        rlds_types.OBSERVATION: {
            'field0': [1., 3.],
            'field1': [3., 3.],
    ...
    total_sum = nested_ops.sum_dataset(
        obs_action, dataset_concat.zeros_from_spec(obs_action.element_spec))

    self.expect_nested_dict_equality(total_sum, expected_sum)

  def test_check_final_step_tfdata(self):
    result = nested_ops.check_final_step(
        self.steps1_dataset,
        lambda step: step[rlds_types.IS_TERMINAL],
        optimization_batch_size=0)
    self.assertTrue(result)

  def test_check_final_step_batched(self):
    result = nested_ops.check_final_step(
        self.steps1_dataset, lambda step: step[rlds_types.IS_TERMINAL])
    self.assertTrue(result)

  def test_check_final_step_false_tfdata(self):
    result = nested_ops.check_final_step(
        self.steps1_dataset,
        lambda step: step[rlds_types.IS_FIRST],
        optimization_batch_size=0)
    self.assertFalse(result)

  def test_check_final_step_false_batched(self):
    result = nested_ops.check_final_step(
        self.steps1_dataset, lambda step: step[rlds_types.IS_FIRST])
    self.assertFalse(result)
