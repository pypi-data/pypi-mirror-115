...
- animation (dict; default {show: {type: 'fade', from: 0, to: 1}, hide: {type: 'fade', to: 0}}): Configures UI component visibility animations. This object contains two fields: show and hide.
- closeOnOutsideClick (boolean; default True): Specifies whether to close the UI component if a user clicks outside it
- container (string; optional): Specifies the UI component's container
...
- delay (number; default 0): The delay in milliseconds after which the load panel is displayed
- elementAttr (dict; optional): Specifies the global attributes to be attached to the UI component's container element
- focusStateEnabled (boolean; default False): Specifies whether or not the UI component can be focused
- height (number | string; default 90): Specifies the UI component's height in pixels
...
- indicatorSrc (string; default ''): A URL pointing to an image to be used as a load indicator
...
- message (string; default 'Loading...'): Specifies the text displayed in the load panel. Ignored in the Material Design theme
...
- position (a value equal to: 'bottom', 'center', 'left', 'left bottom', 'left top', 'right', 'right bottom', 'right top', 'top' | dict; default {my: 'center', at: 'center', of: window}): Positions the UI component
...
- shadingColor (string; default 'transparent'): Specifies the shading color. Applies only if shading is enabled.
- showIndicator (boolean; default True): A Boolean value specifying whether or not to show a load indicator
- showPane (boolean; default True): A Boolean value specifying whether or not to show the pane behind the load indicator
- visible (boolean; default False): A Boolean value specifying whether or not the UI component is visible
- width (number | string; default 222): Specifies the UI component's width in pixels
