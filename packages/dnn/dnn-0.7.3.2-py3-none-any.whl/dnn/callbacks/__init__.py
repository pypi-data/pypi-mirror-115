...
def compose (
    ...
    # composed_callbacks.append (TqdmCallback (verbose=1))
    ...
    if datasets.validset and custom_validation:
        if custom_metric:
            ...
            for cm in custom_metric:
                custom_metric = numpy_metric.NumpyMetricCallback (cm, datasets.validset, custom_validation)
        ...
        if callable (learning_rate):
            ...
            if len (learning_rate) == 3:
                ...
                scheduler = scheduler = tf.keras.callbacks.LearningRateScheduler (
                    step_decay (initial_lrate, decay_rate)
                )
...
def conventional (
    ...
    # reload (best_metrics); print ('remove reload')
    ...
    return compose (
        ...
        early_stop = (early_stop, monitor, mode) if early_stop else None,
        learning_rate = learning_rate,
        plots = plots,
        average = average,
        reset_train_dir = reset_train_dir,
