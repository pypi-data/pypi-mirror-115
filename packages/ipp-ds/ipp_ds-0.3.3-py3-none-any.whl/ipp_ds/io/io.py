...
from ipp_ds.io import storage_options
...
def create_blob_service(uri):
    ...
    if ('client_id' in cred.keys()) & ('client_secret' in cred.keys()) & ('tenant_id' in cred.keys()):
        creds = cred.copy()
        creds.pop('account_name')
        credential = ClientSecretCredential(**creds)
    elif 'account_key' in cred.keys():
        credential = cred['account_key']
    else:
        raise "You didn't give any credentials to connect"

    blobService = BlobServiceClient(account_url="{}://{}.blob.core.windows.net".format("https",cred['account_name']), 
