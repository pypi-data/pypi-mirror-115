    ...
    def save(self, ofname=None, zoomed=False, canvas=False, animation=False):
        try:
            ...
            if zoomed:
                ...
                if os.path.splitext(ofname)[1].lower() == '.webp':
                    pass
                elif os.path.splitext(ofname)[1].lower() == '.mp4':
                    from pysmtb.utils import write_mp4
                    write_mp4(frames=ims, fname=ofname)
                elif os.path.splitext(ofname)[1].lower() == '.gif':
                    from pysmtb.utils import write_gif
                    write_gif(ofname, ims)
                else:
