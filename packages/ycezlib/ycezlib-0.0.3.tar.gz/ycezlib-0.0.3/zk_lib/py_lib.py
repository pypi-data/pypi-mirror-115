...
def xls_auto_skip_rows(xls_file_src,match_sht_name_str=r"\d{4}\.[\d]{1,2}"):
    import pandas as pd
    import numpy as np
    ...
    df_concat.insert(0, "姓名",df_concat.loc[:, df_concat.columns[df_concat.columns.str.find("姓名") > -1][-2]] )    #定位倒数第二个个包含姓名的列，将其复制插入到第一列
