    ...
    for line in content.split("\r\n"):
        ...
        if len(arr) >= 2:
            ...
            if key == "Implied Volatility (%)":
                detail["iv"] = float(val) / 100
            if key == "Theta (Daily)":
                detail["theta"] = float(val)
            if key == "Vega":
                detail["vega"] = float(val)
            if key == "Delta (%)":
                detail["delta"] = float(val)/100
            if key == "Bid":
                detail["bid"] = float(val)
            if key == "Ask":
                detail["ask"] = float(val)
            if key == "Volume":
                detail["volume"] = float(val)
            if key == "High":
                detail["high"] = float(val)
            if key == "Low":
                detail["low"] = float(val)
