...
def get_pnl_plot(delta, ret, return_plot=False, alpha=False):
    """
    delta: pd.Series with DatatimeIndex
    ret: pd.Series with DatatimeIndex
    """
    ...
    if alpha:
        (day_pnl.cumsum() - day_ret.cumsum()).plot(ax=ax1, label='Alpha', ls='--', alpha=0.4)
...
def get_composite_pnl_plot(delta, ret, return_plot=False, alpha=False):
    """
    Multi-Assets PnL
    delta: pd.DataFrame with DatatimeIndex
    ret: pd.DataFrame with DatatimeIndex
