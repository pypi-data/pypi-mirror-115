...
def table_print(arr: 'Sequence[Sequence[str]]', colors: 'tuple[str,...]' = (bcolors.WHITE, bcolors.GRAY_LIGHT)):
    color_regex = '\x1b' + r'\[[0-9]+m'
    line_count = len(arr)
    col_count = len(arr[0])
    col_size = [0] * col_count
    for i in range(col_count):
        for j in range(line_count):
            col_size[i] = max(col_size[i], len(re.sub(color_regex, '', arr[j][i])))

    for j, line in enumerate(arr):
        color = colors[j % len(colors)]
        for i, item in enumerate(line):
            extra_size = sum([len(match) for match in re.findall(color_regex, item)], start=0)
            print(colorize(item.rjust(col_size[i] + extra_size, ' '), color), end='  |  ')
            if i == col_count - 1:
                print()
