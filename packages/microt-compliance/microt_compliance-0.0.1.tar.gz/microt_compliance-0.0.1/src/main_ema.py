...
import time
import os
from src.utils.extract_participant_id import extract_participant_id
...
def compute_features_phone(intermediate_participant_path, output_dir_path, date_in_study, decryption_password):
    ...
    df_prompt = src.feature_engineering_ema.prompt_response.preprocess_promptResponse_raw.clean_response_files(
        intermediate_participant_path, date_in_study, participant_id)

    # - Outcome variable: completion status
    completion_status_binary = src.feature_engineering_ema.compliance.create_column(df_prompt['Answer_Status'],
                                                                                    print_stats)
    # - Feature matrix
    ...
    info_dict = src.utils.load_participant_study_info.execute(intermediate_participant_path, output_dir_path)

    # - feature 1: Day of the week
    feature1 = src.feature_engineering_ema.day_of_the_week.create_column(df_prompt['Prompt_Local_DateTime'], print_stats)
    feature_df['DAY_OF_THE_WEEK'] = feature1
    # - feature 2: Time of the day
    feature2 = src.feature_engineering_ema.time_of_the_day.create_column(df_prompt['Prompt_Local_DateTime'], print_stats)
    feature_df['TIME_OF_THE_DAY'] = feature2
    # - feature 3: Days in the study
    feature3 = src.feature_engineering_ema.days_in_study.create_column(df_prompt, info_dict, print_stats)
    feature_df['DAYS_IN_THE_STUDY'] = feature3


    # - feature 4: battery level
    feature5, feature6 = src.feature_engineering_ema.battery_level.create_column(df_prompt['Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
    feature_df['BATTERY_LEVEL'] = feature5
    feature_df['CHARGING_STATUS'] = feature6

    # - feature 5: GPS location
    feature7 = src.feature_engineering_ema.location.create_column(df_prompt['Prompt_Local_DateTime'], intermediate_participant_path, date_in_study, decryption_password)
    feature_df['LOCATION'] = feature7

    # - feature 6: Location related

    # - feature 7: Screen Status
    feature9 = src.feature_engineering_ema.screen_status.create_column(df_prompt['Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
    feature_df['SCREEN_STATUS'] = feature9

    # - feature 10&11: wake and sleep hour
    feature10, feature11 = src.feature_engineering_ema.waking_hours.create_column(df_prompt['Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
    feature_df['WAKE_TIME'] = feature10
    feature_df['SLEEP_TIME'] = feature11

    # - feature 12: Phone lock status
    feature12, feature13 = src.feature_engineering_ema.phone_lock.create_column(df_prompt['Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
