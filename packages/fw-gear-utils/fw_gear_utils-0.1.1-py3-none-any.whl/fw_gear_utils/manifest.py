...
class Manifest:
    ...
    Args:
        manifest (Path-like or dict): Path to a manifest.json file or a
            manifest dictionary. If none is provided, try to load
            manifest.json from the current working directory.

    Attributes:
        manifest (dotty_dict.Dotty): Dictionary (dotty-enhanced version to be
            exact) of the manifest.json file.
    ...
    def get_manifest_from_file(path=None):
        ...
        Args:
            path (Path-like): Path to manifest. If None, look for manifest.json in
            current directory.
    ...
    def _derive_config_schema(self):
        ...
        for key in self.manifest["config"]:
            # Copy constraints, removing 'base' and 'description' keywords which are
            #   not constraints
...
class ManifestValidationError(Exception):
    ...
    def __init__(self, path, errors):
        """Initialize ManifestValidationError."""
        super().__init__()
        self.path = path
        self.errors = errors
