...
# in the future if needed. Note that the lonlat option typical of various healpy
# methods is not required, theta and phi are always in radians and correspond to
# colatitude and azimuth, respectively.
...
def pix2ang(nside, ipix, nest = False):
    return hp.pix2ang(nside, ipix, nest)
...
def get_all_neighbours(nside, theta, phi = None, nest = False):
    return hp.get_all_neighbours(nside, theta, phi, nest)
...
def get_interp_weights(nside, theta, phi=None, nest=False):
    return hp.get_interp_weights(nside, theta, phi, nest)
