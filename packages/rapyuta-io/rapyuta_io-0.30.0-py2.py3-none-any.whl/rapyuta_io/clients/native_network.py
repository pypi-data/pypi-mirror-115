...
class NativeNetwork(PartialMixin, ObjBase):
    ...
    def validate(name, runtime, ros_distro, parameters=None):
        ...
        # TODO: remove after device native networks feature
        if runtime == rapyuta_io.clients.package.Runtime.DEVICE:
            raise InvalidParameterException('device native networks are not supported yet')
        # TODO: write unit test for the below if statement after the above if statement is removed
