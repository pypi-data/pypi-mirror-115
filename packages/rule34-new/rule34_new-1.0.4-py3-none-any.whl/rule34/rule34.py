...
class Rule34:
    ...
    async def download(self, URL, destination, filename):
        try:
            if self.session.closed:
                self.session = aiohttp.ClientSession(loop=self.loop)
            async with self.session.get(URL) as resp:
                ...
                if filename:
                    name = str(filename) + '.' + URL.split("/")[-1].split('.')[-1]
                else:
                    name = URL.split("/")[-1][-7:]
                if destination:
                ...
                while os.path.exists(name):
                    i += 1
                    name = name.split('.')[0]+f' ({i})'+name.split('.')[1]
...
class Sync:
    ...
    def download(self, url, destination=None, filename=None):
        return self.l.run_until_complete(self.r.download(url, destination, filename))