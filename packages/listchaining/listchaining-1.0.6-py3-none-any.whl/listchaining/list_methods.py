...
def last_index_of_method(self: List, searched_element: Any, from_index: int = -1) -> int:
    ...
    '''
    If the required element is in the last 20% of the array, then the reverse of the entire array will be
    disadvantageous in terms of execution speed. Therefore, it makes sense to first check in a regular loop
    the first 5% of the array from the end, and if there is no element in them (and most often, if the array
    ...
    if array_len > 100 and from_index == -1:
        for index in range(array_len - 1, array_len - array_len // 20, -1):
