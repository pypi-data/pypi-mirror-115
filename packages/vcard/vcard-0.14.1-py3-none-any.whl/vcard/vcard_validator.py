...
from typing import Generator, List, MutableMapping, Optional, Sequence, Set, TextIO
...
@contextmanager
def get_file_pointer(filename: str) -> Generator[TextIO, None, None]:
    if filename == "-":
        yield sys.stdin
    else:
...
def validate_file(filename: str, verbose: bool) -> Optional[str]:
    """
...
def unfold_vcard_lines(lines: Sequence[str]) -> List[str]:
    ...
    for index, line in enumerate(lines):
        ...
        if len(line) > VCARD_LINE_MAX_LENGTH_RAW:
            warnings.warn(f"Long line in vCard: {line}")
...
def get_vcard_property(property_line: str) -> VcardProperty:
    ...
    if not property_name.upper() in ALL_PROPERTIES and not re.match(
        "^X-[{0}]+$".format(re.escape(ID_CHARACTERS)), property_name, re.IGNORECASE
    ):
        raise VCardNameError("{0}: {1}".format(NOTE_INVALID_PROPERTY_NAME, property_name), {})
...
def get_vcard_property_parameter(param_string: str) -> VCARD_PROPERTY_PARAMETER:
    ...
    """
    try:
        param_name, values_string = vcard_utils.split_unescaped(  # pylint: disable=unbalanced-tuple-unpacking
            param_string, "="
        )
    except ValueError as error:
        raise VCardItemCountError("{0}: {1}".format(NOTE_MISSING_PARAM_VALUE, str(error)), {}) from error
...
def get_vcard_property_sub_values(value_string: str) -> List[str]:
    ...
    for sub_value in sub_values:
        if not re.match("^[{0}]*$".format(re.escape(VALUE_CHARACTERS)), sub_value):
...
def get_vcard_property_param_values(values_string: str) -> Set[str]:
    ...
    for value in values:
        if not re.match(
            '^[{0}]+$|^"[{1}]+"$'.format(re.escape(SAFE_CHARACTERS), re.escape(QUOTE_SAFE_CHARACTERS)), value
        ):
