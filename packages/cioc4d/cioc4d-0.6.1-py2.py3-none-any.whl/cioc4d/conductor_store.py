...
class ConductorStore(object):
    ...
    def title(self):
        return clean_string(self._container[TITLE])

    def set_title(self, value):
        self._container.SetString(TITLE, clean_string(value))
    ...
    def destination(self):
        return clean_string(self._container[DESTINATION])

    def set_destination(self, value):
        self._container.SetString(DESTINATION, clean_string(value))
    ...
    def custom_range(self):
        return clean_string(self._container[CUSTOM_RANGE])

    def set_custom_range(self, value):
        self._container.SetString(CUSTOM_RANGE, clean_string(value))
    ...
    def scout_frames(self):
        return clean_string(self._container[SCOUT_FRAMES])

    def set_scout_frames(self, value):
        self._container.SetString(SCOUT_FRAMES, clean_string(value))
    ...
    def task_template(self):
        return clean_string(self._container[TASK_TEMPLATE])

    def set_task_template(self, value):
        self._container.SetString(TASK_TEMPLATE, clean_string(value))
    ...
    def autosave_filename(self):
        return clean_string(self._container[AUTOSAVE_FILENAME])

    def set_autosave_filename(self, value):
        self._container.SetString(AUTOSAVE_FILENAME, clean_string(value))
    ...
    def location_tag(self):
        return clean_string(self._container[LOCATION_TAG])

    def set_location_tag(self, value):
        self._container.SetString(LOCATION_TAG, clean_string(value))
    ...
    def emails(self):
        return clean_string(self._container[EMAILS])

    def set_emails(self, value):
        self._container.SetString(EMAILS, clean_string(value))
    ...
    def assets(self):
        return [clean_string(item[1]) for item in self._container.GetContainerInstance(EXTRA_ASSETS)]
    ...
    def set_assets(self, assets=[]):
        ...
        for i, asset in enumerate(assets):
            assets_container.SetFilename(i, clean_string(asset))
    ...
    def reset(self):
        ...
        self.set_metadata([
            ("submitter", "cioc4d {}".format(k.VERSION))
        ])
...
def _get_destination_dir():
    ...
    try:
        render_path = os.path.dirname(
            Path(render_file).fslash()) if render_file else None
    except BaseException:
        render_path = None

    result = render_path if render_path else os.path.join(
        doc_path, "renders") if doc_path else os.path.expanduser("~/renders")
...
def _get_default_task_template():
    ...
    if not render_file:
        if doc_path:
            render_file = os.path.join(doc_path, "renders", "out")
        else:
            render_file = os.path.join(os.path.expanduser("~/renders"), "out")

    result = DEFAULT_TASK_TEMPLATE
    result = "{} -oimage \"{}\"".format(
        result,
        Path(render_file).fslash(with_drive=False))
