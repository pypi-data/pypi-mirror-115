...
class ValidateAbsoluteWindowsPaths(Validator):
    def run(self, _):
        ...
        if bad_assets:
            msg = "Some assets have absolute paths containing Windows drive letters. These files may not be found correctly on Conductor's render nodes. "
            msg += "For best result, you should use relative texture paths, or move the assets to your tex folder and enter the filename only in materials. "
            c4d.WriteConsole("-" * 30)
            c4d.WriteConsole("\n")
            c4d.WriteConsole(msg)
