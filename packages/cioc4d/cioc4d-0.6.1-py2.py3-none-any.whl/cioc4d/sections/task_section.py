...
class TaskSection(CollapsibleSection):
    ...
    def resolve(self, expander, **kwargs):
        ...
        if self.widget.get_visible():
            ...
            if not dest_path:
                c4d.WriteConsole("[Conductor] Can't resolve output image paths. Please check render settings.\n")
                dest_path = ""
        ...
        try:
            ...
            return {
                "tasks_data": "INVALID SEQUENCE - Check frames section! Enter frame-spec such as: 1-10 or 1-10x2 or 1,3,5-10",
            }

        result.update(self._resolve_tasks(raw_template, main_sequence, scout_sequence))
    ...
    def _resolve_tasks(self, template, main_sequence, scout_sequence):
        ...
        try:
            template = c4d.modules.tokensystem.StringConvertTokens(template, rpd())
        except SystemError:
            pass
        ...
        for chunk in chunks:
            ...
            tasks.append({
                "command": _expand_dollar_vars(template, context), 
                "frames": str(chunk)
            })
    ...
    def get_custom_destination(self):
        ...
        result = self.destination_widget.get_value()
        if not result:
            return ""

        try:
            result = c4d.modules.tokensystem.FilenameConvertTokens(result, rpd())
        except SystemError:
            return ""
        ...
        if Path(result).relative:
            ...
            if not docdir:
                return ""
            result =  os.path.join(docdir, result)
        return Path(result).fslash()
    ...
    def generate_template(self):
        ...
        if image_path:
            template = '{} -oimage "{}"'.format(template, image_path.fslash(with_drive=False))

        if multipass_image_path:
            template = '{} -omultipass "{}"'.format(
                template, multipass_image_path.fslash(with_drive=False)
            )
        return template
    ...
    def get_common_destination(self):
        ...
        try:
            return PathList(*out_dirs).common_path().fslash()
...
def _get_image_path(doc_path, render_data):
    ...
    if save_enabled and do_image_save:
        try:
            image_path = c4d.modules.tokensystem.FilenameConvertTokens(render_data[c4d.RDATA_PATH], rpd())
        except SystemError:
            return
        if image_path:
...
def _get_multipass_image_path(doc_path, render_data):
    ...
    if save_enabled and do_image_save:
        try:
            image_path = c4d.modules.tokensystem.FilenameConvertTokens(
                render_data[c4d.RDATA_MULTIPASS_FILENAME], rpd())
        except SystemError:
            return
        if image_path:
            if Path(image_path).relative:
