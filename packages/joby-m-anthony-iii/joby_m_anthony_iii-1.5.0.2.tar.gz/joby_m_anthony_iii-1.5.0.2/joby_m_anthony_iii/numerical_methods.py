...
def diagonality(A):
    ...
    else: raise ValueError(bad_matrix)
...
def eigen_values(A):
    ...
    if np.sum(A.shape[0]) != np.sum(A.shape[1]): raise ValueError("ERROR! " + bad_matrix)
    A = np.array(A)
    sym_r = sp.Symbol('r')
...
def l_2_norm(x, x0=[]):
    ...
    if x0.shape[0] == 0:
        ...
        l2_norm = math.sqrt(spectral_radius(x0*xt))
    else: raise ValueError("ERROR! 'x', and 'x0' must be the same size!")
    return l2_norm
...
def max_iterations(a, b, power, method, k=0, p0=0):
    ...
    else: raise ValueError('ERROR! ' + bad_method)
...
def bisection(f, a, b, power, iter_guess="yes"):
    ...
    if isinstance(f,(FunctionType, sp.Expr)):
        ...
        else: raise ValueError('ERROR! ' + opposite_signs)
    # abort if not expression
    else: raise TypeError('ERROR! ' + must_be_expression)
    return P, ERROR, I
    ...
    if isinstance(f,(FunctionType, sp.Expr)):
        ...
        if f(p0)*f(p1) < 0:
            ...
            if iter_guess == "yes": 
                ...
                N = iter_guess
            # else, break for bad assignment
            else: raise TypeError("ERROR! " + bad_iter)
            i, error = 0, tol*10        # initialize
            # exit by whichever condition is TRUE first
            while error >= tol and i <= N:
                q0, q1 = f(p0), f(p1)
...
def jacobi(A, x0, b, N, power, norm_type):
    ...
    A = np.array(A)
    if np.sum(A.shape[0]) != np.sum(A.shape[1]): raise ValueError("ERROR! " + bad_matrix)
    if np.sum(x0.shape) > np.sum(x0.shape[0]): raise ValueError('ERROR! ' + bad_x0)
    if np.sum(b.shape) > np.sum(b.shape[0]): raise ValueError('ERROR! ' + bad_b)
    if N <= 0 or not isinstance(N, int): raise ValueError('ERROR! ' + bad_N)
    if norm_type != 'l_infinity' and norm_type != 'l_2': raise ValueError('ERROR! ' + bad_type)
    diagonality(A)
    tol = float(10**power)
    n = len(x0)
...
def gauss_seidel(A, x0, b, N, power, norm_type):
    ...
    bad_type = "Desired norm type was not understood. Please choose 'l_infinity' or 'l_2'."
    A = np.array(A)
    if np.sum(A.shape[0]) != np.sum(A.shape[1]): raise ValueError(bad_matrix)
    if np.sum(x0.shape) > np.sum(x0.shape[0]): raise ValueError('ERROR! ' + bad_x0)
    if np.sum(b.shape) > np.sum(b.shape[0]): raise ValueError('ERROR! ' + bad_b)
    if N <= 0 or not isinstance(N, int): raise ValueError('ERROR! ' + bad_N)
    if norm_type != 'l_infinity' and norm_type != 'l_2': raise ValueError('ERROR! ' + bad_type)
...
def successive_relaxation(A, x0, b, N, power, norm_type, w=0):
    ...
    else: raise ValueError("ERROR! " + bad_omega)
    tol = float(10**power)
    n = len(x0)
    k, x0, b, norm = 0, np.reshape(x0,(n,1)), np.reshape(b,(n,1)), tol*10
    xi = np.zeros_like(x0)
