...
class Truss():
    def __init__(self,name,db_dir,port,area_x,area_y,area_z):
    ...
    async def picking(self,index,synced,index2):
        ...
        async def z(self,start,end):
            for interval in np.linspace(start,end,int(abs(start - end) / 40)):
                if index == 0: self.db1[10],self.db1[11],self.db1[12],self.db1[13] = pack('!f',interval)
                elif index == 1: self.db1[26],self.db1[27],self.db1[28],self.db1[29] = pack('!f',interval)
                self.z[index] = interval
                await asyncio.sleep(0.001)
    ...
    async def update(self):
        ...
        try:
            #画钢板
            plate_corner,plate_degree = self.status['corner'],self.status['degree']
            ...
            self.point(plate_p1,f'{plate_p1} {plate_degree}°')
            ...
            self.text(p2,str(drop_pos))

            if self.plate['finished']:
                drop_positions = self.plate['tiling'][-1]
            else:
                tiling_index = self.plate['tiling_index']
                drop_positions = self.plate['tiling'][tiling_index]
    ...
    def text(self,p,s=''):
        self.view.text(p[0],p[1],p[2],s=s)
    def point(self,p,s=''):
        if p:
            self.view.text(p[0],p[1],p[2],s=s)
            self.view.scatter(p[0],p[1],p[2])
