...
class VerifiedHTTPSConnection(_VerifiedHTTPSConnection):
    ...
    def _tunnel(self):
        ...
        logger.info('sending negotiate header')
        ...
        if code == PROXY_AUTHENTICATION_REQUIRED:
            ...
            while True:
                if last_line_is_blank:
                    logger.debug(
                        '* reading a line from response: last_line_is_blank=%s',
                        last_line_is_blank
                    )
                ...
                logger.debug('* this line is blank? %s', this_line_is_blank)
                if last_line_is_blank and this_line_is_blank:
                    logger.info('* draining the connection')
                    self._flush_response_buffer(response)
                    logger.info('* connection successfully drained')
                    break
                ...
                if line.decode("utf-8").startswith(match_string):
                    ...
                    logger.debug('* authenticate message (not yet sent): %s', authenticate_hdr)
                    continue
            ...
            logger.info('* trying to send authenticate message: %s', header_bytes)
            self.send(header_bytes)
            version, code, message, response = self._get_response()

        if code != 200:
            logger.error('* tunnel connection failed. HTTP %s; %s', code, message)
            self.close()
            raise socket.error(
                "Tunnel connection failed: %d %s" % (code, message.strip())
            )
        while self._continue_reading_headers:
            logger.debug('* attempting to manually drain the headers')
            line = response.fp.readline()
