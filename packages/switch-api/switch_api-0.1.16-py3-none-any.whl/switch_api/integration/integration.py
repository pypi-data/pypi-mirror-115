...
def upsert_device_sensors(df: pandas.DataFrame, api_inputs: ApiInputs, tag_columns: list = None,
    ...
    data_frame_grpd = data_frame.groupby('DeviceCode')
    chunk_list = []
    for name, group in data_frame_grpd:
        logger.info('Upserting data for DeviceCode = %s', str(name))
        ...
        if response.status_code != 200:
            logger.error("API Call was not successful. Response Status: %s. Reason: %s.", response.status_code,
                         response.reason)
            chunk_list += [{'Chunk': str(name), 'DeviceCountToUpsert': 1, 'SensorCountToUpsert': str(group.shape[0]),
        ...
        if response_data_frame.shape[1] > 0:
            ...
            if invalid_rows.shape[0] > 0:
                ...
                chunk_list += [
                    {'Chunk': str(name), 'DeviceCountToUpsert': 1, 'SensorCountToUpsert': str(group.shape[0]),
                     'response_status': response_status, 'response_df': response_data_frame,
                     'invalid_rows': invalid_rows}]
            else:
                chunk_list += [{'Chunk': str(name), 'DeviceCountToUpsert': 1,
...
def upsert_workorders(df: pandas.DataFrame, api_inputs: ApiInputs, save_additional_columns_as_slices: bool = False):
    ...
    # optional_columns_df = pandas.DataFrame({'ColumnName': ['SubType', 'Vendor', 'VendorId', 'EquipmentClass',
    #                                                        'RawEquipmentClass', 'EquipmentLabel', 'RawEquipmentId',
    #                                                        'TenantId', 'TenantName', 'NotToExceedCost', 'TotalCost',
    #                                                        'BillableCost',  'NonBillableCost', 'Location',
    #                                                        'RawLocation', 'ScheduledStartDate',
...
def upsert_timeseries_ds(df: pandas.DataFrame, api_inputs: ApiInputs, is_local_time: bool = True,
    ...
    """Appends data to Timeseries_Ds table.
