...
def get_device_sensors(api_inputs: ApiInputs, include_tag_groups: Union[list, bool] = False,
    ...
    """
    if (include_tag_groups is True or type(include_tag_groups) == list) and (include_metadata_keys is True or type(include_metadata_keys) == list):
        logger.exception('Tags and Metadata cannot be returned in a single call. Please set either include_tag_groups or include_metadata_keys to False. ')
        return
...
def get_data(query_text, api_inputs: ApiInputs, query_language: QUERY_LANGUAGE = "sql"):
    ...
    query_language : QUERY_LANGUAGE
        The query language the query_text is written in.
