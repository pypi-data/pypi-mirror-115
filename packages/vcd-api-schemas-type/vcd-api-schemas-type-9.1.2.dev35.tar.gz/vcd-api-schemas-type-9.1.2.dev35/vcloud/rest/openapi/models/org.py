...
class Org(object):
    ...
    def __init__(self, id=None, name=None, display_name=None, description='', is_enabled=False):
        """
    ...
    def display_name(self, display_name):
        ...
        if display_name is not None and not re.search('(?=.*\\S.*)^[^\\p{Zl}\\p{Zp}\\p{C}]*$', display_name):
            raise ValueError("Invalid value for `display_name`, must be a follow pattern or equal to `/(?=.*\\S.*)^[^\\p{Zl}\\p{Zp}\\p{C}]*$/`")
