...
class ContinEnv(gym.Env):
  def __init__(self):
    ...
    #self.observation_space = spaces.Tuple([self.observation_box, self.Y_obs_space])
    ...
    self.state=self.df.sample(n=1, random_state=1).values.reshape(3,)[0:2]
...
#"action" argument is one value in the range of the action space (logit transform)
  def step(self, action):
    ...
    data = []
    for self.state in self.df.itertuples(index=False):
      Xs = self.state[0]
      Xa = self.state[1]
      Y = self.state[2]
      Xsa=(self.thetas[0])+(self.thetas[1])*(Xs)+(self.thetas[2])*(Xa)
      rho2 = (1/(1+np.exp(-Xsa)))  #prob of Y=1
      g2 = ((action) + 0.5*((action)+math.sqrt(1+(action)**2)))*(1-rho2) + ((action) - 0.5*((action)+math.sqrt(1+(action)**2)))*rho2
      if (rho2>0.2):
        Xa=g2
      else: Xa=Xa 
