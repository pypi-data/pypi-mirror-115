...
def create(
    ...
    client.upload_datastore(
        source=source, credential_id=credential['credentialId'], name=name, compression=compression, cluster=cluster
    )
...
@click.pass_context
def delete(ctx, name: str, version: int, credential: str, cluster: Optional[str] = None) -> None:
    """Deletes a datastore."""
    client = Grid()
    client.delete_datastore(name=name, version=version, credential_id=credential, cluster=cluster)
