...
from grid.cli.commands.git import execute_git_command
from grid.cli.core.repository import LocalRepository
from grid.cli.datastore import create_datastore_session, DatastoreUploadSession
...
"""
    ...
    def delete_datastore(self, name: str, version: int, credential_id: str, cluster: Optional[str] = None):
        """
        ...
        params = {'name': name, 'version': version, 'credentialId': credential_id, 'clusterId': cluster}
    ...
    def validate_and_configure_datastore(grid_config: dict):
        ...
        """
        from grid.cli.core import Datastore, Team
        ...
        if len(split) > 2:
            ...
            datastores = []
            for team in teams:
                datastores.extend(Datastore.get_all(team=team))
        else:
            datastores = Datastore.get_all()
        usable_version = -1
        datastore_id = None
    ...
    def list_datastores(is_global: Optional[bool] = False):
        ...
        if is_global:
            ...
            for team in teams:
                try:
                    team_datastores = Datastore.get_all(team=team)
                    datastores.extend(team_datastores)
                    ...
                    if env.DEBUG:
                        click.echo(e)
        if not is_global or len(teams) == 0:
            try:
                user_datastore = Datastore.get_all(team=None)
                datastores.extend(user_datastore)
            except ResourceNotFound:
                # No datastores found
                pass
            except Exception as e:
        ...
        if is_global:
            table_cols = ["Credential Id", "Name", "Version", "Size", "Created At", "Created By", "Status"]
        else:
        ...
        for ds in sorted_datastores:
            created_at = date_string_parse(ds.created_at)
            created_at = f'{created_at:%Y-%m-%d %H:%M}'
            size = ds.size
            if size or size == 0:
                size = humanize.naturalsize(size * (1024**2))

            if ds.snapshot_status in {'pending', 'preparing', 'queued', 'running'}:
                status = 'Optimizing'
            else:
                status = ds.snapshot_status.title() if ds.snapshot_status else "Unknown"

            if is_global:
                username = ds.user_details['username'] if ds.user_details else None
                table.add_row(ds.credential_id, ds.name, ds.version, size, created_at, username, status)
            else:
                table.add_row(ds.credential_id, ds.name, ds.version, size, created_at, status)

        console = Console()
        console.print(table)

    def upload_datastore(
        self, source: str, name: str, credential_id: str, compression: bool, cluster: Optional[str] = None
    ):
        """
