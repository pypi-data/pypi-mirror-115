...
from grid.cli.tar import get_split_size, tar_path
from grid.cli.uploader import S3Uploader, UploadProgressCallback
...
def create_datastore(name: str, credential_id: str, client: Client, source: str = None, cluster: Optional[str] = None):
    ...
    params = {'name': name, 'credentialId': credential_id, 'source': source, 'clusterId': cluster}
...
def create_datastore_session(
    name: str, source: str, credential_id: str, compression: bool, client: Client, cluster: Optional[str] = None
):
    """Creates datastore session based on source"""
    check_source(source)

    if source.startswith("http"):
        return DatastoreRemoteUpload(
            name=name, source_url=source, credential_id=credential_id, client=client, cluster=cluster
        )

    s = DatastoreUploadSession(name=name, source_dir=source, credential_id=credential_id, compression=compression)
...
class DatastoreRemoteUpload:
    ...
    """
    def __init__(self, name: str, source_url: str, credential_id: str, client: Client, cluster: Optional[str] = None):
        self.name = name
        self.source_url = source_url
        self.credential_id = credential_id
        self.client = client
        self.logger = logging.getLogger("grid.cli.datastore")
...
class DatastoreUploadSession(UploadProgressCallback):
    ...
    def uploaded_target_file(self) -> str:
        ...
        source_path = Path(self.source_dir)
        if source_path.is_dir():
            # Since we're compressing the directory ourselves, we know
            # the file name is ending with .tar.gz
            return "data.tar.gz"
        else:
    ...
    def _create_datastore(self):
        ...
        datastore_id, datastore_version = create_datastore(
            ...
            credential_id=self.credential_id,
            client=self.client,
            source=self.uploaded_target_file,
    ...
    def _compress_source(self):
        source_path = Path(self.source_dir)
        if source_path.is_file():
            # We assume we already checked that the file is a compressed archive.
            # Skip compressing
            # We don't know the uncompressed size, so leaving it None so we can compute it
            # online.
    ...
    def _mark_upload_complete(self):
        ...
        params = {'datastoreId': self.id, 'sizeMib': math.ceil(self.original_size / (1024**2))}
