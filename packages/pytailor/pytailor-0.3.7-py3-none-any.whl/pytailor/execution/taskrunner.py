...
class TaskRunner(APIBase):
        ...
        if download:
            ...
            for filetag in download:
                try:
                    ...
                    if (isinstance(e, BackendResponseError) and
                            "Could not get download urls." in str(e)):
                        pass
                    elif (isinstance(e, httpx.HTTPStatusError) and
                            e.response.status_code == httpx.codes.NOT_FOUND):
                        pass
                    else:
                        raise
    ...
    def __update_context_with_file_info(self, wf_id: str, filetag: str,
                                        use_storage_dirs: bool):
        # get workflow details
        with RestClient() as client:
            wf = self._handle_request(
                client.get_workflow,
                self.__project_id,
                wf_id,
                error_msg="Could not get workflow details.",
            )
