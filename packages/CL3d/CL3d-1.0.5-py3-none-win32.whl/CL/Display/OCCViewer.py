...
from OCC.Core.AIS import AIS_Shape, AIS_Shaded, AIS_TexturedShape, AIS_WireFrame, AIS_Shape_SelectionMode, AIS_Manipulator
...
from OCC.Core.Aspect import Aspect_TOTP_RIGHT_LOWER, Aspect_TOTP_LEFT_LOWER, Aspect_FM_STRETCH, Aspect_FM_NONE
...
modes = itertools.cycle([TopAbs_FACE,TopAbs_WIRE, TopAbs_EDGE,
                         TopAbs_VERTEX,
...
class Viewer3d(Display3d):
    def __init__(self, window_handle, parent=None):
        ...
        self.disable_shift_select = False
    ...
    def display_graduated_trihedron(self):
        self.View.GraduatedTrihedronDisplay(Graphic3d_GraduatedTrihedron())
    ...
    def display_triedron(self):
        ...
        self.View.TriedronDisplay(Aspect_TOTP_LEFT_LOWER, Quantity_Color(Quantity_NOC_BLACK), 0.1, 0)
    ...
    def DisplayMessage(self, point, text_to_write, height=None, message_color=[1, 0.9, 0.6], update=False):
        """
    ...
    def DisplayColoredShape(self, shapes, color='YELLOW', update=False, ):
        if isinstance(color, str):
            ...
            raise ValueError(
                'color should either be a string ( "BLUE" ) or a Quantity_Color(0.1, 0.8, 0.1) got %s' % color)
    ...
    def SetSelectionMode(self, mode=None):
        ...
        if mode is None:
            ...
            # if topo_level == TopAbs_FACE:
            #     self.Context.HighlightStyle(3).SetDisplayMode(1)
            #     self.Context.HighlightStyle(3).SetColor(rgb_color(1, 1, 1))
    ...
    def SetSelectionModeCOMPOUND(self):
        self.SetSelectionMode(TopAbs_COMPOUND)
    ...
    def SelectArea(self, Xmin, Ymin, Xmax, Ymax):
        ...
        while self.Context.MoreSelected():
            if self.Context.HasSelectedShape():
                ...
                obj = self.Context.SelectedInteractive()
                obj.owner = self.Context.SelectedOwner()
                ais_objs.append(obj)
            self.Context.NextSelected()
        # callbacks
        for callback in self._select_callbacks:
            callback(self.selected_shapes, Xmin, Ymin, Xmax, Ymax, ais_objs)

    def ShiftSelectArea(self, Xmin, Ymin, Xmax, Ymax):
        if self.disable_select:
            return
        if self.disable_shift_select:
            return
        self.Context.ShiftSelect(Xmin, Ymin, Xmax, Ymax, self.View, True)
        self.Context.InitSelected()
        # reinit the selected_shapes list
        self.selected_shapes = []
        ais_objs = []
    ...
    def Select(self, X, Y):
        ...
        # callbacks
        for callback in self._select_callbacks:
            callback(self.selected_shapes, X, Y, obj)
    ...
    def ShiftSelect(self, X, Y):
        ...
        # callbacks
        for callback in self._select_callbacks:
            callback(self.selected_shapes, X, Y, ais_objs)
