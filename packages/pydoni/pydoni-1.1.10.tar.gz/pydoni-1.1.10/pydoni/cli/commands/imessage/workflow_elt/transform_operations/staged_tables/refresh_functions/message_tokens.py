...
def refresh_message_tokens(pg, vb, tag, dry_run, table_schema, table_name, limit, batch_size, message_tokens_table):
    """
    ...
    sql = f"""
    select
        message_uid :: text as message_uid
        , source
        , "ROWID" :: int as "ROWID"
        , "text"
    from
        imessage.message_vw
    where
        is_text = true  -- Not a 'like' or other emote reaction
        and message_uid not in (select distinct message_uid from {table_schema}.{table_name})  -- Not already in target table
    order by
        message_date asc
    """
    message = pg.read_sql(sql)
    vb.info(f'Total eligible messages: {len(message)}', tag=tag)
    ...
    if isinstance(batch_size, int):
        ...
        vb.info(f'Expanding {n_batches} batch{plural} of {batch_size} messages ({len(message)} total) to one-row-per-token format', tag=tag)
    else:
        is_batched = False
        target_indices = [list(message.index)]
        vb.info(f'Expanding {len(message)} messages to one-row-per-token format', tag=tag)
    ...
    for i, targets in enumerate(target_indices):
        ...
        if is_batched:
            vb.info(f'Executing batch {i + 1} of {n_batches}', tag=tag)
        ...
        existing_message_tokens = pg.read_table(table_schema, table_name)
        ...
        vb.info(f"Inserted {len(message_tokens)} tokens to {table_schema}.{table_name}", tag=tag)
    ...
    result['n_messages_processed'] = len(message)
    result['n_message_tokens_inserted'] = total_tokens_inserted
    vb.info(f'Total messages processed: ' + str(len(message)), tag=tag)
    vb.info(f'Total message tokens inserted: ' + str(total_tokens_inserted), tag=tag)
