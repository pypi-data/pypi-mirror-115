...
def getAwsInstanceDetails(config, authentication_token, account_id, host, log, debug):
    ...
    metadata_token_url = config['aws_metadata_service_token_url']
    aws_token = _fetch_aws_metadata_token(metadata_token_url, log)
    log.info("token {}".format(aws_token))

    locator_url = config['aws_instance_id_locator_url']
    instance_id = getAwsInstanceId(locator_url, log, token=aws_token)
...
def getAwsInstanceId(locator_url, log, token=None):
    response = None
    if locator_url:
...
def _fetch_aws_metadata_token(token_url, token_ttl=21600, log=logging.getLogger('spotinst-agent')):
    response = None
