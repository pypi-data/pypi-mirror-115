...
from .handlers import DefaultOutputStyleHandler, ReportPortalHandler
...
def into_event_stream(config: Config) -> events.EventStream:
    ...
    inner = runner.from_schema(loaded_schema, **from_schema_kwargs).execute()
...
def cli_entrypoint(input_config: TestConformanceCommandConfig):
    ...
    if input_config.report_to_saas:
        ...
        handlers.append(ReportPortalHandler("test", token, input_config.schema))
