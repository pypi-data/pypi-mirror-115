...
class FlowsAPI(ModelHelper):
    ...
    def bo_access_rebate_instance(self):
        """
        @precondition: the player is logged in on bo.

        :return: whether access rebate instance successfully

        """
        logger.info('[AtLog] ----- Click the Rebate link')
        self.open_rebate_menu(self.EM.bo_rebate_link_xpath)
        logger.info('[AtLog] ----- Click the Rebate instance')
        self.is_opened_rebate_instance_search(self.EM.bo_rebate_create_new_instance_link_css)
        return
