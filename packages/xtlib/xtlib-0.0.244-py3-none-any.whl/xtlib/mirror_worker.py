...
def writer_worker(q, mirror_delay_mins, store, show_calls):
    ...
    while True:
        ...
        for i in range(len(pending_items)):
            j = len(pending_items)-1 - i
            ready, ws_name, run_name, blob_path, fn, job_id = pending_items[j]

            if process_all or time.time() >= ready:
                # run it now
                if show_calls:
                    console.print("ww: writing: ", fn)
                store.upload_file_to_run(ws_name, run_name, blob_path, fn, job_id)
                del pending_items[j]
