...
/** 
  ...
  a permutation ``i -> p_out[i]``, ``0 <= i < 24``. The result is 
  returned in the array ``p_out[i]``.
...
// (without any indication of failure) if p1 is not in Mat24.
{
    ...
    // %%FOR i in range(6)
    j8 = q_inv[0];
    q_inv[q[0]] = q_inv[q[j8]];
    q[j8] = q[0];
    res = res * 8 + j8 - 0;
    j8 = q_inv[1];
    q_inv[q[1]] = q_inv[q[j8]];
    q[j8] = q[1];
    res = res * 7 + j8 - 1;
    j8 = q_inv[2];
    q_inv[q[2]] = q_inv[q[j8]];
    q[j8] = q[2];
    res = res * 6 + j8 - 2;
    j8 = q_inv[3];
    q_inv[q[3]] = q_inv[q[j8]];
    q[j8] = q[3];
    res = res * 5 + j8 - 3;
    j8 = q_inv[4];
    q_inv[q[4]] = q_inv[q[j8]];
    q[j8] = q[4];
    res = res * 4 + j8 - 4;
    j8 = q_inv[5];
    q_inv[q[5]] = q_inv[q[j8]];
    q[j8] = q[5];
    res = res * 3 + j8 - 5;
    // %%END FOR
